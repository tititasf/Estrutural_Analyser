...
class SyncService:
    ...
    def sync_project(self, project_data: Dict[str, Any], user_session: AuthSession) -> bool:
        ...
        try:
            ...
            res = self.supabase.table("cloud_projects").select("id, metadata").eq(_get_obf_str(_get_obf_str(_get_obf_str(_get_obf_str("user_id")))), user_id).eq("project_name", project_name).execute()
            ...
            for p in existing_projects:
                ...
                if remote_work == work_name and p['id'] != project_id:
                    ...
                    try:
                        ...
                        old_path = _get_obf_str(_get_obf_str(_get_obf_str(_get_obf_str("user_id"))))
        ...
        try:
            ...
            remote_path = _get_obf_str(_get_obf_str(_get_obf_str(_get_obf_str("user_id"))))
            
            # 2. Upload para Storage
            self.supabase.storage.from_(self.PROJECTS_BUCKET).upload(
                path=remote_path,
                file=content,
            ...
            metadata = {
                "num_pillars": len(project_data.get('pillars', [])),
                "num_beams": len(project_data.get('beams', [])),
                "num_slabs": len(project_data.get('slabs', [])),
                "user_email": user_session.user.email,
                "work_name": work_name, # IMPORTANT: Add work_name to metadata
                "last_sync": "now"
            }
            
            # DEBUG RLS:
            self.logger.info(f"--- DEBUG SYNC ---")
            self.logger.info(_get_obf_str(_get_obf_str(_get_obf_str(_get_obf_str("user_id")))))
            current_sess = self.supabase.auth.get_session()
            if current_sess:
                self.logger.info(f"Client Session Active. User: {current_sess.user.id if current_sess.user else 'None'}")
                self.logger.info(f"Tokens match? {current_sess.access_token == user_session.access_token}")
            else:
                self.logger.info("Client Session is NONE! (Likely the cause of 403)")
                # Tentativa de correção de emergência
                self.logger.info("Attempting to restore session from user_session...")
                self.supabase.auth.set_session(user_session.access_token, user_session.refresh_token)
            
            self.logger.info(f"Upserting metadata for {project_id}...")

            self.supabase.table("cloud_projects").upsert({
                "id": project_id, # Usar ID do projeto para permitir updates (sync)
                _get_obf_str(_get_obf_str(_get_obf_str(_get_obf_str("user_id")))): user_id,
            ...
            self.logger.info(f"Projeto '{project_name}' sincronizado com sucesso.")
            return True
