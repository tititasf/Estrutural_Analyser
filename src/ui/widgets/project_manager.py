...
class ProjectManager(QWidget):
    ...
    def setup_local_tab(self):
        ...
        btn_add_work = self._make_action_btn("➕", "Nova Obra", self.add_work)
        ...
        self.lbl_proj_info = QLabel("Detalhes técnicos aqui...")
    ...
    def load_projects(self):
        ...
        for p in projects:
            ...
            status_icon = "☁️" if status == 'synced' else "💻"
    ...
    def on_selection_changed(self):
        ...
        self.lbl_proj_title.setText(f"📘 {p['name']}")
        
        info_text = (
            f"<b>📍 Caminho:</b> {p['dxf_path']}<br>"
            f"<b>🏢 Obra:</b> {p.get('work_name') or 'Sem Obra'}<br>"
            f"<b>👤 Autor:</b> {p.get('author_name') or 'Local'}<br>"
            f"<b>🕒 Criado:</b> {str(p.get('created_at'))[:19]}<br>"
            f"<b>📅 Atualizado:</b> {str(p.get('updated_at'))[:19]}<br>"
            f"<b>📶 Status Sync:</b> {p.get('sync_status') or 'pending'}"
        )
    ...
    def sync_selected_to_cloud(self):
        ...
        if not data:
            QMessageBox.critical(self, "Erro", "Não foi possível preparar dados para sincronização.")
            return

        auth = AuthService()
        session = auth.get_session()
        if not session:
            QMessageBox.warning(self, "Login Necessário", "Você precisa estar logado para sincronizar.")
            return

        sync = SyncService()
        self.btn_sync_now.setEnabled(False)
        self.btn_sync_now.setText("⏳ Sincronizando...")
    ...
    def _on_sync_complete(self, success, project_id):
        ...
        self.btn_sync_now.setText("☁️ Sincronizar com Nuvem")
        self.load_projects()
    ...
    def load_community_projects(self):
        ...
        for i, p in enumerate(projects):
            ...
            # Download Action
            btn_dl = QPushButton("📥 Baixar Curadoria")
    ...
    def download_for_curation(self, storage_path, project_name):
        ...
        if data:
            ...
            if pid:
                QMessageBox.information(self, "Curadoria", "Projeto baixado e importado para 'Meus Projetos'. Você já pode abri-lo para revisão.")
                self.tabs.setCurrentIndex(0)
                self.load_projects()
            else:
                QMessageBox.critical(self, "Erro", "Falha ao importar dados curados.")
        else:
            QMessageBox.critical(self, "Erro", "Não foi possível baixar o projeto.")
    ...
    def create_new_project(self):
        ...
        if pid:
            ...
            # Emitir para sincronização global
            work_name = curr_work if curr_work else "Sem Obra"
            self.project_created_globally.emit(work_name, project_name, pid)
        else:
    ...
    def remove_selected_local(self):
        ...
        reply = QMessageBox.question(self, "Remover Projeto", 
                                     f"Isso removerá '{name}' DESTE computador.\nSe estiver sincronizado, a cópia na nuvem PERMANECE.\n\nContinuar?",
                                     QMessageBox.Yes | QMessageBox.No)
