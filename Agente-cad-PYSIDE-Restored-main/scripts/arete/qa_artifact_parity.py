...
def run_parity(
    ...
    def compute() -> dict[str, Any]:
        ...
        checks = [evaluate_check(check, values) for check in spec.get("checks", [])]
        ...
        return {
            ...
            "checks": checks,
            "provenance": {
                "chain": "contract -> payload -> dxf -> html",
    ...
    result["runtime"] = {
        ...
        "variants": len(variants), "fields": len(spec.get("fields", [])),
        "checks": len(spec.get("checks", [])),
    }
    return result
