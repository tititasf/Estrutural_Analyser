...
# Match from 'def _sync_robos_pavimentos' all the way down to the line before 'def _current_pavement_name'
match = re.search(r'    def _sync_robos_pavimentos\(self.*?(?=    def _current_pavement_name)', content, re.DOTALL)
if match:
    content = content.replace(match.group(0), "")

# Also remove any leftover _sync_robos_obras_e_pavimentos just in case there are multiple
match2 = re.search(r'    def _sync_robos_obras_e_pavimentos\(self.*?(?=    def _current_pavement_name)', content, re.DOTALL)
while match2:
    content = content.replace(match2.group(0), "")
    match2 = re.search(r'    def _sync_robos_obras_e_pavimentos\(self.*?(?=    def _current_pavement_name)', content, re.DOTALL)

match3 = re.search(r'    def _sync_robos_obras\(self.*?(?=    def _current_pavement_name)', content, re.DOTALL)
while match3:
    content = content.replace(match3.group(0), "")
    match3 = re.search(r'    def _sync_robos_obras\(self.*?(?=    def _current_pavement_name)', content, re.DOTALL)
...
print("Indentation fixed and code replaced cleanly!")
