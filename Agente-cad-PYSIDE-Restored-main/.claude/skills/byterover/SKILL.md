---
name: byterover
description: "Use ByteRover selectively to retrieve or curate stable, approved project patterns, decisions, and architectural rules in .brv/context-tree. Do not query an empty tree or use it for transient work, unvalidated evidence, or production RAG promotion."
---
...
## CAD-ANALYZER Project Policy

- ByteRover is development memory for human-operated CLI sessions. It is not the
  source of truth for N1, N2, N3, N4, Arete, FAISS, Chroma, or
  `project_data.vision`.
- Human edits, captured events, automatic diagnostics, and open Arete findings
  remain evidence/T0. Never curate them as validated knowledge.
- Curate only stable decisions or patterns explicitly approved by the owner and
  trace them to current source files or canonical documentation.
- Use `brv search` first because it is local and has no LLM cost. Use `brv query`
  only when the tree has relevant content and synthesis is useful.
- Keep HITL review enabled. Never auto-approve high-impact or truth-promoting
  changes.
- Do not run `brv vc push`, `pull`, `clone`, or configure a remote unless the
  owner explicitly requests cloud synchronization.

## Workflow
1. **Retrieve when useful:** Search stored context only when the task may depend
   on prior project decisions and the tree is not empty.
2. **Curate deliberately:** Save only durable, owner-approved knowledge. Do not
   curate automatically after every implementation.
