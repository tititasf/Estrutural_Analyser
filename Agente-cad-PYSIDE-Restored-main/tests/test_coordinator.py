"""
test_coordinator.py — Testa DataCoordinator (singleton + signals PySide6).

Guarda: imports PySide6/DataCoordinator DENTRO do setUp para evitar crash headless.
"""
import os
import sys
import unittest
...
@pytest.mark.skipif(
    ...
    reason="Sem display (headless) — PySide6 signals skipped",
)
class TestDataCoordinator(unittest.TestCase):
    ...
    def setUpClass(cls):
        _setup_path()
        PySide6_QtCore = pytest.importorskip("PySide6.QtCore", reason="PySide6 não disponível")
        QCoreApplication = PySide6_QtCore.QCoreApplication
        ...
        try:
            from src.core.services.data_coordinator import DataCoordinator, get_coordinator  # noqa: PLC0415
            cls.DataCoordinator = DataCoordinator
            cls.get_coordinator = staticmethod(get_coordinator)
        except ImportError as exc:
            pytest.skip(f"data_coordinator não importável: {exc}")

    def setUp(self):
        self.coordinator = self.__class__.get_coordinator()
        self.coordinator._current_project = None
        self.coordinator._current_pavement = None
        self.coordinator._current_work = None
        self.received_signals: dict = {}
    ...
    def test_singleton_behavior(self):
        c1 = self.__class__.DataCoordinator()
        c2 = self.__class__.get_coordinator()
