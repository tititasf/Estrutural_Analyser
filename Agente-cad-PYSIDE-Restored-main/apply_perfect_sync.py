...
if "def _sync_robos_obras_e_pavimentos" in content:
    print("Já aplicado.")
...
# Also inject into _on_work_changed
old_on_work = """        # Reverse sync: SA sidebar combo de obras
...
new_on_work = """        # Reverse sync: SA sidebar combo de obras
        if hasattr(self, 'sa_cmb_obras'):
            sa_idx = self.sa_cmb_obras.findData(work_name)
            if sa_idx >= 0 and self.sa_cmb_obras.currentIndex() != sa_idx:
                self.sa_cmb_obras.blockSignals(True)
        ...
        # Força todos os robôs a espelharem a Obra global e seus pavimentos limpos
        self._sync_robos_obras_e_pavimentos(global_work_name=work_name)

        # Nao auto-selecionar pavimento: carregar projeto apenas apos escolha explicita."""

content = content.replace(old_on_work, new_on_work)

# Also update _refresh_nav_combos
old_refresh = """        self.cmb_obras.blockSignals(False)"""
new_refresh = """        self.cmb_obras.blockSignals(False)
        self._sync_robos_obras_e_pavimentos()"""
content = content.replace(old_refresh, new_refresh)

with open('main.py', 'w', encoding='utf-8') as f:
    f.write(content)
print("Perfect sync applied!")
