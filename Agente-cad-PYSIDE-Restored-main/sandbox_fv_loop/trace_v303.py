...
for b in beam_labels:
    if b['text'].strip() == 'V303':
        pos = b['pos']
        lines = bt._capture_geometry(pos, orientations['V303'], orientations, beam_labels, 'V303')
        geo = bt._process_beam_geometry(pos, lines, orientations['V303'])
        for seg in geo['classified']['seg_bottom']:
            xs = [p[0] for p in seg]
            ys = [p[1] for p in seg]
            print(f"ACCEPTED BOTTOM: X=[{min(xs):.0f}, {max(xs):.0f}] Y=[{min(ys):.0f}, {max(ys):.0f}] dx={max(xs)-min(xs):.1f}")
        for seg in geo['classified']['seg_side_a'] + geo['classified']['seg_side_b']:
            xs = [p[0] for p in seg]
            ys = [p[1] for p in seg]
            print(f"REJECTED SIDE: X=[{min(xs):.0f}, {max(xs):.0f}] Y=[{min(ys):.0f}, {max(ys):.0f}] dx={max(xs)-min(xs):.1f}")
