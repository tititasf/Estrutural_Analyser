...
def fix_beam_tracer(filepath):
    ...
    new_methods = """    def detect_beams(self, texts: List[Dict], all_lines: List[Dict]) -> List[Dict]:
        ...
        # PRE-COMPUTE TRUE ORIENTATIONS AND FULL LINES FOR ALL BEAMS
        pre_beams = []
        for b_text in beam_labels:
            content = b_text['text'].strip()
            pos = b_text['pos']
            is_h, lines = self._grow_geometry(pos)
            pre_beams.append({'name': content, 'pos': pos, 'is_h': is_h, 'raw_lines': lines})
        ...
        for pb in pre_beams:
            ...
            for line in pb['raw_lines']:
                ...
                for other in pre_beams:
                    ...
                    if is_h and abs(op[1] - pos[1]) < 40:
                        other_dist = abs(cx - op[0])
                        if other_dist < my_dist:
                            owns = False
                            break
                    elif not is_h and abs(op[0] - pos[0]) < 40:
                        other_dist = abs(cy - op[1])
                        if other_dist < my_dist:
                            owns = False
                            break
                if owns:
            ...
            geometry = self._process_beam_geometry(pos, owned_lines, all_lines)
    ...
    def _grow_geometry(self, pos: Tuple[float, float]) -> Tuple[bool, List[List[Tuple[float, float]]]]:
        def _grow(is_h):
            contain_long = 4000
            contain_trans = 80
            if is_h:
            ...
            if min_dist == float('inf'):
                if is_h:
                    ...
                    min_dist = abs(cy - pos[1])
                else:
                    cx = sum(all_x)/len(all_x)
                    min_dist = abs(cx - pos[0])
        ...
        score_h = len_h / max(1.0, dist_h)
        score_v = len_v / max(1.0, dist_v)
        
        is_horiz = score_h >= score_v
        return is_horiz, (lines_h if is_horiz else lines_v)

    def _process_beam_geometry(self, pos: Tuple[float, float], raw_lines: List[List[Tuple[float, float]]], all_lines: List[Dict]) -> Dict:
        beam_geometry = {
            'lines': raw_lines,
            'texts': [],
            'dimension_texts': [],
