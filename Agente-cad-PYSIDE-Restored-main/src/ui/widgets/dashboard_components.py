...
class DocumentationListWidget(QFrame):
    ...
    def set_items(self, categories_data):
        ...
        # Ordenação natural das categorias (1, 2, 3... 10, 11, 12 ao invés de 1, 10, 11, 2, 3...)
        import re
        def natural_sort_key(text):
            """Converte texto em lista de inteiros e strings para ordenação natural"""
            return [int(c) if c.isdigit() else c.lower() for c in re.split(r'(\d+)', text)]
        
        sorted_categories = sorted(categories_data.items(), key=lambda x: natural_sort_key(x[0]))
        
        for name, docs in sorted_categories:
            self._add_category_group(name, docs)
            
        # TODAS as pastas começam COLAPSADAS (removido auto-expand)
    ...
    def _add_category_group(self, name, docs):
        ...
        # Ordenação natural dos documentos (1, 2, 3... 10, 11, 12)
        import re
        def natural_sort_key(text):
            return [int(c) if c.isdigit() else c.lower() for c in re.split(r'(\d+)', text)]
        
        sorted_docs = sorted(docs, key=lambda d: natural_sort_key(d.get('name', '')))
