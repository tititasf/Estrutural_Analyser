"""Auditable learning store for N1 FV (Fundo de Viga) interpretation.

This store is intentionally separated from project_data.vision. It records
human and engine events for N1 beam-fundo interpretation without enabling any
...
DEFAULT_ENGREV_FV_N1_INTERPRETACAO_LEARNING_DB_PATH = Path(
    "D:/Agente-cad-PYSIDE/engrev_fv_n1_interpretacao_learning.vision"
)

EVENTS_TABLE = "engrev_fv_n1_interpretacao_events"
FEATURES_TABLE = "engrev_fv_n1_interpretacao_features"
CALIBRATORS_TABLE = "engrev_fv_n1_interpretacao_calibrator_versions"

EVENT_TYPES = {
    "baseline_generated",
    "engrev_assisted_generated",
    "human_fundo_outline_validated",
    "human_fundo_outline_rejected",
...
DEFAULT_ENGINE_VERSION = "structural_analyzer:fv-n1-interpretação-v1"
...
def ensure_engrev_fv_n1_interpretacao_learning_schema(
    db_path: str | Path = DEFAULT_ENGREV_FV_N1_INTERPRETACAO_LEARNING_DB_PATH,
) -> None:
    ...
    try:
        conn.executescript(
            ...
            CREATE TABLE IF NOT EXISTS {EVENTS_TABLE} (
                ...
                obra_name TEXT,
                pavimento TEXT,
                classe TEXT NOT NULL DEFAULT 'FV',
            ...
            CREATE INDEX IF NOT EXISTS idx_fv_n1_events_item
                ON {EVENTS_TABLE}(obra_name, pavimento, classe, elemento_id, event_type);
            CREATE INDEX IF NOT EXISTS idx_fv_n1_events_mode
                ON {EVENTS_TABLE}(analysis_mode, created_at);
            CREATE INDEX IF NOT EXISTS idx_fv_n1_calibrators_status
...
def record_engrev_fv_n1_interpretacao_event(
    *,
    event_type: str,
    elemento_id: str,
    ...
    learning_db_path: str | Path = DEFAULT_ENGREV_FV_N1_INTERPRETACAO_LEARNING_DB_PATH,
) -> int:
    if event_type not in EVENT_TYPES:
        raise ValueError(f"event_type invalido: {event_type}")
    if not elemento_id:
        raise ValueError("elemento_id e obrigatorio")
    if analysis_mode not in {"baseline", "engrev_assisted", "cascade"}:
        raise ValueError(f"analysis_mode invalido: {analysis_mode}")

    ensure_engrev_fv_n1_interpretacao_learning_schema(learning_db_path)
        ...
        conn.execute(
            f"""INSERT INTO {FEATURES_TABLE}
                (event_id, obra_name, pavimento, classe, elemento_id,
                 panels_n1, panels_n2, comprimento_n1, comprimento_n2,
                 h_n1, h_n2, segs_matched, score, features_json)
                VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
            (
                event_id,
                obra_name,
                ...
                "FV",
                elemento_id,
                features.get("panels_n1"),
                features.get("panels_n2"),
                features.get("comprimento_n1"),
                features.get("comprimento_n2"),
                features.get("h_n1"),
                features.get("h_n2"),
                features.get("segs_matched"),
                features.get("score"),
...
def record_human_fundo_outline_validated(
    *,
    elemento_id: str,
    obra_name: str | None = None,
    pavimento: str | None = None,
    ...
    return record_engrev_fv_n1_interpretacao_event(
        event_type="human_fundo_outline_validated",
...
def record_human_fundo_outline_rejected(
    *,
    elemento_id: str,
    obra_name: str | None = None,
    pavimento: str | None = None,
    ...
    return record_engrev_fv_n1_interpretacao_event(
        event_type="human_fundo_outline_rejected",
        elemento_id=elemento_id,
        analysis_mode="engrev_assisted",
        obra_name=obra_name,
        pavimento=pavimento,
