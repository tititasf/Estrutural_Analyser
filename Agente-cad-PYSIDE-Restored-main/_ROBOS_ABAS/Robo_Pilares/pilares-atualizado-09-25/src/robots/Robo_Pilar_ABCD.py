...
class AplicacaoUnificada:
    def __init__(self):
        try:
            ...
            self.gerador = self.gerador_pilares # Alias para compatibilidade com interface antiga
...
class GeradorPilares:
    ...
    def configurar_estilos(self):
        ...
        self.cores_paineis = {
            ...
            # Adicionar suporte para chaves lowercase (retrocompatibilidade)
            'a': {'bg': '#C8E6C9', 'fg': '#1B5E20', 'border': '#2E7D32'},
            'b': {'bg': '#BBDEFB', 'fg': '#0D47A1', 'border': '#1565C0'},
            'c': {'bg': '#FFE0B2', 'fg': '#E65100', 'border': '#F57C00'},
            'd': {'bg': '#E1BEE7', 'fg': '#4A148C', 'border': '#7B1FA2'}
        }
    ...
    def criar_painel_colorido(self, frame_pai, tipo_painel, coluna, num_alturas, cor_fundo):
        ...
        titulo = tk.Label(frame_painel, 
                         ...
                         bg=self.cores_paineis[tipo_painel.upper()]['bg'],
                         fg=self.cores_paineis[tipo_painel.upper()]['fg'],
                         font=('Arial', 10, 'bold'))
    ...
    def criar_frame_abertura(self, frame_pai, tipo_painel, row):
        ...
        for i, (nome, frame_id) in enumerate(zip(frames_nomes, frames_ids)):
            ...
            ttk.Checkbutton(frame_checks, text="Do Topo2", 
                           variable=self.abertura_topo2[tipo_painel.upper()][frame_id],
            ...
            laje_entry = ttk.Entry(frame_checks, width=3, textvariable=self.abertura_laje[tipo_painel.upper()][frame_id])
            ...
            self.campos[f'abertura_{frame_id}_pos_{tipo_painel.upper()}'] = pos
            self.campos[f'abertura_{frame_id}_dist_{tipo_painel.upper()}'] = dist
            self.campos[f'abertura_{frame_id}_prof_{tipo_painel.upper()}'] = prof
            self.campos[f'abertura_{frame_id}_larg_{tipo_painel.upper()}'] = larg
...
"""
        ...
        laje_value = self.abertura_laje[tipo_painel.upper()][abertura_id].get().strip()
        ...
        if hasattr(self, 'abertura_laje'):
            print(f"  🔍 Painel existe? {tipo_painel.upper() in self.abertura_laje}")
            if tipo_painel.upper() in self.abertura_laje:
                print(f"  🔍 Abertura existe? {abertura_id in self.abertura_laje[tipo_painel.upper()]}")
                if abertura_id in self.abertura_laje[tipo_painel.upper()]:
                    campo = self.abertura_laje[tipo_painel.upper()][abertura_id]
...
"""
            ...
            if salvar_arquivo:
                ...
                from utils.robust_path_resolver import robust_path_resolver
                # Usar SCRIPTS_ROBOS em vez de output/scripts para compatibilidade com automation_service
...
;"""
            ...
            for tipo_painel, x_inicial, y_inicial in [
                ...
                try:
                    ...
                    # Obter opções de hatch para este painel (uppercase para match com atributo)
                    hatch_opcoes = getattr(self, f'hatch_opcoes_{tipo_painel.upper()}', [])
...
;"""
                    ...
                    if modo_sarrafos == "Pline":
                        ...
                        if i > 0:  # Não desenha no painel de 2cm
                            # Desenhar sarrafos superior e inferior do painel
                            script += f""";
_ZOOM
C
...
;"""
                            ...
                            # Desenhar sarrafos intermediários baseados na configuração
                            # Calcular Y da base do pilar
                            y_base_pilar = y_inicial - pe_direito
                            
                            # Obter posições configuradas
                            horizontal_positions = self.config_manager.get_config("horizontal_positions", "positions") or []
                            
                            # Calcular limites relativos deste painel
                            relative_base_y = y_atual - y_base_pilar
                            relative_top_y = relative_base_y + altura

                            # Lógica para mover o último sarrafo para 292.2 se altura >= 300 (baseado em ROBO_GRADES)
                            posicao_y_topo = 292.2
                            ajustar_topo = pe_direito >= 300
                            altura_limite = pe_direito - 10
                            
                            # Identificar o último sarrafo válido globalmente
                            last_valid_pos = -1
                            if ajustar_topo:
                            ...
                            for pos in horizontal_positions:
                                ...
                                # Aplicar ajuste se for o último sarrafo válido e altura >= 300
                                if ajustar_topo and pos == last_valid_pos:
                                    current_pos = posicao_y_topo

                                # Verificar se a posição (ajustada ou não) cai dentro deste painel (com margem de segurança)
                                # Adicionamos margem para não sobrepor com os sarrafos de topo/base
                                margem = sarrafo_offset + 2 # 2cm de folga
                                if (relative_base_y + margem) <= current_pos <= (relative_top_y - margem):
                                    y_sarrafo = y_base_pilar + current_pos
        ...
        try:
            ...
            if is_script2_especial:
                ...
                if altura <= 0:
                    ...
                    print(f"[VALIDATION-ERROR] A altura deve ser maior que zero!")
                    return False
                ...
                if comprimento <= 0 or largura <= 0 or altura <= 0:
                    print(f"[VALIDATION-ERROR] As dimensões devem ser maiores que zero! C={comprimento}, L={largura}, A={altura}")
                    return False
                
        except ValueError:
            print(f"[VALIDATION-ERROR] As dimensões devem ser valores numéricos!")
            return False
...
;"""
            ...
            try:
                # Alteração: Aceitar PD igual a 0,00 para evitar "INDEFINIDO"
                if pd_texto != "ERRO":
                    pd_float = self.converter_para_float(pd_texto) if pd_texto != "0,00" else 0.0
