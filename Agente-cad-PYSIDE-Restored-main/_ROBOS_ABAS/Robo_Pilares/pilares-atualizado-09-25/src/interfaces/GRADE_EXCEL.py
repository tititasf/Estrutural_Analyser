...
"""
    ...
    def gerar_scripts_scr(self, caminho_arquivo_excel):
        ...
        try:
            ...
            # Check for Direct Mode / Dummy Path first
            path_check = caminho_arquivo_excel.strip() if caminho_arquivo_excel else ""
            print(f"[DEBUG GRADES] Verificando caminho: '{path_check}'")
            if "DUMMY_PATH_DIRECT_MODE" in path_check or not os.path.exists(path_check):
                 print(f"[DEBUG GRADES] Modo Direto detectado (sem Excel). Verificando dados internos...")
                 if self.dados_grades:
                      # Try to extract from first item
                      first_val = list(self.dados_grades.values())[0]
                      pilar_especial_ativo = bool(first_val.get('pilar_especial_ativo', False))
                 print(f"[DEBUG GRADES] ✅ Pilar especial ativo (interno): {pilar_especial_ativo}")
            else:
                try:
                    import openpyxl
                    wb = openpyxl.load_workbook(caminho_arquivo_excel, data_only=True)
    ...
    def gerar_scripts_especiais(self, caminho_arquivo_excel):
        ...
        try:
                ...
                try:
                    ...
                    try:
                        ...
                        try:
                            ...
                            diretorio_base = os.path.join(project_root, "SCRIPTS_ROBOS")
                    ...
                    if script_gerado and not script_gerado.endswith('\r\n'):
                        script_gerado = script_gerado.rstrip('\n').rstrip('\r') + '\r\n'
                    ...
                    print(f"[ERRO] Falha ao gerar script para grade especial {letra.upper()}: {e}")
                    import traceback
                    traceback.print_exc()
            ...
            print(f"Erro ao gerar scripts especiais: {e}")
            import traceback
            traceback.print_exc()
            return False
...
def preencher_campos_diretamente_e_gerar_scripts(dados_pilar):
    """
    Função de compatibilidade para gerar scripts diretamente dos dados.
    """
    print("Iniciando preencher_campos_diretamente_e_gerar_scripts para GRADES")
    try:
        # Instanciar automação
        robo = GradeAutomation()
        
        nome = dados_pilar.get('nome', 'Pilar')
        pavimento = dados_pilar.get('pavimento', 'Pavimento')
        
        # CORREÇÃO CRÍTICA: Detectar se é arquivo .A ou .B pelo nome
        nome_arquivo = dados_pilar.get('nome', 'Pilar')
        is_arquivo_b = nome_arquivo.endswith('.B') or nome_arquivo.endswith('.b')
        painel_letra = 'B' if is_arquivo_b else 'A'
        print(f"[GRADES] Arquivo detectado: {nome_arquivo} -> Painel {painel_letra}")
        
        # Tentar extrair grades reais do grupo 1
        grades_ativas = []
        gr1 = dados_pilar.get('grades_grupo1', {})
        
        # Mapear distâncias entre grades
        distancias_reais = {}
        if 'distancia_1' in gr1: distancias_reais['1_2'] = float(gr1['distancia_1'] or 0)
        if 'distancia_2' in gr1: distancias_reais['2_3'] = float(gr1['distancia_2'] or 0)

        # CORREÇÃO CRÍTICA: Extrair alturas do painel (h1_A, h2_A, etc. ou h1_B, h2_B, etc.)
        alturas_painel = []
        for j in range(1, 6):  # h1 até h5
            campo_h = f'h{j}_{painel_letra}'
            altura_valor = dados_pilar.get(campo_h, 0)
            try:
                altura_float = float(altura_valor) if altura_valor else 0.0
                alturas_painel.append(altura_float)
                print(f"  [GRADES] {campo_h} = {altura_float}")
            except (ValueError, TypeError):
                alturas_painel.append(0.0)
        
        # CORREÇÃO CRÍTICA: Extrair larguras do painel (larg1_A, larg2_A, larg3_A ou larg1_B, etc.)
        larguras_painel = []
        for j in range(1, 4):  # larg1, larg2, larg3
            campo_larg = f'larg{j}_{painel_letra}'
            largura_valor = dados_pilar.get(campo_larg, 0)
            try:
                largura_float = float(largura_valor) if largura_valor else 0.0
                larguras_painel.append(largura_float)
                print(f"  [GRADES] {campo_larg} = {largura_float}")
            except (ValueError, TypeError):
                larguras_painel.append(0.0)
        
        # Calcular altura total do painel (soma de h1 a h5)
        altura_total_painel = sum(alturas_painel)
        print(f"  [GRADES] Altura total do painel {painel_letra}: {altura_total_painel}cm (h1={alturas_painel[0]}, h2={alturas_painel[1]}, h3={alturas_painel[2]}, h4={alturas_painel[3]}, h5={alturas_painel[4]})")
        
        # Calcular largura total do painel (soma de larg1, larg2, larg3)
        largura_total_painel = sum(larguras_painel)
        print(f"  [GRADES] Largura total do painel {painel_letra}: {largura_total_painel}cm")

        for i in range(1, 4):
            larg = float(gr1.get(f'grade_{i}', 0) or 0)
            if larg > 0:
                # CORREÇÃO CRÍTICA: Preencher alturas_detalhes com valores do painel
                # IMPORTANTE: As alturas dos sarrafos verticais devem ser a ALTURA TOTAL (soma de h1+h2+h3+h4+h5)
                # alturas_detalhes[0] = altura da extremidade esquerda (altura total)
                # alturas_detalhes[1] = altura do sarrafo central 1 (altura total)
                # alturas_detalhes[2] = altura do sarrafo central 2 (altura total)
                # alturas_detalhes[3] = altura do sarrafo central 3 (altura total)
                # alturas_detalhes[4] = altura do sarrafo central 4 (altura total)
                # alturas_detalhes[5] = altura da extremidade direita (altura total)
                
                alturas_detalhes_grade = []
                # CORREÇÃO: Usar altura_total_painel para TODOS os sarrafos verticais (como no legacy)
                # O legacy usa altura total (282.2 = 2+122+122+54 = 300cm - 17.8cm de ajuste)
                # Mas vamos usar a altura total real do painel
                if altura_total_painel > 0:
                    # Todos os sarrafos verticais têm a mesma altura (altura total do painel)
                    alturas_detalhes_grade = [altura_total_painel] * 6
                    print(f"  [GRADES] Grade {i}: usando altura_total_painel={altura_total_painel}cm para todos os sarrafos verticais")
                else:
                    # Fallback: usar altura do pilar se não houver alturas do painel
                    altura_pilar = float(dados_pilar.get('altura', 0) or 0)
                    if altura_pilar > 0:
                        alturas_detalhes_grade = [altura_pilar] * 6
                        print(f"  [GRADES] Grade {i}: usando altura_pilar={altura_pilar}cm como fallback")
                    else:
                        alturas_detalhes_grade = [0] * 6
                        print(f"  [GRADES] Grade {i}: nenhuma altura encontrada, usando zeros")
                
                # CORREÇÃO CRÍTICA: Preencher larguras_detalhes com valores do painel
                # larguras_detalhes[0] = distância da ponta esquerda até centro do sarrafo 1 (larg1/2)
                # larguras_detalhes[1] = distância do centro do sarrafo 1 até centro do sarrafo 2 (larg2)
                # larguras_detalhes[2] = distância do centro do sarrafo 2 até centro do sarrafo 3 (larg3)
                # larguras_detalhes[3] = distância do centro do sarrafo 3 até centro do sarrafo 4 (0 se não houver)
                # larguras_detalhes[4] = distância do último sarrafo até ponta direita (resto)
                
                larguras_detalhes_grade = []
                if len(larguras_painel) >= 3:
                    # Campo 0: distância até primeiro sarrafo (metade de larg1)
                    larguras_detalhes_grade.append(larguras_painel[0] / 2.0 if larguras_painel[0] > 0 else 0)
                    # Campo 1: distância entre sarrafo 1 e 2 (larg2)
                    larguras_detalhes_grade.append(larguras_painel[1] if larguras_painel[1] > 0 else 0)
                    # Campo 2: distância entre sarrafo 2 e 3 (larg3)
                    larguras_detalhes_grade.append(larguras_painel[2] if larguras_painel[2] > 0 else 0)
                    # Campo 3: distância entre sarrafo 3 e 4 (0 se não houver)
                    larguras_detalhes_grade.append(0.0)
                    # Campo 4: distância do último sarrafo até ponta direita (metade de larg1 + resto)
                    resto = largura_total_painel - (larguras_painel[0]/2.0 + larguras_painel[1] + larguras_painel[2])
                    larguras_detalhes_grade.append(resto if resto > 0 else larguras_painel[0] / 2.0)
                else:
                    # Fallback: distribuir largura igualmente
                    largura_por_campo = largura_total_painel / 5.0 if largura_total_painel > 0 else 0
                    larguras_detalhes_grade = [largura_por_campo] * 5
                
                print(f"  [GRADES] Grade {i}: alturas_detalhes={alturas_detalhes_grade}, larguras_detalhes={larguras_detalhes_grade}")
                
                grades_ativas.append({
                    'largura': larg,
                    'esquerda': alturas_detalhes_grade[0] if len(alturas_detalhes_grade) > 0 else 0,  # Campo 0
                    'direita': alturas_detalhes_grade[-1] if len(alturas_detalhes_grade) > 0 else 0,  # Último campo
                    'alturas': alturas_detalhes_grade[1:4] if len(alturas_detalhes_grade) > 4 else [0, 0, 0],  # Campos 1-3
                    'larguras_detalhes': larguras_detalhes_grade[:5] if len(larguras_detalhes_grade) >= 5 else [0]*5,
                    'alturas_detalhes': alturas_detalhes_grade[:6] if len(alturas_detalhes_grade) >= 6 else [0]*6,
                    'esquerdo_central': False, 'direito_central': False,
                    'distancias': [0, 0, 0]
                })
        
        if not grades_ativas:
            # Fallback para largura do pilar se não houver grades
            try:
                 l = float(dados_pilar.get('largura', 20))
            except: l = 20.0
            grades_ativas = [{
                'largura': l,
                'esquerda': 0, 'direita': 0,
                'alturas': [0, 0, 0],
                'larguras_detalhes': [0]*5,
                'alturas_detalhes': [0]*6,
                'esquerdo_central': False, 'direito_central': False,
                'distancias': [0, 0, 0]
            }]
        
        # CORREÇÃO CRÍTICA: Definir x_inicial baseado no legacy (4000.0 para A, 4302.0 para B)
        x_inicial_base = 4000.0  # Legacy usa 4000.0 para A
        x_inicial_custom = None
        if is_arquivo_b:
            # Legacy B usa 4302.0, deslocamento de 302.0
            x_inicial_custom = 4302.0
            print(f"[GRADES] Arquivo .B detectado, usando x_inicial={x_inicial_custom} (legacy)")
        else:
            x_inicial_custom = x_inicial_base
            print(f"[GRADES] Arquivo .A detectado, usando x_inicial={x_inicial_custom} (legacy)")
        
        script_gerado = robo_module.gerar_script_grade(
            pavimento=pavimento,
            nome=nome,
            grades_ativas=grades_ativas,
            altura_base=float(dados_pilar.get('altura', 0) or 0),
            x_inicial_custom=x_inicial_custom,  # CORREÇÃO: Usar x_inicial legacy
            y_inicial_custom=0.0,  # Legacy usa 0.0
            distancias=distancias_reais
        )
        ...
        try:
            ...
            diretorio_base = os.path.join(os.getcwd(), "SCRIPTS_ROBOS")
            
        diretorio_pavimento = os.path.join(diretorio_base, f"{pavimento}_GRADES")
        os.makedirs(diretorio_pavimento, exist_ok=True)
        
        caminho = os.path.join(diretorio_pavimento, f"{nome}.scr")
        print(f"[DEBUG_GRADES_SAVE] Salvando script GRADES com nome: '{nome}'")
        print(f"[DEBUG_GRADES_SAVE] Caminho completo: {caminho}")
        ...
        print(f"[DEBUG_GRADES_SAVE] Script GRADE salvo com sucesso em: {caminho}")
        return script_gerado
    except Exception as e:
        print(f"Erro GRADES: {e}")
        traceback.print_exc()
        return None
