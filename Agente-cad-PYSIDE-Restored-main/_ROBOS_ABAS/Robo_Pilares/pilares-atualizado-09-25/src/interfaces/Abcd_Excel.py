...
# Importando AplicacaoUnificada de forma robusta
AplicacaoUnificada = None

# Primeiro, garantir que o diretório dos robôs está no path
...
import_paths = [
    lambda: __import__('Robo_Pilar_ABCD', fromlist=['AplicacaoUnificada']).AplicacaoUnificada,
    lambda: __import__('src.robots.Robo_Pilar_ABCD', fromlist=['AplicacaoUnificada']).AplicacaoUnificada,
    lambda: __import__('robots.Robo_Pilar_ABCD', fromlist=['AplicacaoUnificada']).AplicacaoUnificada,
]

for i, import_func in enumerate(import_paths):
    try:
        AplicacaoUnificada = import_func()
        print(f"✅ AplicacaoUnificada importada com sucesso usando método {i+1}")
        break
    except Exception as e:
        print(f"❌ Falha na importação {i+1}: {e}")
        continue

if AplicacaoUnificada is None:
    print("CRITICAL IMPORT ERROR: Could not import AplicacaoUnificada after all attempts")
    # Tenta importar o módulo completo para debug
    try:
        import Robo_Pilar_ABCD
        print(f"Módulo Robo_Pilar_ABCD importado, conteúdo: {dir(Robo_Pilar_ABCD)}")
        if hasattr(Robo_Pilar_ABCD, 'AplicacaoUnificada'):
            AplicacaoUnificada = Robo_Pilar_ABCD.AplicacaoUnificada
            print("✅ AplicacaoUnificada encontrada após importação direta do módulo")
        else:
            print("❌ AplicacaoUnificada não encontrada no módulo")
    except Exception as e2:
        print(f"❌ Não foi possível importar o módulo Robo_Pilar_ABCD: {e2}")
    ...
    try:
        ...
        try:
            ...
            if script_content:
                ...
                diretorio_base = os.path.join(robust_path_resolver.get_project_root(), "SCRIPTS_ROBOS")
...
def preencher_campos_e_gerar_scripts(caminho_arquivo_excel, coluna_especifica=None, interface_principal=None, gerar_pelo_pavimento=False):
    ...
    try:
        ...
        coluna_base = 'B'  # Coluna inicial para os dados dos pilares
        ...
        colunas_processadas_set = set()  # Conjunto para rastrear colunas já processadas
        
        # DEBUG: Print rows 3, 4, 5 for column E (or first column processed)
        first_col_idx = colunas_a_processar[0] if isinstance(colunas_a_processar, list) else list(colunas_a_processar)[0]
        # In openpyxl, columns are 1-based? No, sheet.cell uses 1-based indexing for both row and column. 
        # But 'colunas_a_processar' contains 0-based indices from 'letra_para_coluna'?
        # letra_para_coluna('A') -> 0. sheet.cell(column=1) is 'A'. So we need +1 for openpyxl.
        
        debug_col_1based = first_col_idx + 1
        print(f"[DEBUG-XLS] Checking Column {first_col_idx} (1-based: {debug_col_1based})")
        for r in range(3, 7):
             val = sheet.cell(row=r, column=debug_col_1based).value
             print(f"[DEBUG-XLS] Row {r}: '{val}'")

        coluna_anterior = None
        dados_pilares = {} # Dicionário para armazenar dados dos pilares por coluna
        for col_idx in colunas_a_processar:
            if parar_busca:
                break
            
            letra_coluna = coluna_para_letra(col_idx)
            coluna = letra_coluna
            # DEBUG: Check rows 1-10 for every column
            print(f"[DEBUG-XLS] Checking Col {letra_coluna} (idx {col_idx})")
            for r in range(1, 11):
                val = sheet.cell(row=r, column=col_idx + 1).value
                if val:
                    print(f"   Row {r}: '{val}'")

            # Verifica se o nome do pilar existe
            linha_nome = linhas_abcd['nome']
            nome_pilar = sheet.cell(row=linha_nome, column=col_idx + 1).value
            
            # FORCE FIX FOR DEBUG IF COLUMN E
            if letra_coluna == 'E' and not nome_pilar:
                print("[DEBUG-FIX] Forcing Nome='P2' for Column E")
                nome_pilar = "P2"
                ...
                if script_abcd is None:
                    ...
                    try:
                        ...
                        try:
                            ...
                            # Fallback manual
                            diretorio_base = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "SCRIPTS_ROBOS")
                    ...
                    with open(nome_arquivo_abcd, 'w') as f:
                        f.write(b'\xFF\xFE')
                    ...
                    if eh_ultima_coluna or total_colunas == 1:
                        ...
                        # Para múltiplas colunas, apenas fecha a janela mas mantém a aplicação
                        try:
                            if app and app.root:
                                app.root.withdraw()
...
def preencher_campos_diretamente_e_gerar_scripts(dados_pilar):
    """
    Preenche os campos da interface ABCD diretamente com os dados fornecidos
    e gera os scripts .scr correspondentes.
    """
    global AplicacaoUnificada
    import time
    import os
    print(f"[ABCD_SCRIPT] ===== INÍCIO DA FUNÇÃO preencher_campos_diretamente_e_gerar_scripts =====")
    print(f"[ABCD_SCRIPT] Dados recebidos: {dados_pilar}")

    try:
        # Verificar se AplicacaoUnificada está disponível
        if AplicacaoUnificada is None:
            print("[ABCD_SCRIPT] ❌ ERRO CRÍTICO: AplicacaoUnificada não está disponível")
            print("[ABCD_SCRIPT] Tentando importar novamente...")
            
            # Tentar reimportar em tempo real
            try:
                import importlib
                import sys
                # Remover do cache se existir
                if 'Robo_Pilar_ABCD' in sys.modules:
                    del sys.modules['Robo_Pilar_ABCD']
                
                # Tentar importar novamente
                modulo = importlib.import_module('Robo_Pilar_ABCD')
                AplicacaoUnificada = getattr(modulo, 'AplicacaoUnificada', None)
                globals()['AplicacaoUnificada'] = AplicacaoUnificada
                
                if AplicacaoUnificada is None:
                    raise ImportError("AplicacaoUnificada não encontrada no módulo")
                    
                print("[ABCD_SCRIPT] ✅ AplicacaoUnificada reimportada com sucesso")
            except Exception as reimport_error:
                print(f"[ABCD_SCRIPT] ❌ Falha na reimportação: {reimport_error}")
                return None

        # Criar a aplicação UMA ÚNICA VEZ
        print("[DEBUG-FUNC] Iniciando aplicação ABCD...")
        
        # Criar instância da aplicação com tratamento de erro
        try:
            app = AplicacaoUnificada()
            print("[DEBUG-FUNC] Aplicação ABCD criada com sucesso")
        except Exception as app_error:
            print(f"[DEBUG-FUNC] ❌ Erro ao criar aplicação: {app_error}")
            # Tentar criar sem interface gráfica
            try:
                print("[DEBUG-FUNC] Tentando criar aplicação sem interface gráfica...")
                import tkinter as tk
                root = tk.Tk()
                root.withdraw()  # Esconder janela principal
                app = AplicacaoUnificada()
                print("[DEBUG-FUNC] Aplicação criada com janela oculta")
            except Exception as fallback_error:
                print(f"[DEBUG-FUNC] ❌ Falha no fallback: {fallback_error}")
                return None
        
        # Fallback inspection for 'gerador' com debug detalhado
        print("[DEBUG-FUNC] Procurando gerador na aplicação...")
        print(f"[DEBUG-FUNC] Atributos disponíveis em app: {[attr for attr in dir(app) if not attr.startswith('_')]}")
        
        gerador = None
        
        # Procurar o gerador em diferentes locais possíveis
        if hasattr(app, 'gerador'):
            gerador = app.gerador
            print("[DEBUG-FUNC] ✅ Gerador encontrado em app.gerador")
        elif hasattr(app, 'gerador_pilares'):
             gerador = app.gerador_pilares
             print("[DEBUG-FUNC] ✅ Gerador encontrado em app.gerador_pilares")
        elif hasattr(app, 'interface_principal'):
             print("[DEBUG-FUNC] Verificando interface_principal...")
             gerador = getattr(app.interface_principal, 'gerador', None)
             if gerador:
                 print("[DEBUG-FUNC] ✅ Gerador encontrado em app.interface_principal.gerador")
             else:
                 print("[DEBUG-FUNC] ❌ Gerador não encontrado em interface_principal")
        else:
             print("[DEBUG-FUNC] ❌ Nenhum local padrão encontrado para o gerador")
             
        if not gerador:
            print("[ABCD_SCRIPT] CRITICAL: Não foi possível obter instância do gerador em AplicacaoUnificada.")
            print("[ABCD_SCRIPT] Tentando criar gerador manualmente...")
            
            # Tentar criar um gerador básico se possível
            try:
                # Verificar se existe uma classe GeradorPilares ou similar
                if hasattr(app, 'GeradorPilares'):
                    gerador = app.GeradorPilares()
                    print("[DEBUG-FUNC] ✅ Gerador criado com GeradorPilares()")
                elif hasattr(app, 'GeradorPilar'):
                    gerador = app.GeradorPilar()
                    print("[DEBUG-FUNC] ✅ Gerador criado com GeradorPilar()")
                else:
                    # Procurar no módulo completo
                    import Robo_Pilar_ABCD as robo_module
                    if hasattr(robo_module, 'GeradorPilares'):
                        gerador = robo_module.GeradorPilares()
                        print("[DEBUG-FUNC] ✅ Gerador criado com robo_module.GeradorPilares()")
                    elif hasattr(robo_module, 'GeradorPilar'):
                        gerador = robo_module.GeradorPilar()
                        print("[DEBUG-FUNC] ✅ Gerador criado com robo_module.GeradorPilar()")
            except Exception as gerador_error:
                print(f"[DEBUG-FUNC] ❌ Falha ao criar gerador manualmente: {gerador_error}")
                return None

        print(f"[DEBUG-FUNC] Gerador obtido: {gerador}")
        print(f"[DEBUG-FUNC] Tipo do gerador: {type(gerador)}")
        print(f"[DEBUG-FUNC] Atributos do gerador: {[attr for attr in dir(gerador) if not attr.startswith('_')]}")
        
        # Define helper for robust filling
        def preencher_campo_com_trigger(widget, valor):
            val_str = str(valor) if valor is not None else ""
            if hasattr(widget, 'delete'):
                widget.delete(0, 'end')
            if hasattr(widget, 'insert'):
                widget.insert(0, val_str)
            # Verificar se o valor foi realmente inserido
            if hasattr(widget, 'get'):
                valor_inserido = widget.get()
                if valor_inserido != val_str:
                    print(f"  [WARN] Valor não foi inserido corretamente! Esperado: '{val_str}', Obtido: '{valor_inserido}'")
                    # Tentar novamente
                    widget.delete(0, 'end')
                    widget.insert(0, val_str)
            # NÃO gerar eventos automáticos que podem limpar o campo
            # if hasattr(widget, 'event_generate'):
            #     try:
            #         widget.event_generate('<FocusOut>')
            #     except Exception as e:
            #         print(f"  [WARN] Failed to generate event for widget: {e}")

        # 1. Preencher dados básicos
        campos_basicos = ['pavimento', 'nome', 'comprimento', 'largura', 'altura', 'nivel_saida', 'nivel_chegada']
        for campo in campos_basicos:
            valor = dados_pilar.get(campo, "")
            # Converter valores numéricos para formato com vírgula (padrão brasileiro)
            if campo in ['comprimento', 'largura', 'altura', 'nivel_saida', 'nivel_chegada']:
                if isinstance(valor, (int, float)):
                    # Formatar com 2 casas decimais e vírgula
                    valor = f"{valor:.2f}".replace('.', ',')
                elif valor == "" or valor == 0 or valor == "0":
                    # Se estiver vazio ou zero, usar valor padrão para altura
                    if campo == 'altura':
                        valor = "300,00"  # Altura padrão
                    else:
                        valor = "0,00"
            # Check if gerador has 'campos'
            if hasattr(gerador, 'campos') and campo in gerador.campos:
                preencher_campo_com_trigger(gerador.campos[campo], valor)
                print(f"  Preenchendo {campo}: {valor} (tipo: {type(valor).__name__})")

        # 2. Preencher parafusos (P1 a P9)
        parafusos_ordem = [f'parafuso_p{i}_p{i+1}' for i in range(1, 9)]
        for i, parafuso in enumerate(parafusos_ordem):
            if i < len(gerador.parafuso_entries):
                valor = dados_pilar.get(parafuso, 0)
                preencher_campo_com_trigger(gerador.parafuso_entries[i], valor)

        # 3. Preencher Dados dos Painéis A, B, C, D
        mapeamento_paineis = {'a': 'a', 'b': 'b', 'c': 'c', 'd': 'd'}
        
        # Inicializar larguras_globais se não existir
        if not hasattr(gerador, 'larguras_globais'):
            gerador.larguras_globais = {}
        
        for painel_orig, painel_dest in mapeamento_paineis.items():
            # Comp1 e Comp2
            for j in range(1, 3):
                campo_comp = f"comp{j}_{painel_orig}"
                valor = dados_pilar.get(campo_comp, 0)
                if painel_dest in ['c', 'd']:
                    if f'comp{j}_{painel_dest}' in gerador.campos_cd:
                        preencher_campo_com_trigger(gerador.campos_cd[f'comp{j}_{painel_dest}'], valor)
                else:
                    if f'comp{j}_{painel_dest}' in gerador.campos_ab:
                        preencher_campo_com_trigger(gerador.campos_ab[f'comp{j}_{painel_dest}'], valor)
            
            # Alturas h1-h5
            for j in range(1, 6):
                campo_h = f"h{j}_{painel_orig}"
                valor = dados_pilar.get(campo_h, 0)
                alturas_lista = gerador.campos_altura.get(painel_dest, [])
                if (j-1) < len(alturas_lista):
                    preencher_campo_com_trigger(alturas_lista[j-1], valor)
            
            # CORREÇÃO CRÍTICA: Preencher larguras (larg1, larg2, larg3) para calcular largura total
            larg1 = dados_pilar.get(f'larg1_{painel_orig.upper()}', 0)
            larg2 = dados_pilar.get(f'larg2_{painel_orig.upper()}', 0)
            larg3 = dados_pilar.get(f'larg3_{painel_orig.upper()}', 0)
            
            # Calcular largura total (soma das 3 larguras)
            try:
                larg1_float = float(larg1) if larg1 else 0.0
                larg2_float = float(larg2) if larg2 else 0.0
                larg3_float = float(larg3) if larg3 else 0.0
                largura_total = larg1_float + larg2_float + larg3_float
                
                # Armazenar em larguras_globais (usado por calcular_largura_total_painel)
                gerador.larguras_globais[painel_dest.lower()] = largura_total
                print(f"  [LARGURAS] Painel {painel_dest.upper()}: larg1={larg1_float}, larg2={larg2_float}, larg3={larg3_float}, total={largura_total}")
            except (ValueError, TypeError) as e:
                print(f"  [ERRO] Erro ao calcular largura total para painel {painel_dest}: {e}")
                # Fallback: usar comprimento para A/B, largura para C/D
                if painel_dest in ['a', 'b']:
                    comprimento = dados_pilar.get('comprimento', 0)
                    try:
                        largura_total = float(comprimento) + 22 if comprimento else 122
                    except:
                        largura_total = 122
                else:
                    largura = dados_pilar.get('largura', 0)
                    try:
                        largura_total = float(largura) if largura else 20
                    except:
                        largura_total = 20
                gerador.larguras_globais[painel_dest.lower()] = largura_total
            
            # CORREÇÃO CRÍTICA: Preencher laje e posição da laje
            laje_valor = dados_pilar.get(f'laje_{painel_orig.upper()}', 0)
            pos_laje_valor = dados_pilar.get(f'posicao_laje_{painel_orig.upper()}', 0)
            
            # Preencher laje_A, laje_B, etc.
            if hasattr(gerador, f'laje_{painel_dest}_var'):
                laje_var = getattr(gerador, f'laje_{painel_dest}_var')
                if hasattr(laje_var, 'set'):
                    laje_var.set(str(laje_valor) if laje_valor else "0")
                    print(f"  [LAJE] Preenchendo laje_{painel_dest}: {laje_valor}")
            
            # Preencher pos_laje_A, pos_laje_B, etc.
            if hasattr(gerador, f'pos_laje_{painel_dest}'):
                pos_laje_var = getattr(gerador, f'pos_laje_{painel_dest}')
                if hasattr(pos_laje_var, 'set'):
                    try:
                        pos_int = int(float(pos_laje_valor)) if pos_laje_valor else 0
                        pos_laje_var.set(pos_int)
                        print(f"  [POS_LAJE] Preenchendo pos_laje_{painel_dest}: {pos_int}")
                    except (ValueError, TypeError):
                        pos_laje_var.set(0)
            
            # Preencher também campos['laje_A'] se existir
        ...
        # MOCK GUI ELEMENTS TO PREVENT IO ERRORS
        # MOCK GUI ELEMENTS TO PREVENT IO ERRORS
        if hasattr(gerador, 'log_text'):
             # Create a dummy object that accepts 'insert' and 'see'
            class DummyLog:
                def insert(self, index, text, *args): 
                    print(f"[GUI-LOG] {text.strip()}")
                def see(self, *args): pass
            gerador.log_text = DummyLog()

        # MOCK MESSAGEBOX
        import tkinter.messagebox
        def mock_showerror(title, message):
            print(f"[MOCK-MESSAGEBOX] Error: {title} - {message}")
            return "ok"
        
        # Patch system-wide
        tkinter.messagebox.showerror = mock_showerror
        
        # Patch specifically in the module
        import src.robots.Robo_Pilar_ABCD
        src.robots.Robo_Pilar_ABCD.messagebox.showerror = mock_showerror

        # Verificar campos ANTES de gerar script
        print(f"[DEBUG-FUNC] Verificando campos antes de gerar script...")
        if hasattr(gerador, 'campos'):
            for campo in ['nome', 'comprimento', 'largura', 'altura']:
                if campo in gerador.campos:
                    valor = gerador.campos[campo].get() if hasattr(gerador.campos[campo], 'get') else 'N/A'
                    print(f"[DEBUG-FUNC] Campo {campo} ANTES de gerar: '{valor}'")
                    # Se altura está vazia ou zero, corrigir
                    if campo == 'altura' and (not valor or valor == '0' or valor == '0,00' or valor == '0.00'):
                        print(f"[DEBUG-FUNC] CORRIGINDO altura de '{valor}' para '300,00'")
                        gerador.campos[campo].delete(0, 'end')
                        gerador.campos[campo].insert(0, '300,00')
                    # Se comprimento está vazio, corrigir
                    if campo == 'comprimento' and (not valor or valor == '0' or valor == '0,00' or valor == '0.00'):
                        print(f"[DEBUG-FUNC] CORRIGINDO comprimento de '{valor}' para '20,00'")
                        gerador.campos[campo].delete(0, 'end')
                        gerador.campos[campo].insert(0, '20,00')
                        print(f"[DEBUG-FUNC] Campo {campo} APÓS correção: '{gerador.campos[campo].get()}'")
                    # Se largura está vazia, corrigir
                    if campo == 'largura' and (not valor or valor == '0' or valor == '0,00' or valor == '0.00'):
                        print(f"[DEBUG-FUNC] CORRIGINDO largura de '{valor}' para '20,00'")
                        gerador.campos[campo].delete(0, 'end')
                        gerador.campos[campo].insert(0, '20,00')
                        print(f"[DEBUG-FUNC] Campo {campo} APÓS correção: '{gerador.campos[campo].get()}'")
        ...
        # In Robo_Pilar_ABCD.py, 'gerar_script' returns the content and saves the file if salvar_arquivo=True
        print(f"[DEBUG-FUNC] Chamando gerar_script()...")
        script_abcd = gerador.gerar_script(salvar_arquivo=True)
        print(f"[DEBUG-FUNC] Script gerado! Tamanho: {len(script_abcd) if script_abcd else 0} caracteres")
        if not script_abcd or len(script_abcd) == 0:
            print(f"[DEBUG-FUNC] ERRO: Script retornou vazio!")
            # Verificar validação
            if hasattr(gerador, 'validar_entrada'):
                validacao_ok = gerador.validar_entrada()
                print(f"[DEBUG-FUNC] Validação: {validacao_ok}")
                if not validacao_ok:
                    print(f"[DEBUG-FUNC] Validação falhou! Verificando campos novamente...")
                    if hasattr(gerador, 'campos'):
                        for campo in ['nome', 'comprimento', 'largura', 'altura']:
                            if campo in gerador.campos:
                                valor = gerador.campos[campo].get() if hasattr(gerador.campos[campo], 'get') else 'N/A'
                                print(f"[DEBUG-FUNC] Campo {campo}: '{valor}'")
