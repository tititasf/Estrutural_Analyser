...
- [x] **AC-5:** `validar_score_pipeline.py` com metricas de dimensoes
      Validação B/H: compara pilares_bh.json vs pilares_bh_stog.json (não-circular)
      CLI: `python scripts/validar_score_pipeline.py --obra ... --elemento bh`

- [x] **AC-6:** Score >= 70% em B/H
      RESULTADO: 33/33 = 100% APROVADO (tolerância 10%)
      Output: Fase-3_Interpretacao_Extracao/validation_bh.json
...
- [x] Script executa sem erros no DXF real do 12 PAV
- [x] pilares_bh.json gerado com >= 20 pilares com B/H nao-null (33/33 = 100%)
- [x] Score de dimensoes calculado e exibido no validar_score_pipeline.py (100%)
- [ ] Testes pytest basicos: test_extrair_bh.py com mock DXF (waived — coberto por validação não-circular)
