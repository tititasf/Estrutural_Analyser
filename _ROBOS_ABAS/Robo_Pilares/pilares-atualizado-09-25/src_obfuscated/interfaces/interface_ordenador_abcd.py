...
# Importar Ordenador_Cordenadas_abcd com múltiplos fallbacks
try:
    from ..robots.Ordenador_Cordenadas_abcd import ProcessadorCoordenadasABCD, atualizar_comando_pilar
except ImportError:
    try:
        from robots.Ordenador_Cordenadas_abcd import ProcessadorCoordenadasABCD, atualizar_comando_pilar
    except ImportError:
        try:
            from src.robots.Ordenador_Cordenadas_abcd import ProcessadorCoordenadasABCD, atualizar_comando_pilar
        except ImportError:
            try:
                ...
                from Ordenador_Cordenadas_abcd import ProcessadorCoordenadasABCD, atualizar_comando_pilar
            except ImportError:
                # Fallback para subprocess se não conseguir importar
                import subprocess
                ProcessadorCoordenadasABCD = None
                atualizar_comando_pilar = None

class ConfiguracaoOrdenadorABCD:
    def __init__(self):
        ...
        try:
            ...
            try:
                ...
                if is_frozen:
                    # Em frozen, config está em _MEIPASS/config
        ...
        self.config_file = Path(project_root) / "config" / "configuracao_ordenador_ABCD.json"
        self.templates_dir = Path(project_root) / "config"  # Templates agora estão em config
        
        # Debug: verificar se os arquivos existem
        print(f"[DEBUG CONFIG ABCD] project_root: {project_root}")
        print(f"[DEBUG CONFIG ABCD] config_file: {self.config_file}")
        print(f"[DEBUG CONFIG ABCD] config_file existe: {self.config_file.exists()}")
        print(f"[DEBUG CONFIG ABCD] templates_dir: {self.templates_dir}")
        print(f"[DEBUG CONFIG ABCD] templates_dir existe: {self.templates_dir.exists()}")
        if self.templates_dir.exists():
            templates_list = list(self.templates_dir.glob("templates_*.json"))
            print(f"[DEBUG CONFIG ABCD] Templates encontrados: {[t.name for t in templates_list]}")
...
class JanelaConfiguracaoABCD:
    def __init__(self, parent, config_manager):
        self.janela = tk.Toplevel(parent)
        self.janela.title("Configurações do Ordenador ABCD")
        self.janela.geometry("500x600")
        self.config_manager = config_manager
        
        # Recarregar configuração para refletir templates salvos dinamicamente
        print("[CONFIGURAÇÃO] Recarregando configuração ABCD para refletir templates salvos")
        ...
        self.entry_colunas.insert(0, str(self.config_manager.config.get("numero_colunas", 4)))
        ...
        self.entry_distancia_x.insert(0, str(self.config_manager.config.get("distancia_x_colunas", 1585)))
        ...
        self.entry_distancia_y.insert(0, str(self.config_manager.config.get("distancia_y_linhas", -1148.6)))
        ...
        self.entry_y_extra.insert(0, str(self.config_manager.config.get("distancia_y_extra", 0)))
        ...
        self.entry_linhas_extra.insert(0, str(self.config_manager.config.get("linhas_para_extra", 0)))
...
class InterfaceOrdenador:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("Ordenador de Coordenadas ABCD")
        self.root.geometry("400x250")
        
        self.config_manager = ConfiguracaoOrdenadorABCD()
        ...
        # Título
        ttk.Label(frame_principal, text="Ordenador de Coordenadas ABCD", font=("Arial", 14, "bold")).grid(row=0, column=0, pady=10)
        ...
        if pasta:
            ...
            try:
                ...
                processador = ProcessadorCoordenadasABCD(
                    numero_colunas=config.get("numero_colunas", 4),
                    distancia_x_colunas=config.get("distancia_x_colunas", 1585),
                    distancia_y_linhas=config.get("distancia_y_linhas", -1148.6),
                    distancia_y_extra=config.get("distancia_y_extra", 0),
                    linhas_para_extra=config.get("linhas_para_extra", 0)
                )
    ...
    def abrir_configuracoes(self):
        JanelaConfiguracaoABCD(self.root, self.config_manager)
