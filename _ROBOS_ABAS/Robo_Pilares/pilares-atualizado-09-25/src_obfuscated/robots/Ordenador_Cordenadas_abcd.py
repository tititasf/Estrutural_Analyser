...
def get_template_path_abcd():
    """Obtém o path do template de forma robusta"""
    try:
        from config_paths import TEMPLATES_DIR
        return Path(TEMPLATES_DIR) / "templates_configuracao_ordenador_ABCD.json"
    except ImportError:
        # Fallback relativo
        current_dir = Path(__file__).parent
        project_root = current_dir.parent.parent
        return project_root / "templates" / "templates_configuracao_ordenador_ABCD.json"

def get_config_path_abcd():
    """Obtém o path da configuração de forma robusta"""
    try:
        from config_paths import CONFIG_DIR
        return Path(CONFIG_DIR) / "configuracao_ordenador_ABCD.json"
    except ImportError:
        # Fallback relativo
        current_dir = Path(__file__).parent
        project_root = current_dir.parent.parent
        return project_root / "config" / "configuracao_ordenador_ABCD.json"

def carregar_template_abcd():
    """Carrega o template específico para visão ABCD"""
    caminho_template = get_template_path_abcd()
    
    if caminho_template.exists():
        with open(caminho_template, 'r', encoding='utf-8') as f:
            return json.load(f)
    else:
        print(f"ERRO: Template para visão ABCD não encontrado em: {caminho_template}")
        return None

def carregar_configuracao_abcd():
    """Carrega a configuração específica para visão ABCD"""
    caminho_config = get_config_path_abcd()
    
    if caminho_config.exists():
        with open(caminho_config, 'r', encoding='utf-8') as f:
            return json.load(f)
    else:
        print(f"ERRO: Configuração para visão ABCD não encontrada em: {caminho_config}")
        return None

class ProcessadorCoordenadasABCD:
    def __init__(self, numero_colunas=4, distancia_x_colunas=1585, distancia_y_linhas=-1148.6, distancia_y_extra=0, linhas_para_extra=0):
        """
        Inicializa o processador com as 4 configurações principais:
        
        Args:
            numero_colunas (int): Quantidade de colunas no layout
            distancia_x_colunas (float): Distância X entre colunas
            distancia_y_linhas (float): Distância Y entre linhas
            distancia_y_extra (float): Distância Y extra a ser aplicada
            linhas_para_extra (int): A cada quantas linhas aplicar o Y extra
        """
    ...
    def processar_arquivo(self, caminho_arquivo):
        ...
        while tentativas < 100:
            try:
                ...
                # Primeiro, identifica todas as linhas de texto que devem ser protegidas
                linhas_protegidas = set()
                for i, linha in enumerate(linhas):
                    if "_TEXT" in linha.upper():
                        # Conta quantas linhas abaixo está o valor do texto
                        # Padrão: _TEXT -> J -> L -> coordenada -> 0 -> valor_texto -> ;
                        for j in range(i+1, min(i+6, len(linhas))):
                            if re.match(r'^\d+,\d+$', linhas[j].strip()):
                                linhas_protegidas.add(j)
                                break
                ...
                for i, linha in enumerate(linhas):
                    ...
                    if any(padrao in linha for padrao in ['PD:', 'NÍVEL DE CHEGADA']):
                        nova_linha = linha
                    # Protege linhas identificadas como valores de texto
                    elif i in linhas_protegidas:
                        nova_linha = linha  # Não processa linhas de valor de texto
                    else:
                        nova_linha = re.sub(
                            r'(-?\d+\.?\d*),(-?\d+\.?\d*)',
...
def atualizar_comando_pilar(pasta_selecionada):
    """Atualiza o caminho no arquivo comando_pilar_combinado_TPATPA.scr (específico para ABCD)"""
    caminho_comando_pilar = Path(os.path.join(robust_path_resolver.get_project_root(), "output", "comando_pilar_combinado_tpatpa.scr"))
    ...
    if caminho_comando_pilar.exists():
        ...
        with open(caminho_comando_pilar, 'w', encoding='utf-16') as f:
            f.writelines(linhas)
        print(f"\nABCD - Arquivo comando_pilar_combinado_TPATPA.scr atualizado com sucesso para a pasta: {pasta_selecionada}")
    else:
        print(f"ABCD - Arquivo comando_pilar_combinado_TPATPA.scr não encontrado no caminho: {caminho_comando_pilar}")

if __name__ == "__main__":
    # Exemplo de uso do novo sistema simplificado
    print("EXEMPLO DE USO DO SISTEMA SIMPLIFICADO")
    print("="*55)
    
    # Cria processador com configurações padrão
    ...
    print("\nExemplo com configurações personalizadas:")
    processador2 = ProcessadorCoordenadasABCD(
        numero_colunas=3,
