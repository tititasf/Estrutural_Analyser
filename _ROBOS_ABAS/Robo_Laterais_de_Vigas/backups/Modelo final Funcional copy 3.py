...
def calcular_paineis_largura(*args):
    try:
        ...
        # Calcular valores baseado na largura disponível
        if largura_disponivel <= 244:
            if posicoes_livres:
                paineis_largura_entries[posicoes_livres[0]].insert(0, str(int(largura_disponivel)))
                
        elif 245 <= largura_disponivel <= 304:
        ...
        elif 305 <= largura_disponivel <= 425:
            if len(posicoes_livres) >= 1:
        ...
        elif 426 <= largura_disponivel <= 487:
            if len(posicoes_livres) >= 1:
                paineis_largura_entries[posicoes_livres[0]].insert(0, "244")
            if len(posicoes_livres) >= 2:
        ...
        elif 488 <= largura_disponivel <= 547:
            if len(posicoes_livres) >= 1:
                paineis_largura_entries[posicoes_livres[0]].insert(0, "244")
            if len(posicoes_livres) >= 2:
                paineis_largura_entries[posicoes_livres[1]].insert(0, "122")
        ...
        elif 548 <= largura_disponivel <= 609:
            if len(posicoes_livres) >= 1:
                paineis_largura_entries[posicoes_livres[0]].insert(0, "244")
            if len(posicoes_livres) >= 2:
        ...
        elif 610 <= largura_disponivel <= 731:
            if len(posicoes_livres) >= 1:
                paineis_largura_entries[posicoes_livres[0]].insert(0, "244")
            if len(posicoes_livres) >= 2:
                paineis_largura_entries[posicoes_livres[1]].insert(0, "244")
        ...
        elif 732 <= largura_disponivel <= 791:
            if len(posicoes_livres) >= 1:
                paineis_largura_entries[posicoes_livres[0]].insert(0, "244")
            if len(posicoes_livres) >= 2:
                paineis_largura_entries[posicoes_livres[1]].insert(0, "244")
        ...
        elif 792 <= largura_disponivel <= 913:
            if len(posicoes_livres) >= 1:
                paineis_largura_entries[posicoes_livres[0]].insert(0, "244")
            if len(posicoes_livres) >= 2:
                paineis_largura_entries[posicoes_livres[1]].insert(0, "244")
        ...
        elif 914 <= largura_disponivel <= 974:
            if len(posicoes_livres) >= 1:
                paineis_largura_entries[posicoes_livres[0]].insert(0, "244")
            if len(posicoes_livres) >= 2:
                paineis_largura_entries[posicoes_livres[1]].insert(0, "244")
        ...
        elif 975 <= largura_disponivel <= 1097:
            valores = [244, 244, 244, 122]
            for i in range(min(len(posicoes_livres), len(valores))):
                paineis_largura_entries[posicoes_livres[i]].insert(0, str(valores[i]))
            if len(posicoes_livres) > len(valores):
                paineis_largura_entries[posicoes_livres[-1]].insert(0, str(int(largura_disponivel - sum(valores))))
...
def desenhar_sarrafo_horizontal_direita(script, x_inicial, y_inicial, largura_painel):
    """Desenha um sarrafo horizontal de 2.2 de altura dentro de um painel, terminando a 15cm a esquerda da parede direita."""
    altura_sarrafo = 2.2
    script.append("_PLINE")
    script.append(f"{x_inicial},{y_inicial}")
    script.append(f"{x_inicial},{y_inicial - altura_sarrafo}")
    ...
    script.append("_PLINE")
    script.append(f"{x_inicial},{y_inicial - altura_sarrafo}")
    script.append(f"{x_inicial + largura_painel - 15},{y_inicial - altura_sarrafo}")
    script.append("")
    script.append(";")
    
    script.append("_PLINE")
    script.append(f"{x_inicial + largura_painel - 15},{y_inicial - altura_sarrafo}")
    script.append(f"{x_inicial + largura_painel - 15},{y_inicial}")
    script.append("")
    script.append(";")
...
def desenhar_sarrafo_horizontal_esquerda(script, x_inicial, y_inicial, largura_painel):
    """Desenha um sarrafo horizontal de 2.2 de altura dentro de um painel, começando a 15cm a direita da parede esquerda."""
    altura_sarrafo = 2.2
    script.append("_PLINE")
    script.append(f"{x_inicial + 15},{y_inicial}")
    script.append(f"{x_inicial + 15},{y_inicial - altura_sarrafo}")
    script.append("")
    script.append(";")
...
def desenhar_sarrafo_horizontal_centralizado(script, x_inicial, y_inicial, largura_painel):
    """Desenha um sarrafo horizontal de 2.2 de altura dentro de um painel, começando a 15cm da parede esquerda e terminando a 15cm da parede direita."""
    altura_sarrafo = 2.2
    script.append("_PLINE")
    script.append(f"{x_inicial + 15},{y_inicial}")
    script.append(f"{x_inicial + 15},{y_inicial - altura_sarrafo}")
    ...
    try:
        ...
        for j, altura in enumerate(alturas_paineis):
            ...
            for i, largura in enumerate(larguras_paineis):
                if len(larguras_paineis) == 1:
                    # Somente 1 painel de largura
                    desenhar_sarrafo_horizontal_centralizado(script, x_atual, y_paineis[j] + altura, largura)
                elif i == 0:
                    # Primeiro painel (extrema esquerda)
                    desenhar_sarrafo_horizontal_esquerda(script, x_atual, y_paineis[j] + altura, largura)
                elif i == len(larguras_paineis) - 1:
                    # Último painel (extrema direita)
                    desenhar_sarrafo_horizontal_direita(script, x_atual, y_paineis[j] + altura, largura)
                else:
        ...
        if len(larguras_paineis) > 0:
            ...
            # Criar nova lista com valores ajustados subtraindo 2.2
            grades_valores_ajustados = [max(0, valor - 2.2) for valor in grades_valores]
            
            if len(larguras_paineis) > 0 and grades_valores_ajustados:  # Só desenha se tiver painéis e grades
                # 1. Sarrafo retangular no primeiro painel (15cm da esquerda)
                if len(grades_valores_ajustados) > 0:  # Verifica se tem pelo menos uma grade
                    x_grade = x_inicial + 15
                    script.append("_PLINE")
                    script.append(f"{x_grade},{y_topo-2.2}")
                    script.append(f"{x_grade+3.5},{y_topo-2.2}")
                    script.append(f"{x_grade+3.5},{y_topo-2.2-grades_valores_ajustados[0]}")
                    script.append(f"{x_grade},{y_topo-2.2-grades_valores_ajustados[0]}")
                    script.append("C")
                    script.append(";")

                # 2. Sarrafo retangular no último painel (15cm da direita)
                if grades_valores_ajustados:  # Usa a última grade disponível
                    x_grade = x_inicial + largura_total - 15 - 3.5
                    script.append("_PLINE")
                    script.append(f"{x_grade},{y_topo-2.2}")
                    script.append(f"{x_grade+3.5},{y_topo-2.2}")
                    script.append(f"{x_grade+3.5},{y_topo-2.2-grades_valores_ajustados[-1]}")
                    script.append(f"{x_grade},{y_topo-2.2-grades_valores_ajustados[-1]}")
                    script.append("C")
                    script.append(";")

                # 3. Sarrafos retangulares nas uniões entre painéis
                for i in range(len(larguras_paineis)-1):
                    if i+1 < len(grades_valores_ajustados):  # Só desenha se tiver grade definida para esta posição
                        ...
                        script.append(f"{x_grade},{y_topo-2.2}")
                        script.append(f"{x_grade+3.5},{y_topo-2.2}")
                        script.append(f"{x_grade+3.5},{y_topo-2.2-grades_valores_ajustados[i+1]}")
                        script.append(f"{x_grade},{y_topo-2.2-grades_valores_ajustados[i+1]}")
                        script.append("C")
                        script.append(";")
        ...
        distancia_pilar_2 = distancia_pilar_2_entry.get().strip()
        recorte_pilar_2 = recorte_pilar_2_entry.get().strip()
        
        # Valores das aberturas de viga
        abertura_viga_largura = abertura_viga_largura_entry.get().strip()
        abertura_viga_altura = abertura_viga_altura_entry.get().strip()
        abertura_viga_2_largura = abertura_viga_2_largura_entry.get().strip()
        abertura_viga_2_altura = abertura_viga_2_altura_entry.get().strip()
        
        # 1. PRIMEIRO PILAR
        if distancia_pilar and recorte_pilar:
            try:
                distancia_pilar_valor = float(distancia_pilar)
                recorte_pilar_valor = float(recorte_pilar)

                if distancia_pilar_valor >= 0 and recorte_pilar_valor > 0:
                    x_retangulo_1 = x_inicial + distancia_pilar_valor
                    y_retangulo_1 = y_paineis[-1] + alturas_paineis[-1]
                    x_retangulo_2 = x_retangulo_1 + recorte_pilar_valor
                    y_retangulo_2 = y_paineis[0]
                    
                    # Desenho do primeiro pilar
                    script.append("-LAYER")
                    script.append("S 0")
                    script.append("")
                    script.append(";")
                    ...
                    script.append("_ZOOM")
                    script.append("W")
                    script.append(f"{x_retangulo_1-10},{y_retangulo_1+10}")
                    script.append(f"{x_retangulo_2+10},{y_retangulo_2-10}")
                    script.append(";")
                    script.append("appv")
                    ...
                    # Cota do primeiro pilar
                    script.append("-LAYER")
                    script.append("S COTA")
                    script.append("")
                    script.append(";")
                    
                    y_cota_fundo = y_paineis[0]
                    script.append("_DIMLINEAR")
                    script.append(f"{x_retangulo_1},{y_cota_fundo}")
                    script.append(f"{x_retangulo_1 + recorte_pilar_valor},{y_cota_fundo}")
                    script.append(f"{x_retangulo_1 + recorte_pilar_valor/2},{y_cota_fundo + 5}")
                    script.append(";")
            except ValueError:
                pass

        # 2. SEGUNDO PILAR
        if distancia_pilar_2 and recorte_pilar_2:
            try:
                distancia_pilar_2_valor = float(distancia_pilar_2)
                recorte_pilar_2_valor = float(recorte_pilar_2)

                if distancia_pilar_2_valor >= 0 and recorte_pilar_2_valor > 0:
                    x_retangulo_1_2 = x_inicial + distancia_pilar_2_valor
                    y_retangulo_1_2 = y_paineis[-1] + alturas_paineis[-1]
                    x_retangulo_2_2 = x_retangulo_1_2 + recorte_pilar_2_valor
                    y_retangulo_2_2 = y_paineis[0]
                    
                    # Desenho do segundo pilar
                    script.append("-LAYER")
                    script.append("S 0")
                    script.append("")
                    script.append(";")
                    ...
                    script.append("_ZOOM")
                    script.append("W")
                    script.append(f"{x_retangulo_1_2-10},{y_retangulo_1_2+10}")
                    script.append(f"{x_retangulo_2_2+10},{y_retangulo_2_2-10}")
                    script.append(";")
                    script.append("appv")
                    script.append(f"{x_retangulo_1_2},{y_retangulo_1_2}")
                    script.append(f"{x_retangulo_2_2},{y_retangulo_2_2}")
                    script.append(";")
                    ...
                    # Cota do segundo pilar
                    script.append("-LAYER")
                    script.append("S COTA")
                    script.append("")
                    script.append(";")
                    
                    y_cota_fundo_2 = y_paineis[0]
                    script.append("_DIMLINEAR")
                    script.append(f"{x_retangulo_1_2},{y_cota_fundo_2}")
                    script.append(f"{x_retangulo_1_2 + recorte_pilar_2_valor},{y_cota_fundo_2}")
                    script.append(f"{x_retangulo_1_2 + recorte_pilar_2_valor/2},{y_cota_fundo_2 + 5}")
                    script.append(";")
            except ValueError:
                pass

        # 3. ABERTURAS DE VIGA
        if (abertura_viga_largura and abertura_viga_altura and parede_uniao_selecionada.get()):
            try:
                # Preparar dados para primeira abertura
                abertura_viga_largura_valor = float(abertura_viga_largura)
                abertura_viga_altura_valor = float(abertura_viga_altura)
                parede_uniao_selecionada_valor = int(parede_uniao_selecionada.get())
                
                # Calcular coordenadas primeira abertura
                if parede_uniao_selecionada_valor == 0:
                    x_retangulo = x_inicial
                else:
                    x_retangulo = x_inicial + sum(larguras_paineis[:parede_uniao_selecionada_valor])
                y_retangulo = y_paineis[-1] + alturas_paineis[-1]

                # Ajustes das dimensões
                abertura_viga_largura_valor += 15.5
                abertura_viga_altura_valor += 2.2
                abertura_viga_altura_valor -= altura_laje_valor

                # Se houver segunda abertura
                if (abertura_viga_2_largura and abertura_viga_2_altura and parede_uniao_2_selecionada.get()):
                    # Preparar dados para segunda abertura
                    abertura_viga_2_largura_valor = float(abertura_viga_2_largura)
                    abertura_viga_2_altura_valor = float(abertura_viga_2_altura)
                    parede_uniao_2_selecionada_valor = int(parede_uniao_2_selecionada.get())
                    
                    # Calcular coordenadas segunda abertura
                    if parede_uniao_2_selecionada_valor == 0:
                        x_retangulo_2 = x_inicial
                    else:
                        x_retangulo_2 = x_inicial + sum(larguras_paineis[:parede_uniao_2_selecionada_valor])
                    y_retangulo_2 = y_paineis[-1] + alturas_paineis[-1]

                    # Ajustes das dimensões
                    abertura_viga_2_largura_valor += 15.5
                    abertura_viga_2_altura_valor += 2.2
                    abertura_viga_2_altura_valor -= altura_laje_valor

                    # Primeiro desenhar as cotas
                    script.append("-LAYER")
                    script.append("S COTA")
                    script.append("")
                    script.append(";")

                    # Cotas da primeira abertura
                    script.append("_DIMLINEAR")
                    script.append(f"{x_retangulo + abertura_viga_largura_valor - 15.5},{y_retangulo}")
                    script.append(f"{x_retangulo + abertura_viga_largura_valor - 15.5},{y_retangulo - abertura_viga_altura_valor + 2.2}")
                    script.append(f"{x_retangulo + abertura_viga_largura_valor - 20},{y_retangulo - abertura_viga_altura_valor/2 + 1.1}")
                    script.append("")
                    script.append(";")

                    script.append("_DIMLINEAR")
                    script.append(f"{x_retangulo},{y_retangulo - abertura_viga_altura_valor}")
                    script.append(f"{x_retangulo + abertura_viga_largura_valor - 15.5},{y_retangulo - abertura_viga_altura_valor}")
                    script.append(f"{x_retangulo + (abertura_viga_largura_valor - 15.5)/2},{y_retangulo - abertura_viga_altura_valor - 20}")
                    script.append("")
                    script.append(";")

                    # Cotas da segunda abertura
                    script.append("_DIMLINEAR")
                    script.append(f"{x_retangulo_2 + abertura_viga_2_largura_valor - 15.5},{y_retangulo_2}")
                    script.append(f"{x_retangulo_2 + abertura_viga_2_largura_valor - 15.5},{y_retangulo_2 - abertura_viga_2_altura_valor + 2.2}")
                    script.append(f"{x_retangulo_2 + abertura_viga_2_largura_valor - 20},{y_retangulo_2 - abertura_viga_2_altura_valor/2 + 1.1}")
                    script.append("")
                    script.append(";")

                    script.append("_DIMLINEAR")
                    script.append(f"{x_retangulo_2},{y_retangulo_2 - abertura_viga_2_altura_valor}")
                    script.append(f"{x_retangulo_2 + abertura_viga_2_largura_valor - 15.5},{y_retangulo_2 - abertura_viga_2_altura_valor}")
                    script.append(f"{x_retangulo_2 + (abertura_viga_2_largura_valor - 15.5)/2},{y_retangulo_2 - abertura_viga_2_altura_valor - 20}")
                    script.append(";")

                    # Depois desenhar as aberturas
                    script.append("-LAYER")
                    script.append("S 0")
                    script.append("")
                    script.append(";")
                    
                    script.append("avvsl2")
                    script.append(f"{x_retangulo},{y_retangulo}")
                    script.append(f"{x_retangulo + abertura_viga_largura_valor},{y_retangulo - abertura_viga_altura_valor}")
                    script.append(f"{x_retangulo_2},{y_retangulo_2}")
                    script.append(f"{x_retangulo_2 + abertura_viga_2_largura_valor},{y_retangulo_2 - abertura_viga_2_altura_valor}")
                    script.append(";")
                else:
                    # Primeiro desenhar as cotas
                    script.append("-LAYER")
...
# Frame principal
main_frame = ttk.Frame(root, padding="5")  # Reduzido o padding de 10 para 5
main_frame.grid(row=0, column=0, sticky="nsew")

# Frame para campos à esquerda
left_frame = ttk.Frame(main_frame)
left_frame.grid(row=0, column=0, sticky="nsew", padx=(0, 5))  # Reduzido o padx de 10 para 5
...
nome_entry = ttk.Entry(left_frame, width=30)  # Reduzido o width de 40 para 30
...
observacoes_entry = ttk.Entry(left_frame, width=30)  # Reduzido o width de 40 para 30
observacoes_entry.insert(0, "observacoes")
observacoes_entry.grid(row=1, column=1, columnspan=3, sticky="we")

# Frame para laje
laje_frame = ttk.LabelFrame(left_frame, text="Laje", padding="5")  # Reduzido o padding de 10 para 5
laje_frame.grid(row=2, column=0, columnspan=4, sticky="nsew", pady=5)  # Reduzido o pady de 10 para 5
...
dimensoes_frame = ttk.LabelFrame(left_frame, text="Dimensões", padding="5")  # Reduzido o padding de 10 para 5
dimensoes_frame.grid(row=3, column=0, columnspan=4, sticky="nsew", pady=5)  # Reduzido o pady de 10 para 5

# Largura total
ttk.Label(dimensoes_frame, text="Largura Total:").grid(row=0, column=0, sticky="w")
largura_total_entry = ttk.Entry(dimensoes_frame, width=8)  # Reduzido o width de 10 para 8
largura_total_entry.insert(0, "600")
largura_total_entry.grid(row=0, column=1, sticky="w")
largura_total_entry.bind('<KeyRelease>', calcular_paineis_largura)

# Altura total
ttk.Label(dimensoes_frame, text="Altura Total:").grid(row=0, column=2, sticky="w", padx=(10,0))  # Reduzido o padx de 20 para 10
altura_total_entry = ttk.Entry(dimensoes_frame, width=8)  # Reduzido o width de 10 para 8
altura_total_entry.insert(0, "180")
altura_total_entry.grid(row=0, column=3, sticky="w")
altura_total_entry.bind('<KeyRelease>', calcular_paineis_altura)

# Frame para painéis
paineis_frame = ttk.LabelFrame(left_frame, text="Painéis", padding="5")  # Reduzido o padding de 10 para 5
paineis_frame.grid(row=4, column=0, columnspan=4, sticky="nsew", pady=5)  # Reduzido o pady de 10 para 5
...
# Frame para grades
grades_frame = ttk.LabelFrame(left_frame, text="Grades", padding="5")  # Reduzido o padding de 10 para 5
grades_frame.grid(row=5, column=0, columnspan=4, sticky="nsew", pady=5)  # Reduzido o pady de 10 para 5
...
textos_frame = ttk.LabelFrame(left_frame, text="Textos Laterais", padding="5")  # Reduzido o padding de 10 para 5
textos_frame.grid(row=6, column=0, columnspan=4, sticky="nsew", pady=5)  # Reduzido o pady de 10 para 5
...
# Frame para pilar
pilar_frame = ttk.LabelFrame(right_frame, text="Pilar", padding="5")  # Reduzido o padding de 10 para 5
pilar_frame.grid(row=0, column=0, columnspan=2, sticky="nsew", pady=5)  # Reduzido o pady de 10 para 5
...
# Campos para a segunda abertura de pilar
ttk.Label(pilar_frame, text="Distância do Pilar 2 Esquerda:").grid(row=1, column=0, sticky="w")
distancia_pilar_2_entry = ttk.Entry(pilar_frame, width=10)
distancia_pilar_2_entry.grid(row=1, column=1, sticky="w")

ttk.Label(pilar_frame, text="Recorte Pilar 2 largura:").grid(row=1, column=2, sticky="w", padx=(20,0))
recorte_pilar_2_entry = ttk.Entry(pilar_frame, width=10)
recorte_pilar_2_entry.grid(row=1, column=3, sticky="w")

# Frame para abertura da viga
abertura_frame = ttk.LabelFrame(right_frame, text="Abertura da Viga", padding="5")  # Reduzido o padding de 10 para 5
abertura_frame.grid(row=1, column=0, columnspan=2, sticky="nsew", pady=5)  # Reduzido o pady de 10 para 5
...
# Frame para abertura da viga 2
abertura_2_frame = ttk.LabelFrame(right_frame, text="Abertura da Viga 2", padding="5")  # Reduzido o padding de 10 para 5
abertura_2_frame.grid(row=2, column=0, columnspan=2, sticky="nsew", pady=5)  # Reduzido o pady de 10 para 5

ttk.Label(abertura_2_frame, text="Largura:").grid(row=0, column=0, sticky="w")
abertura_viga_2_largura_entry = ttk.Entry(abertura_2_frame, width=10)
abertura_viga_2_largura_entry.grid(row=0, column=1, sticky="w")

ttk.Label(abertura_2_frame, text="Altura:").grid(row=0, column=2, sticky="w", padx=(20,0))
abertura_viga_2_altura_entry = ttk.Entry(abertura_2_frame, width=10)
abertura_viga_2_altura_entry.grid(row=0, column=3, sticky="w")

ttk.Label(abertura_2_frame, text="Selecione a Parede/União:").grid(row=1, column=0, sticky="w")
parede_uniao_2_frame = ttk.Frame(abertura_2_frame)
parede_uniao_2_frame.grid(row=1, column=1, columnspan=3, sticky="w")

parede_uniao_2_vars = []
parede_uniao_2_labels = [
    "Parede Esquerda Painel 1",
    "União Painel 1 e 2",
    "União Painel 2 e 3",
    "União Painel 3 e 4",
...
parede_uniao_2_selecionada = tk.StringVar()

for i in range(5):
    rb = ttk.Radiobutton(parede_uniao_2_frame, text=parede_uniao_2_labels[i], variable=parede_uniao_2_selecionada, value=str(i))
    rb.grid(row=i, column=0, sticky="w")
    parede_uniao_2_vars.append(rb)

# Frame para opções dos painéis
opcoes_frame = ttk.LabelFrame(right_frame, text="Opções dos Painéis", padding="5")  # Reduzido o padding de 10 para 5
opcoes_frame.grid(row=3, column=0, columnspan=2, sticky="nsew", pady=5)  # Reduzido o pady de 10 para 5
...
ttk.Button(main_frame, text="Salvar Script", command=salvar_script).grid(row=1, column=0, pady=5)  # Reduzido o pady de 10 para 5
ttk.Button(main_frame, text="Salvar Teste", command=salvar_teste).grid(row=1, column=1, pady=5)  # Reduzido o pady de 10 para 5

# Botões Conectar AutoCAD e Executar Comandos
ttk.Button(main_frame, text="Conectar AutoCAD", command=conectar_autocad).grid(row=2, column=0, pady=5)  # Reduzido o pady de 10 para 5
ttk.Button(main_frame, text="Executar Comandos", command=executar_comandos).grid(row=2, column=1, pady=5)  # Reduzido o pady de 10 para 5

# Frame para log
log_frame = ttk.LabelFrame(main_frame, text="Log", padding="5")  # Reduzido o padding de 10 para 5
log_frame.grid(row=3, column=0, columnspan=2, sticky="nsew", pady=5)  # Reduzido o pady de 10 para 5
