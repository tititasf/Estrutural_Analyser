...
def criar_script():
    ...
    try:
        ...
        # Desenhar sarrafos horizontais em cada painel
        x_atual = x_inicial
        for i, largura in enumerate(larguras_paineis):
            if i == 0:
                # Primeiro painel, começa 15 cm à direita
                desenhar_sarrafo_horizontal(script, x_atual + 15, y_paineis[0] + alturas_paineis[0], largura - 15)
            elif i == len(larguras_paineis) - 1:
                # Último painel, termina 15 cm à esquerda
                desenhar_sarrafo_horizontal(script, x_atual, y_paineis[0] + alturas_paineis[0], largura - 15)
            else:
                # Painéis intermediários
                desenhar_sarrafo_horizontal(script, x_atual, y_paineis[0] + alturas_paineis[0], largura)
            x_atual += largura
