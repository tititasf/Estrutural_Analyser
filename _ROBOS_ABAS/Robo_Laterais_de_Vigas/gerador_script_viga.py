...
class GeradorScriptViga:
    ...
    def gerar_script(self, dados, diretorio_saida=""):
        ...
        try:
            ...
            # Cabeçalho com numeracao, largura, altura, continuacao (exigido pelo Ordenador_VIGA)
            largura = dados.get('largura', 0)
            try:
                largura = float(largura)
            except (TypeError, ValueError):
                largura = 0.0
            alt_raw = dados.get('altura_geral', 0)
            if isinstance(alt_raw, (int, float)):
                altura = float(alt_raw)
            else:
                s = str(alt_raw)
                altura = sum(float(p.strip()) for p in s.split('+') if p.strip()) if '+' in s else (float(s) if s else 0.0)
            continuacao = str(dados.get('continuacao', '')).strip() or 'Não informado'
            header = f"; (numeracao: {numero or nome}, largura: {largura:.2f}, altura: {altura:.2f}, continuacao: {continuacao})\n"
            script_text = header + script_text
    ...
    def _gerar_conteudo_script(self, dados):
        ...
        try:
            ...
            # O texto deve ficar na base do painel 1; +80cm em Y (subir)
            DESLOCAMENTO_Y_TEXTOS_LATERAIS_CM = 80
            y_texto_lateral = y_inicial - altura1_painel1 + DESLOCAMENTO_Y_TEXTOS_LATERAIS_CM
...
;
"""
            # Definir estilo de texto (standard) antes dos comandos TEXT
            script += ";\n-style\nstandard\n\n0\n\n\n\n\n\n;\n"
...
"""
            ...
            # Em modo "Viga continua após obstáculo" ou "Viga continua após abertura de viga", não desenhar cotas verticais à direita.
            continuacao_norm = str(dados.get('continuacao', '')).replace(' ', '').lower()
            eh_modo_continuacao = continuacao_norm in ('obstaculo', 'vigacontinuacao')
            if not eh_modo_continuacao:
