...
class FundoProducaoApp(tk.Tk):
    def __init__(self):
        ...
        self.preview_combinacao = PreviewCombinacao(self) # Instancia a classe de preview
        self.gerador_combinados = GeradorScriptCombinados() # Instancia o gerador de scripts combinados

        # Trace adds, etc.
        self.nivel_oposto_var.trace_add('write', self._calcular_ajuste)
        self.nivel_viga_var.trace_add('write', self._calcular_ajuste)
        
        # Estes deveriam ser métodos da classe, não definidos dentro de __init__
        # def _gerar_teste_combinados():
        # def _gerar_script_um_combinado():
        # def _gerar_todos_scripts_combinados():

        # Calls to setup UI and load data after all initializations and method definitions
        self._setup_ui()
...
class FundoProducaoApp(tk.Tk):
    ...
    def _criar_lista_combinacoes(self, parent):
        ...
        # Novos botões em um novo frame
        botoes_novos = ttk.Frame(self.comb_frame)  # Novo frame para os novos botões
        botoes_novos.pack(fill=tk.X, pady=(2, 2))  # Empacota o novo frame abaixo do anterior
        
        btn_teste_combinados = ttk.Button(botoes_novos, text="Teste Combinados", command=self._gerar_teste_combinados, width=14)
        btn_teste_combinados.pack(side=tk.LEFT, padx=2)
        btn_gerar_1 = ttk.Button(botoes_novos, text="Gerar 1", command=self._gerar_script_um_combinado, width=6)
        btn_gerar_1.pack(side=tk.LEFT, padx=0)
        btn_gerar_todos = ttk.Button(botoes_novos, text="Gerar Todos", command=self._gerar_todos_scripts_combinados, width=10)
        btn_gerar_todos.pack(side=tk.LEFT, padx=0)
    ...
    def _on_select_combinacao(self, event=None):
        ...
        self.preview_combinacao.exibir_preview_combinacao(comb) # Passa o objeto comb completo
        self.modo_combinacao = True
    ...
    def _criar_aba_dados_gerais(self, parent):
        ...
        ttk.Label(info_frame, text="Fundo:").grid(row=5, column=0, sticky="w", padx=5)
        fundo_entry = ttk.Entry(info_frame, textvariable=self.fundo_var)
        fundo_entry.grid(row=5, column=0, sticky="e", padx=(70,5))
        frame_continuacao = ttk.Frame(info_frame)
        frame_continuacao.grid(row=6, column=0, columnspan=2, sticky="w", pady=2)
        ...
        area_util_label.grid(row=7, column=0, columnspan=2, sticky="w", padx=5, pady=(2, 8))
    ...
    def _gerar_todos_scripts(self):
        ...
        if not confirmed['ok'] or not pavimento_selecionado or pavimento_selecionado not in pavimentos:
            self._mostrar_comentario_flutuante("Operação cancelada ou pavimento inválido.", mensagem_erro=True)
            return
        ...
        if not erros:
            self._mostrar_comentario_flutuante(f"Todos os scripts de combinações ({total_gerados}) do pavimento '{pavimento_selecionado}' foram gerados com sucesso!")
        else:
            self._mostrar_comentario_flutuante(f"{total_gerados} scripts de combinações gerados. Erros:\n" + '\n'.join(erros), mensagem_erro=True)
        ...
        try:
            ...
            self._mostrar_comentario_flutuante(f"Combinador_VIGA.py executado com sucesso em: {diretorio_saida_combinados}")
        except Exception as e:
            self._mostrar_comentario_flutuante(f"Erro ao executar o Combinador_VIGA.py: {e}", mensagem_erro=True)
            return
        ...
        try:
            ...
            self._mostrar_comentario_flutuante(f"Ordenador_VIGA.py executado com sucesso em: {diretorio_saida_combinados}")
        except Exception as e:
            self._mostrar_comentario_flutuante(f"Erro ao executar o Ordenador_VIGA.py: {e}", mensagem_erro=True)
            return
    ...
    def _gerar_teste_combinados(self):
        """Chama a função para gerar o script de teste para combinados."""
        caminho_base = os.path.dirname(os.path.abspath(__file__))
        sucesso, mensagem = gerar_script_teste_combinados(caminho_base)
        if sucesso:
            messagebox.showinfo("Script Gerado", mensagem)
        else:
            messagebox.showerror("Erro ao Gerar Script", f"Erro: {mensagem}")
