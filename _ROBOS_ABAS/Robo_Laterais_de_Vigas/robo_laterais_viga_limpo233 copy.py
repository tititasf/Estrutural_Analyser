...
class FundoProducaoApp(tk.Tk):
    ...
    def _criar_botoes(self, parent):
        ...
        ttk.Button(botoes_frame, text="Gerar Script", command=self._gerar_script).pack(side=tk.LEFT, padx=5)
        ttk.Button(botoes_frame, text="Gerar Todos Scripts", command=self._gerar_todos_scripts).pack(side=tk.LEFT, padx=5)
        ttk.Button(botoes_frame, text="Salvar Teste", command=self._salvar_teste).pack(side=tk.LEFT, padx=5)
        ...
        ttk.Button(botoes_frame, text="Combinar Scripts", command=self._combinar_scripts).pack(side=tk.LEFT, padx=5)
        ttk.Button(botoes_frame, text="Ordenar Scripts", command=self._ordenar_scripts).pack(side=tk.LEFT, padx=5)
    ...
    def _criar_lista_fundos(self, parent):
        ...
        self.lista_fundos1.heading("Número", text="Nº")
        self.lista_fundos1.heading("Nome", text="Nome")
        self.lista_fundos1.heading("Pavimento", text="Pav")
        ...
        self.lista_fundos2.heading("Número", text="Nº")
        self.lista_fundos2.heading("Nome", text="Nome")
        self.lista_fundos2.heading("Pavimento", text="Pav")
    ...
    def _atualizar_lista(self, qual=None):
        ...
        for numero, dados in sorted(self.fundos_salvos.items()):
            ...
            if qual == 1 and self.modo_reciclagem:
                tree.insert('', 'end', iid=numero, values=(sel, rec, numero, dados.get('nome', ''), pav), tags=(color_tag,))
            elif qual == 1:
                tree.insert('', 'end', iid=numero, values=(sel, '', numero, dados.get('nome', ''), pav), tags=(color_tag,))
            elif qual == 2:
                tree.insert('', 'end', iid=numero, values=(sel, numero, dados.get('nome', ''), pav), tags=(color_tag,))
            i += 1
        if self.modo_combinacao:
    ...
    def _carregar_fundo_selecionado(self, event=None):
        ...
        for i in range(2):
            try:
                self.detalhe_pilar_esq_vars[i].set(float(detalhe_pilar_esq[i] or 0))
            except Exception:
                self.detalhe_pilar_esq_vars[i].set(0)
            try:
                self.detalhe_pilar_dir_vars[i].set(float(detalhe_pilar_dir[i] or 0))
            except Exception:
                self.detalhe_pilar_dir_vars[i].set(0)
        self.laje_sup_universal.set(0)
        self.laje_inf_universal.set(0)
        ...
        tipos1 = dados.get('paineis_tipo1', ["Sarrafeado"]*6)
        tipos2 = dados.get('paineis_tipo2', ["Sarrafeado"]*6)
        grades1 = dados.get('paineis_grade_altura1', [7.0]*6)
        grades2 = dados.get('paineis_grade_altura2', [7.0]*6)
        for i in range(6):
            try:
                self.paineis_tipo1_vars[i].set(tipos1[i])
            except Exception:
                self.paineis_tipo1_vars[i].set("Sarrafeado")
            try:
                self.paineis_tipo2_vars[i].set(tipos2[i])
            except Exception:
                self.paineis_tipo2_vars[i].set("Sarrafeado")
            try:
                self.paineis_grade_altura1_vars[i].set(float(grades1[i]))
            except Exception:
                self.paineis_grade_altura1_vars[i].set(7.0)
            try:
                self.paineis_grade_altura2_vars[i].set(float(grades2[i]))
            except Exception:
                self.paineis_grade_altura2_vars[i].set(7.0)
        self.continuacao_var.set(dados.get('continuacao', 'Proxima Parte'))
        self.laje_sup_universal.set(dados.get('laje_sup_universal', 0))
        self.laje_inf_universal.set(dados.get('laje_inf_universal', 0))
        self.laje_central_alt_universal.set(dados.get('laje_central_alt_universal', 0))
        if 'altura_2_geral_var' in dados:
            try:
                self.altura_2_geral_var.set(str(dados['altura_2_geral_var']))
            except Exception:
                self.altura_2_geral_var.set('0')
        forcar_altura1 = dados.get('forcar_altura1', [False]*4)
        for i in range(4):
            try:
                self.forcar_altura1_vars[i].set(bool(forcar_altura1[i]))
            except Exception:
                self.forcar_altura1_vars[i].set(False)
        self.sarrafo_alt2_esq_var.set(bool(dados.get('sarrafo_alt2_esq', False)))
        self.sarrafo_alt2_dir_var.set(bool(dados.get('sarrafo_alt2_dir', False)))
        self.tipo1_universal_var.set(dados.get('tipo1_universal_var', 'Sarrafeado'))
        self.tipo2_universal_var.set(dados.get('tipo2_universal_var', 'Sarrafeado'))
        self.nivel_pe_direito_var.set(dados.get('nivel_pe_direito', ''))
        self.ajuste_var.set(dados.get('ajuste', ''))
        self._carregando_fundo = False
        self._dados_carregados = self._coletar_dados_atual()
        self._atualizar_preview()  
    ...
    def _excluir_fundo(self):
        ...
        selection = self.lista_fundos.selection()
        if not selection:
            return
        item = selection[0]
        numero = str(self.lista_fundos.item(item)['values'][1])
