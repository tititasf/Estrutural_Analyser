...
try:
    from PySide6.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, 
                                   ...
                                   QGraphicsLineItem, QGraphicsRectItem, QGraphicsTextItem, QGraphicsSimpleTextItem, QGraphicsItem, QGraphicsItemGroup, QGraphicsPathItem,
                                   ...
                                   QListWidget, QTableWidget, QTableWidgetItem, QHeaderView, QButtonGroup)
    from PySide6.QtCore import Qt, Signal, QObject, QTimer, QRectF, QSize, QPoint
    from PySide6.QtGui import QColor, QPen, QBrush, QFont, QPainter, QAction, QIcon, QPalette, QCursor, QPainterPath
except ImportError:
...
# --- Custom Exceptions ---
class CancelledSelectionError(Exception):
    """Raised when the user cancels a selection in AutoCAD (e.g., by pressing ESC)."""
    pass
...
class VigaState:
    ...
    height2_global: str = ""
    grade_height1_global: str = "7.0"
    grade_height2_global: str = "7.0"
    ...
    # New Sarrafo IDs (0=None, 1=Extremidade, 2=Pressão)
    sarrafo_left_id: int = 1
    sarrafo_right_id: int = 1
    sarrafo_h2_left_id: int = 0
    sarrafo_h2_right_id: int = 0
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'VigaState':
        state = cls()
        # Basic fields copy logic if needed, but standard dataclass deserialization usually handles this externally
        # or we just rely on pickle. 
        # However, for JSON loading:
        for k, v in data.items():
            if hasattr(state, k):
                 setattr(state, k, v)
        
        # Legacy fallback for IDs if missing
        if 'sarrafo_left_id' not in data and 'sarrafo_left' in data:
             state.sarrafo_left_id = 1 if data['sarrafo_left'] else 0
        if 'sarrafo_right_id' not in data and 'sarrafo_right' in data:
             state.sarrafo_right_id = 1 if data['sarrafo_right'] else 0
        if 'sarrafo_h2_left_id' not in data and 'sarrafo_h2_left' in data:
             state.sarrafo_h2_left_id = 1 if data['sarrafo_h2_left'] else 0
        if 'sarrafo_h2_right_id' not in data and 'sarrafo_h2_right' in data:
             state.sarrafo_h2_right_id = 1 if data['sarrafo_h2_right'] else 0
...
class AutoCADService(QObject):
    ...
    def _bring_acad_to_front(self):
        ...
        import time
        time.sleep(0.1) # Reduzido de 0.3s

    def _wait_for_cad_idle(self, doc, timeout=0.5): # Reduzido timeout padrão
        """Tenta verificar se o CAD está aceitando chamadas (idle) antes de prosseguir."""
        import time
        t0 = time.time()
        while (time.time() - t0) < timeout:
            try:
                # Se ler CMDNAMES funcionar, o RPC não está bloqueado
                if doc.GetVariable("CMDNAMES") == "":
                    return True
                return True # Mesmo se tiver comando, se leu a var, a conexão está viva
            except:
                time.sleep(0.1) # Menor intervalo de retry
        return False
    ...
    def select_or_type_text(self, prompt_msg: str) -> Optional[str]:
        """Versão ultra-robusta otimizada para velocidade."""
        import time
        retries = 0
        max_retries = 2
        ...
        while retries < max_retries:
            try:
                ...
                self._wait_for_cad_idle(doc, timeout=0.5)

                # 1. Limpeza de Comando Ativo (Rápida)
                try:
                    if doc.GetVariable("CMDNAMES") != "":
                        doc.SendCommand(chr(27)*2) 
                        time.sleep(0.1)
                except: pass
                ...
                try:
                    ...
                    try:
                        ...
                        err_s = str(e).lower()
                        if "-2147352567" in err_s or "cancel" in err_s:
                            raise CancelledSelectionError("Seleção cancelada pelo usuário no AutoCAD.")
                        raise e
                    ...
                    # 3. Fallback: Digitação Manual
                    val = doc.Utility.GetString(1, f"\n{prompt_msg} [Digite o valor e ENTER]: ")
                    return val if val else None

                except CancelledSelectionError:
                    raise
                except Exception as e:
                    err_s = str(e).lower()
                    if any(x in err_s for x in ["rejected", "-2147418111", "server", "falha no selectionset", "rpc"]):
                        self.ac = None
                        time.sleep(0.5)
                        retries += 1
                        continue
    ...
    def select_line_length(self) -> float:
        ...
        self._bring_acad_to_front()
        try:
            # Garante que não há comando pendente
            try: doc.SendCommand(chr(27)*2)
            except: pass
            
            doc.SendCommand("._LINE ")
            import time
            
            # Espera ativa curta com detecção de interrupção
            slept = 0
            while slept < 60: # Max 60s
                if doc.GetVariable("CMDNAMES") == "":
                    break
                time.sleep(0.3)
                slept += 0.3
            ...
            if last_ent.ObjectName == "AcDbLine":
                ...
                try:
                    prompt = f"\nComprimento medido: {length:.2f}\nDigite o AJUSTE ou ENTER p/ 0 (ESC p/ cancelar): "
                    desc_str = doc.Utility.GetString(0, prompt)
                    if not desc_str: desc_str = "0"
                    desc_val = float(desc_str.replace(',', '.'))
                except Exception as e:
                    if "-2147352567" in str(e):
                        raise CancelledSelectionError("Seleção de comprimento cancelada.")
                    desc_val = 0.0
                ...
                # Usuário cancelou sem desenhar a linha ou deu ENTER vazio
                raise CancelledSelectionError("Linha de largura não foi desenhada.")
        except CancelledSelectionError:
            raise
        except Exception as e:
            if "-2147352567" in str(e):
                raise CancelledSelectionError("Seleção cancelada pelo usuário.")
            print(f"Line selection error: {e}")
        return 0.0
    ...
    def select_levels(self) -> Dict[str, float]:
        ...
        def get_level_val(prompt_idx):
            import time
            try:
                sel_name = f"TempSelLvl{prompt_idx}_{uuid.uuid4().hex[:4]}"
                
                # Tenta focar no AutoCAD antes de pedir seleção
                try: doc.Activate() 
                except: pass
                
                doc.Utility.Prompt(f"\nSelecione o NÍVEL {prompt_idx} (ou ESC p/ cancelar): ")
                ...
                try:
                    ...
                    if "-2147352567" in str(e):
                        raise CancelledSelectionError(f"Seleção de nível {prompt_idx} cancelada.")
                    raise e
    ...
    def select_pillar_dims(self, prompt="") -> Dict[str, float]:
        ...
        doc.SendCommand("._LINE ")
        doc.Utility.Prompt("\n>>> DESENHE O PILAR (2 ou 3 pontos) e finalize com ENTER (ou ESC p/ cancelar) <<<\n")
        ...
        try:
            ...
            if len(new_ents) == 0:
                 # Se não desenhou nada e o comando acabou, assumimos cancelamento ou pulo
                 raise CancelledSelectionError("Nenhuma medida de pilar desenhada.")
            ...
            if "-2147352567" in str(e):
                raise CancelledSelectionError("Seleção de pilar cancelada.")
...
class PreviewWidget(QGraphicsView):
    ...
    def draw_multiple(self, states: List[VigaState]):
        """Draws multiple VigaStates stacked vertically and adds Comparison Section View (Visão de Corte)."""
        self.scene.clear()
        if not states: return
        
        # 1. Main Beam Stacking
        y_offset = 0.0
        for state in states:
            root = self.scene.createItemGroup([])
            self._draw_viga_on_item(state, root)
            ...
            y_offset += rect.height() + 2500 

        # 2. Section View (Visão de Corte) - 800cm to the left to avoid any overlap
        uid_to_viga = {s.unique_id: s for s in states}
        processed_uids = set()
        unique_pair_sections = []
        ...
        def get_float(val):
            try:
                ...
                if not s_val.strip(): return 0.0
                return float(s_val)
            except:
                return 0.0

        def get_section_data(s):
            p = s.panels[0] if s.panels else None
            
            # Use Panel Data if available, otherwise default to 7.0
            def safe_grade(val):
                f = get_float(val)
                return f if f > 0.1 else 7.0

            g_h1 = safe_grade(p.grade_h1) if p else 7.0
            g_h2 = safe_grade(p.grade_h2) if p else 7.0

            return {
                'fundo': get_float(s.bottom),
                'h1': get_float(p.height1) if p else 0.0,
                'h2': get_float(p.height2) if p else 0.0,
                'ls': get_float(p.slab_top) if p else 0.0,
                'lc': get_float(p.slab_center) if p else 0.0,
                'li': get_float(p.slab_bottom) if p else 0.0,
                'type1': p.type1 if p else 'Sarrafeado',
                'type2': p.type2 if p else 'Sarrafeado',
                'grade_h1': g_h1,
                'grade_h2': g_h2,
                'name': s.name,
                'segment_class': getattr(s, 'segment_class', 'Unique')
            }

        for s in states:
            if s.unique_id in processed_uids: continue
            partners = [p.get('id') for p in s.combined_faces if p.get('id') in uid_to_viga]
            if partners:
                p_uid = partners[0]
                if p_uid in processed_uids: continue
                s1, s2 = s, uid_to_viga[p_uid]
                processed_uids.add(s1.unique_id)
                processed_uids.add(s2.unique_id)
                d1, d2 = get_section_data(s1), get_section_data(s2)
                f1 = (d1['fundo'], d1['h1'], d1['h2'], d1['ls'], d1['lc'], d1['li'])
                f2 = (d2['fundo'], d2['h1'], d2['h2'], d2['ls'], d2['lc'], d2['li'])
                fingerprint = tuple(sorted([f1, f2]))
                if fingerprint not in seen_fingerprints:
                    seen_fingerprints.add(fingerprint)
                    unique_pair_sections.append((d1, d2))

        # 3. Draw Section Views with safer items
        if unique_pair_sections:
            sx = -5000.0 # Moved further left
            sy = 0.0
            scale_factor = 17.0
            
            # Force Margin Logic: Add invisible item to extend SceneRect to the left
            margin_left = 5000.0 
            dummy_margin = QGraphicsRectItem(sx - margin_left, 0, 1, 1)
            dummy_margin.setPen(QPen(Qt.NoPen))
            self.scene.addItem(dummy_margin)
            
            font_label = QFont("Arial", 12, QFont.Bold)
            font_title = QFont("Arial", 16, QFont.Bold)
            
            # Cosmetic pen (hairline) for crisp edges
            pen_thin = QPen(QColor("#FFFFFF"), 0) 
            brush_solid = QBrush(QColor("#4CAF50"))
            brush_red = QBrush(QColor("#F44336")) # Red for sarrafos
            brush_pink = QBrush(QColor("#FFC0CB")) # Pink for grades

            for d1, d2 in unique_pair_sections:
                # 1. Identify Sides (A vs B)
                name1 = d1['name'].upper()
                name2 = d2['name'].upper()
                ...
                if ".B" in name1 and ".A" in name2:
                    data_A, data_B = d2, d1
                elif ".A" in name1 or "LADO A" in name1:
                    data_A = d1
                    data_B = d2
                elif ".B" in name1 or "LADO B" in name1:
                    data_A = d2
                    data_B = d1
                    
                # 2. Base Dimensions
                w_fundo = max(data_A['fundo'], data_B['fundo'])
                li_max = max(data_A['li'], data_B['li'])
                width = li_max if li_max > 0 else w_fundo
                if width <= 0: width = 50.0 

                # Global Height Calculation (approximate for spacing)
                sum_h_A = data_A['h1'] + data_A['h2'] + data_A['ls'] + data_A['lc'] + data_A['li']
                sum_h_B = data_B['h1'] + data_B['h2'] + data_B['ls'] + data_B['lc'] + data_B['li']
                total_h = max(sum_h_A, sum_h_B)
                if total_h <= 0: total_h = 60.0

                container = self.scene.createItemGroup([])
                container.setScale(scale_factor)
                
                # Pre-calculate mutual existence logic for External Reinforcements
                # Rule: Only draw if condition is met on BOTH sides independent of side, but checked mutually.
                # 1. Base check: No LI (Slab Bottom) on either side
                has_li_A = (data_A['li'] > 0)
                has_li_B = (data_B['li'] > 0)
                allow_base_ext = (not has_li_A) and (not has_li_B)
                
                # 2. Height checks (must satisfy height on BOTH sides)
                h1_A = data_A['h1']
                h1_B = data_B['h1']
                ...
                # 3. Draw Fundo (Bottom Plate + Extensions) - Unified Polygon
                fundo_path = QPainterPath()
                offset_drop_val = 11.0 # +2.2 from 8.8
                h_fundo = 2.0 # Thickness 2.0


                # Determine coordinates
                # FIX: Check if ANY side has LI to drop the ENTIRE bottom structure
                has_li_A = (data_A['li'] > 0)
                has_li_B = (data_B['li'] > 0)
                has_any_li = has_li_A or has_li_B
                
                # Unified Drop Level
                y_unified = offset_drop_val if has_any_li else 0.0
                
                # SHIFT Fundo Downwards ONLY if LI exists
                y_plate_draw = y_unified + (2.2 if has_any_li else 0.0)

                x_min = -70.0 if has_li_A else 0.0
                x_max = width + 70.0 if has_li_B else width
                
                # Construct Polygon (Flat Bottom at y_plate_draw)
                fundo_path.moveTo(x_min, y_plate_draw)
                fundo_path.lineTo(x_max, y_plate_draw)
                fundo_path.lineTo(x_max, y_plate_draw + h_fundo)
                fundo_path.lineTo(x_min, y_plate_draw + h_fundo)
                fundo_path.closeSubpath()
                
                # Draw unified path
                path_item = QGraphicsPathItem(fundo_path)
                path_item.setPen(pen_thin)
                path_item.setBrush(brush_solid)
                path_item.setParentItem(container)
                
                # 3.1 Draw Bottom Sarrafos (Sarrafo de Fundo)
                # Rules: Only if no LI. w<=10: full. 10<w<15: 5cm ends. w>=15: 7cm ends.
                # Every 30cm adds one more centered sarrafo (Longest Zip style distribution).
                if not has_any_li and w_fundo > 0:
                    h_sar = 2.2
                    y_sar = y_plate_draw + h_fundo
                    
                    if w_fundo <= 10:
                        # Case <= 10: Single full width
                        s_item = QGraphicsRectItem(0, y_sar, w_fundo, h_sar)
                        s_item.setPen(pen_thin); s_item.setBrush(brush_red); s_item.setParentItem(container)
                    else:
                        # Case > 10: Extremities
                        w_ext = 7.0 if w_fundo >= 15 else 5.0
                        
                        # Left end
                        sl = QGraphicsRectItem(0, y_sar, w_ext, h_sar)
                        sl.setPen(pen_thin); sl.setBrush(brush_red); sl.setParentItem(container)
                        
                        # Right end
                        sr = QGraphicsRectItem(w_fundo - w_ext, y_sar, w_ext, h_sar)
                        sr.setPen(pen_thin); sr.setBrush(brush_red); sr.setParentItem(container)
                        
                        # Intermediate Sarrafos (Every 30cm)
                        if w_fundo > 30:
                            num_center = int((w_fundo - 0.001) // 30)
                            # Space between end sarrafos
                            start_gap_x = w_ext
                            end_gap_x = w_fundo - w_ext
                            usable_gap = end_gap_x - start_gap_x
                            
                            # Standard size for intermediate is 7.0
                            num_gaps = num_center + 1
                            gap_w = (usable_gap - (num_center * 7.0)) / num_gaps
                            
                            for k in range(1, num_center + 1):
                                # Position = skip left end + k gaps + (k-1) intermediates
                                x_pos = start_gap_x + k * gap_w + (k-1) * 7.0
                                si = QGraphicsRectItem(x_pos, y_sar, 7.0, h_sar)
                                si.setPen(pen_thin); si.setBrush(brush_red); si.setParentItem(container)

                # Base Y Reference for Walls
                base_y_ref_0 = 4.0 
                
                # 4. Draw Vertical Parts (A and B)
                sides_config = [
                    {'data': data_A, 'is_A': True, 'x_wall': -1.8, 'line_dir': -1},
                    {'data': data_B, 'is_A': False, 'x_wall': width, 'line_dir': 1}
                ]

                # Pre-calculate base levels
                local_offset = y_unified
                y_start = base_y_ref_0 + local_offset

                for cfg in sides_config:
                    d = cfg['data']
                    h1 = d['h1']
                    h2 = d['h2']
                    li = d['li']
                    ...
                    ls = d['ls']
                    x_wall = cfg['x_wall']
                    line_dir = cfg['line_dir'] 
                    
                    # Pre-calculate levels (moving UP -> negative Y)
                    # Formula: Level = y_start - cumulative_heights
                    
                    # Logic adjustment: "desenhar a laje com o valor da laje -2"
                    # The vertical gap for slab should be (val - 2.0)
                    # And the support rectangle is 2.0 (was 1.8)
                    
                    val_lc_gap = max(0, lc - 2.0) if lc > 2.0 else 0
                    val_ls_gap = max(0, ls - 2.0) if ls > 2.0 else 0
                    
                    y_after_li = y_start - li

                    y_after_li = y_start - li
                    y_after_h1 = y_after_li - h1
                    y_after_lc = y_after_h1 - val_lc_gap
                    y_after_h2 = y_after_lc - h2
                    y_after_ls = y_after_h2 - val_ls_gap
                    
                    # 1. LI Gap & Line
                    if li > 0:
                        # Line at the top of the LI gap
                        line_x_start = 0 if cfg['is_A'] else width
                        line_x_end = line_x_start + (70.0 * line_dir)
                        l_item = QGraphicsLineItem(line_x_start, y_after_li, line_x_end, y_after_li)
                        l_item.setPen(pen_thin)
                        l_item.setParentItem(container)

                    # 2. Wall H1
                    if h1 > 0:
                        # Wall Item
                        wall_item = QGraphicsRectItem(x_wall, y_after_h1, 1.8, h1)
                        wall_item.setPen(pen_thin)
                        wall_item.setBrush(brush_solid)
                        wall_item.setParentItem(container)
                    
                    # 3. LC Components
                    if lc > 0:
                        # Rect Extra (Gravata) at y_after_h1 (height h1 + li ref)
                        # Now 2.0 height
                        rx = x_wall - 70 if cfg['is_A'] else x_wall
                        extra_rect = QGraphicsRectItem(rx, y_after_h1, 70, 2.0)
                        extra_rect.setPen(pen_thin)
                        extra_rect.setBrush(brush_solid)
                        extra_rect.setParentItem(container)
                        
                        # Sarrafo below LC support (2.2 x 7)
                        s_x = (x_wall - 7.0) if cfg['is_A'] else (x_wall + 1.8)
                        s_rect = QGraphicsRectItem(s_x, y_after_h1 + 2.0, 7.0, 2.2)
                        s_rect.setPen(pen_thin); s_rect.setBrush(brush_red); s_rect.setParentItem(container)
                        
                        # New: Grade or Sarrafo below LC support (Type 1)
                        mode = d['type1']
                        # Original Y position (below support sarrafo)
                        y_rs = y_after_h1 + 4.2
                        
                        if "Grade" in mode:
                            # Decrease height by 2.2
                            raw_h = d['grade_h1']
                            gr_h = max(0.1, raw_h - 2.2)
                            
                            gr_x = (x_wall - 7.0) if cfg['is_A'] else (x_wall + 1.8)
                            gr_item = QGraphicsRectItem(gr_x, y_rs, 7.0, gr_h)
                            gr_item.setPen(pen_thin); gr_item.setBrush(brush_pink); gr_item.setParentItem(container)
                        else: # Sarrafeado
                            sr_h = max(0, h1 - 2.2) # Subtract 2.2 for the support sarrafo
                            sr_x = (x_wall - 2.2) if cfg['is_A'] else (x_wall + 1.8)
                            sr_item = QGraphicsRectItem(sr_x, y_rs, 2.2, sr_h)
                            sr_item.setPen(pen_thin); sr_item.setBrush(brush_red); sr_item.setParentItem(container)
                        
                        # Line at top of LC
                        line_x_start = 0 if cfg['is_A'] else width
                        line_x_end = line_x_start + (70.0 * line_dir)
                        l_item = QGraphicsLineItem(line_x_start, y_after_lc, line_x_end, y_after_lc)
                        l_item.setPen(pen_thin)
                        l_item.setParentItem(container)
                        
                    # 4. LS Components (Support Rect)
                    if ls > 0:
                         # Rect Extra for LS at y_after_h2 ? 
                         # Actually if LS is on top of H2, support is at y_after_h2.
                         # If no H2, it's at y_after_lc?
                         # Let's rely on y_after_h2 which accumulates all previous.
                         
                         rx = x_wall - 70 if cfg['is_A'] else x_wall
                         extra_rect = QGraphicsRectItem(rx, y_after_lc if h2 <= 0 else y_after_h2, 70, 2.0)
                         extra_rect.setPen(pen_thin)
                         extra_rect.setBrush(brush_solid)
                         extra_rect.setParentItem(container)
                         
                         # Sarrafo below LS support (2.2 x 7)
                         s_y = (y_after_lc if h2 <= 0 else y_after_h2) + 2.0
                         s_x = (x_wall - 7.0) if cfg['is_A'] else (x_wall + 1.8)
                         s_rect = QGraphicsRectItem(s_x, s_y, 7.0, 2.2)
                         s_rect.setPen(pen_thin); s_rect.setBrush(brush_red); s_rect.setParentItem(container)
                         
                         # New: Grade or Sarrafo below LS support (Type 2 if LC exists, else Type 1)
                         is_h2 = (lc > 0)
                         mode = d['type2'] if is_h2 else d['type1']
                         
                         y_rs = s_y + 2.2
                         
                         if "Grade" in mode:
                             raw_h = d['grade_h2'] if is_h2 else d['grade_h1']
                             gr_h = max(0.1, raw_h - 2.2)
                             ...
                             wh = h2 if is_h2 else h1
                             sr_h = max(0, wh - 2.2) # Subtract 2.2 for support sarrafo
                             sr_x = (x_wall - 2.2) if cfg['is_A'] else (x_wall + 1.8)
                             sr_item = QGraphicsRectItem(sr_x, y_rs, 2.2, sr_h)
                             sr_item.setPen(pen_thin); sr_item.setBrush(brush_red); sr_item.setParentItem(container)
                    ...
                    # 5. Wall H2
                    if h2 > 0:
                        wall_item = QGraphicsRectItem(x_wall, y_after_h2, 1.8, h2)
                        wall_item.setPen(pen_thin)
                        wall_item.setBrush(brush_solid)
                        wall_item.setParentItem(container)

                        # NEW: Corner reinforcement if LS is missing but H2 exists (lc > 0)
                        if ls <= 0:
                            # Top corner on H2
                            s_x = (x_wall - 7.0) if cfg['is_A'] else (x_wall + 1.8)
                            ts_rect = QGraphicsRectItem(s_x, y_after_h2, 7.0, 2.2)
                            ts_rect.setPen(pen_thin); ts_rect.setBrush(brush_red); ts_rect.setParentItem(container)
                            
                            # Grade/Sarrafo below this top support (using Type 2 settings)
                            mode = d['type2']
                            y_rs = y_after_h2 + 2.2
                            
                            if "Grade" in mode:
                                # User Request: "só no modo sem laje... ajusta para ele apenas"
                                # Restore full height logic for No-Slab termination
                                gr_h = d['grade_h2'] # Full height, no -2.2 reduction
                    ...
                    # 6. Case NO Slabs (Top Reinforcement for H1)
                    if lc <= 0 and ls <= 0:
                        # Draw support sarrafo at the very top of H1
                        top_y = y_after_h1
                        s_x = (x_wall - 7.0) if cfg['is_A'] else (x_wall + 1.8)
                        ts_rect = QGraphicsRectItem(s_x, top_y, 7.0, 2.2)
                        ts_rect.setPen(pen_thin); ts_rect.setBrush(brush_red); ts_rect.setParentItem(container)
                        
                        # Grade or Sarrafo below this top support (using Type 1 settings)
                        mode = d['type1']
                        y_rs = top_y + 2.2
                        ...
                        if "Grade" in mode:
                            # User Request: "só no modo sem laje... ajusta para ele apenas"
                            # Restore full height logic for No-Slab termination
                            gr_h = d['grade_h1'] # Full height, no -2.2 reduction
                    ...
                    # 7. LS Line (at the very top)
                    if ls > 0:
                         # Line at top of LS
                         line_x_start = 0 if cfg['is_A'] else width
                         line_x_end = line_x_start + (70.0 * line_dir)
                         l_item = QGraphicsLineItem(line_x_start, y_after_ls, line_x_end, y_after_ls)
                         l_item.setPen(pen_thin)
                         l_item.setParentItem(container)

                    # 8. External Horizontal Reinforcements (2.2x10)
                    # 8. External Horizontal Reinforcements (2.2x10)
                    if not has_any_li:
                         # Only proceed if the "Base" mutual rule is met (Both sides have no LI)
                         if allow_base_ext:
                            # Determine offset based on mode (using d['type1'] as reference for side's base mode)
                            side_mode = d['type1']
                            h_offset = 8.8 if "Grade" in side_mode else 4.0
                            
                            # Position X (relative to outer edge of fundo sarrafo)
                            if cfg['is_A']:
                                # Left of left end (starts at 0)
                                rx_ext = -h_offset - 10.0
                            else:
                                # Right of right end (ends at width)
                                rx_ext = width + h_offset
                                
                            # Vertical positions (y_start is the base level)
                            y_targets = [y_start]
                            
                            # Apply mutual height rules
                            if allow_40_ext:
                                y_targets.extend([y_start - 22.0, y_start - 26.0])
                            
                            if allow_90_ext:
                                y_targets.extend([y_start - 72.0, y_start - 76.0])
                                
                            for yt in y_targets:
                                # User Request: "subir 2.2cm os horizontais" -> Subtract 2.2 from Y
                                reinc = QGraphicsRectItem(rx_ext, yt - 2.2, 10.0, 2.2)
                                reinc.setPen(pen_thin); reinc.setBrush(brush_red); reinc.setParentItem(container)

                    # 7. Dimensions (Cotas)
                    # Configuration per side
                    dim_x_out = -75.0 if cfg['is_A'] else width + 95.0
                    
                    # Internal Dimension X (Inside the U shape)
                    # Side A (Left): Wall is at -1.8. 
                    # Side B (Right): Wall is at width. 
                    # User requested moving 30cm RIGHT TOTAL from (width-25) -> (width+5)
                    dim_x_in  = 25.0 if cfg['is_A'] else width + 5.0 
                    
                    target_x = 0.0 if cfg['is_A'] else width
                    
                    # Helper function execution inline
                    def draw_dim_segment(x_line, y_bottom, y_top, value, is_bold=False, scale=1.0, align_right=False):
                        if value <= 0.1: return

                        # Draw Dimension Line
                        l = QGraphicsLineItem(x_line, y_bottom, x_line, y_top)
                        l.setPen(pen_thin); l.setParentItem(container)

                        # Draw Witness Lines
                        # Logic: usually go from x_line to target_x.
                        # If x_line is inside and target_x is wall, this works.
                        wb = QGraphicsLineItem(x_line, y_bottom, target_x, y_bottom)
                        wb.setPen(pen_thin); wb.setParentItem(container)
                        
                        wt = QGraphicsLineItem(x_line, y_top, target_x, y_top)
                        wt.setPen(pen_thin); wt.setParentItem(container)

                        # Draw Ticks
                        t1 = QGraphicsLineItem(x_line - 5, y_bottom, x_line + 5, y_bottom)
                        t1.setPen(pen_thin); t1.setParentItem(container)
                        t2 = QGraphicsLineItem(x_line - 5, y_top, x_line + 5, y_top)
                        t2.setPen(pen_thin); t2.setParentItem(container)

                        # Draw Text
                        s_val = f"{value:.0f}"
                        if abs(value - round(value)) >= 0.1: s_val = f"{value:.1f}"
                        
                        txt_item = QGraphicsSimpleTextItem(s_val)
                        txt_item.setFont(font_title if is_bold else font_label)
                        txt_item.setBrush(QBrush(QColor("#FFFFFF")))
                        txt_item.setScale(scale)
                        
                        # Position Text
                        # Scale affects boundingRect size visually but not the rect values directly in local coords
                        # we must account for scale in positioning
                        tw = txt_item.boundingRect().width() * scale
                        th = txt_item.boundingRect().height() * scale
                        ym = (y_bottom + y_top) / 2.0
                        
                        # General logic: 
                        # Side A (Left External): Line at -95. Text to Left (-tw - 5).
                        # Side B (Right External): Line at W+95. Text to Right (+5).
                        # Internal A (25): Text to Right? Or Left? Usually centered or to right. 
                        # Internal B (W-25): Text to Left? 
                        
                        # Let's standardize: 
                        # If align_right=True, place text at x_line + 5
                        # If align_right=False, place text at x_line - tw - 5
                        
                        # Defaults based on config if not explicit
                        if align_right:
                             tx_pos = x_line + 5
                        else:
                             tx_pos = x_line - tw - 5
                            
                        # Override for specific internal cases if needed, but 'align_right' param handles it.
                        # For Side A External: align_right=False
                        # For Side B External: align_right=True
                        
                        txt_item.setPos(tx_pos, ym - th/2.0)
                        txt_item.setParentItem(container)

                    # --- Detailed Segment Dimensions (Run for both A and B) ---
                    # Uses local variables (li, h1...) from the loop
                    # External dimensions -> Standard Font, Standard Position
                    ext_align = not cfg['is_A'] # Right align if Side B
                    
                    if li > 0: draw_dim_segment(dim_x_out, y_start - 2.0, y_after_li, max(0, li - 2.0), align_right=ext_align)
                    if h1 > 0: draw_dim_segment(dim_x_out, y_after_li, y_after_h1 + 2.0, h1, align_right=ext_align)
                    if lc > 0: draw_dim_segment(dim_x_out, y_after_h1, y_after_lc, max(0, lc - 2.0), align_right=ext_align)
                    if h2 > 0: draw_dim_segment(dim_x_out, y_after_lc, y_after_h2 + 2.0, h2, align_right=ext_align)
                    if ls > 0: draw_dim_segment(dim_x_out, y_after_h2, y_after_ls, max(0, ls - 2.0), align_right=ext_align)

                    # --- Internal Dimensions (Side B Only) ---
                    if not cfg['is_A']:
                        # Total Height Sum
                        total_val = li + h1 + lc + h2 + ls
                        
                        # Find Top Y
                        y_top_total = y_start
                        if ls > 0: y_top_total = min(y_top_total, y_after_ls)
                        elif h2 > 0: y_top_total = min(y_top_total, y_after_h2)
                        elif lc > 0: y_top_total = min(y_top_total, y_after_lc)
                        elif h1 > 0: y_top_total = min(y_top_total, y_after_h1)
                        elif li > 0: y_top_total = min(y_top_total, y_after_li)
                        
                        # Draw Internal Dimension (Inside Right Wall)
                        # Position: dim_x_in (width - 25). 
                        # Text: Small (0.5 scale). 
                        # Alignment: "posicionas dentro". If line is at width-25, text to LEFT (width-25 - tw - 5) is "more inside".
                        # But user said "posicao do texto da cota 0". 
                        # Let's try putting text on the Left of the dim line (align_right=False).
                        
                        if total_val > 0.1:
                            draw_dim_segment(dim_x_in, y_start, y_top_total, total_val, is_bold=True, scale=0.5, align_right=False)
                        
                        # --- Extra Height Dimension (50cm -> 150cm to Right) ---
                        # Dim Line at width + 150.0
                        # Target X at width (wall)
                        dim_x_extra = width + 150.0
                        draw_dim_segment(dim_x_extra, y_start, y_top_total, total_val, is_bold=True, scale=1.0, align_right=True)


                        # --- C. Bottom Fundo Dimension (Horizontal) ---
                        # Move closer: y_start + 20.0 (Raised 20cm from previous 40)
                        # User requested "mais 30cm para cima" -> (+20 - 30) = -10.0
                        dim_y_bot = y_start - 10.0
                        
                        # Line
                        ln = QGraphicsLineItem(0, dim_y_bot, width, dim_y_bot)
                        ln.setPen(pen_thin); ln.setParentItem(container)
                        
                        # Witnesses
                        w1 = QGraphicsLineItem(0, y_start, 0, dim_y_bot)
                        w1.setPen(pen_thin); w1.setParentItem(container)
                        w2 = QGraphicsLineItem(width, y_start, width, dim_y_bot)
                        w2.setPen(pen_thin); w2.setParentItem(container)
                        
                        # Ticks
                        tk1 = QGraphicsLineItem(0, dim_y_bot - 5, 0, dim_y_bot + 5)
                        tk1.setPen(pen_thin); tk1.setParentItem(container)
                        tk2 = QGraphicsLineItem(width, dim_y_bot - 5, width, dim_y_bot + 5)
                        tk2.setPen(pen_thin); tk2.setParentItem(container)
                        
                        # Text
                        w_txt = f"{width:.0f}"
                        if abs(width - round(width)) >= 0.1: w_txt = f"{width:.1f}"
                        ti = QGraphicsSimpleTextItem(w_txt)
                        ti.setFont(font_label)
                        ti.setBrush(QBrush(QColor("#FFFFFF")))
                        ti.setScale(0.5) # Half size
                        
                        # Position Text (Centered)
                        ti_w = ti.boundingRect().width() * 0.5
                        ti_h = ti.boundingRect().height() * 0.5
                        
                        # "posicionas dentro do desenho"? 
                        # Placing it slighly ABOVE the dimension line (closer to drawing) might be what's needed.
                        ti.setPos(width/2 - ti_w/2, dim_y_bot - ti_h - 2) 
                        ti.setParentItem(container)

                # ==============================================================================
                # 9. Ties (Gravatas) - Connecting Side A and Side B
                # ==============================================================================
                
                # Calculate Horizontal Positions for Ties (Center to Center of External Sarrafos)
                # Side A
                mode_A = data_A['type1']; off_A = 8.8 if "Grade" in mode_A else 4.0
                rx_A = -off_A - 10.0; center_A = rx_A + 5.0
                
                # Side B
                mode_B = data_B['type1']; off_B = 8.8 if "Grade" in mode_B else 4.0
                rx_B = width + off_B; center_B = rx_B + 5.0
                
                tie_width = center_B - center_A
                
                if allow_base_ext:
                    # 9a. Bottom Tie: 1cm below, 1cm thick
                    tie_y = y_start + 1.0 
                    tie_h = 1.0 # 1cm thick
                    
                    tie_item = QGraphicsRectItem(center_A, tie_y, tie_width, tie_h)
                    tie_item.setPen(pen_thin); tie_item.setBrush(brush_red); tie_item.setParentItem(container)
                    
                    # 9b. Conditional Top Ties (Connecting the PAIRS of conditional sarrafos)
                    # Tie needs to be centered in the gap between the two bars of the pair.
                    # Pair 1: 22 and 26. Lower Top: y-24.2. Upper Bottom: y-26. Gap Center: y-25.1
                    # Pair 2: 72 and 76. Lower Top: y-74.2. Upper Bottom: y-76. Gap Center: y-75.1
                    
                    if allow_40_ext:
                         # Center at -25.1. Top of 1.0 tie at -25.6
                         t_y = y_start - 25.6
                         t_item = QGraphicsRectItem(center_A, t_y, tie_width, 1.0)
                         t_item.setPen(pen_thin); t_item.setBrush(brush_red); t_item.setParentItem(container)

                    if allow_90_ext:
                         # Center at -75.1. Top of 1.0 tie at -75.6
                         t_y = y_start - 75.6
                         t_item = QGraphicsRectItem(center_A, t_y, tie_width, 1.0)
                         t_item.setPen(pen_thin); t_item.setBrush(brush_red); t_item.setParentItem(container)


                # 5. Letters A B C (Outside 'for cfg' loop, inside 'for d1,d2')
                # A (Left) - Adjust Y by LI A
                lA = QGraphicsSimpleTextItem("A")
                lA.setFont(font_label)
                lA.setBrush(QBrush(QColor("#FFFFFF")))
                y_pos_A = (-lA.boundingRect().height()/2.0) - data_A['li']
                lA.setPos(-20 - lA.boundingRect().width(), y_pos_A)
                lA.setParentItem(container)
                
                # B (Right) - Adjust Y by LI B
                lB = QGraphicsSimpleTextItem("B")
                lB.setFont(font_label)
                lB.setBrush(QBrush(QColor("#FFFFFF")))
                y_pos_B = (-lB.boundingRect().height()/2.0) - data_B['li']
                lB.setPos(width + 20, y_pos_B)
                lB.setParentItem(container)
                
                # C (Center Bottom)
                lC = QGraphicsSimpleTextItem("C"); lC.setFont(font_label); lC.setBrush(QBrush(QColor("#FFFFFF")))
                y_pos_C = y_plate_draw + 10.0
                lC.setPos((width/2.0) - (lC.boundingRect().width()/2.0), y_pos_C); lC.setParentItem(container)

                # Title & Dimensions (Centered)
                title_name = QGraphicsSimpleTextItem(f"{data_A['segment_class']}")
                title_name.setFont(font_title)
                title_name.setBrush(QBrush(QColor("#FFFFFF")))
                
                title_dim = QGraphicsSimpleTextItem(f"({width:.0f}x{total_h:.0f})")
                title_dim.setFont(font_title)
                title_dim.setBrush(QBrush(QColor("#FFFFFF")))

                name_width = title_name.boundingRect().width()
                title_name.setPos((width - name_width) / 2.0, -total_h - title_dim.boundingRect().height() - title_name.boundingRect().height() - 5)
                title_name.setParentItem(container)

                dim_width = title_dim.boundingRect().width()
                title_dim.setPos((width - dim_width) / 2.0, -total_h - title_dim.boundingRect().height())
                title_dim.setParentItem(container)

                container.setPos(sx, sy)
                sy += (total_h * scale_factor) + 2000.0
        ...
        try:
            ...
            for i, p in enumerate(state.panels):
                ...
                if h2 > 0:
                    ...
                    # --- Visualização Sarrafo Extremidade H2 ---
                    if is_first and state.sarrafo_h2_left:
                         sar_w = 7 * base_scale
                         sar_h = h2_px 
                         add_rect(current_x, top_y - h2_px, sar_w, sar_h, QPen(Qt.NoPen), c_sarrafo)

                    if is_last and state.sarrafo_h2_right:
                         sar_w = 7 * base_scale
                         sar_h = h2_px 
                         add_rect(current_x + w - sar_w, top_y - h2_px, sar_w, sar_h, QPen(Qt.NoPen), c_sarrafo)
                    ...
                    if p.type2 == "Sarrafeado":
                         ...
                         v_w = 3.5 * base_scale
                         # Corrected: Use grade_h2 and safe default
                         v_h = (p.grade_h2 if p.grade_h2 > 0.1 else 7.0) * base_scale
                         vy = gy + grade_h
                         add_rect(current_x + off_start, vy, v_w, v_h, QPen(Qt.NoPen), c_grade)
                         add_rect(current_x + w - v_w - off_end, vy, v_w, v_h, QPen(Qt.NoPen), c_grade)
                    top_y -= h2_px
            ...
            if first_panel_idx != -1:
                ...
                if h2 > 0:
                    ...
                    # --- H2 Extremity Sarrafos Visualization ---
                    if is_first and state.sarrafo_h2_left:
                        sar_w = 7 * base_scale
                        # Top of H2 in draw coords is roughly (curr_y + h2_px) (since curr_y was just decremented)
                        # Actually h2 block is drawn at add_rect(..., y_h2_base - h2_px, ...)
                        # We need to match that coordinate system. But here we are in DIMENSION drawing block.
                        # Wait, the dimension drawing block is separate from the RECT drawing block.
                        # I should probably move this logic to the RECT drawing block above (around line 1400-1500 in previous view, but here lines 1660 are purely dimensions?)
                        pass
                if s_t > 0:
...
class VigaMainWindow(QMainWindow):
    ...
    def show_settings(self):
        ...
        self.input_parafuso_block = QLineEdit(str(self.config["opcoes"].get("parafuso_block", "PAR_ESQ")))
        form_opts.addRow("BLOCK DOS PARAFUSO:", self.input_parafuso_block)
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            self.config["opcoes"]["parafuso_block"] = self.input_parafuso_block.text()
    ...
    def init_ui(self):
        ...
        self.tree1.header().sectionClicked.connect(self.on_viga_header_clicked)
        self.tree1.header().setSectionsClickable(True)
        ...
        self.list_sort_column = -1
        self.list_sort_order = Qt.AscendingOrder
        ...
        # Auto Combine Buttons
        h_auto_combine = QHBoxLayout()
        btn_auto_set = QPushButton("Comb. Auto Conjunto")
        btn_auto_set.setToolTip("Combina automaticamente as vigas do conjunto atual (Classe/Número) usando lógica A(Cresc) - B(Decresc).")
        btn_auto_set.setStyleSheet("background-color: #26A69A; color: white; font-weight: bold;")
        btn_auto_set.clicked.connect(self.action_auto_combine_set)

        btn_auto_pav = QPushButton("Comb. Auto Pavimento")
        btn_auto_pav.setToolTip("Combina automaticamente TODAS as vigas do pavimento (agrupadas por conjunto) usando lógica A(Cresc) - B(Decresc).")
        btn_auto_pav.setStyleSheet("background-color: #00897B; color: white; font-weight: bold;")
        btn_auto_pav.clicked.connect(self.action_auto_combine_pav)

        h_auto_combine.addWidget(self._wrap_button(btn_auto_set, "Combina A e B do conjunto atual automaticamente."))
        h_auto_combine.addWidget(self._wrap_button(btn_auto_pav, "Combina todo o pavimento automaticamente."))
        layout_btns_common.addLayout(h_auto_combine)
        ...
        cmds = [
            ...
            ("Gerar Segmento", self.generate_script_current),
        ...
        for text, func in cmds:
            ...
            elif text == "Reiniciar Seleção":
                btn.setStyleSheet("background-color: #FF9800; color: black; font-weight: bold;") # Laranja
            elif text in ["Select Pilar", "Select Abertura", "Select Níveis"]:
                btn.setStyleSheet("background-color: #CE93D8; color: black; font-weight: bold;") # Lilás
            elif text == "Gerar Segmento":
                btn.setStyleSheet("background-color: #FFEB3B; color: black; font-weight: bold;") # Amarelo
            elif text == "Gerar conjunto de Viga":
                btn.setStyleSheet("background-color: #1976D2; font-weight: bold;") # Azul
            elif text == "Gerar Pavimento":
                btn.setStyleSheet("background-color: #C62828; font-weight: bold;") # Vermelho
            elif text == "Salvar":
                btn.setStyleSheet("background-color: #00BCD4; color: black; font-weight: bold; height: 35px;") # Ciano, mais gordinho
            
            wrapped = self._wrap_button(btn, cmd_infos.get(text, "Sem informações disponíveis."))
            
            # Ocultar botões específicos solicitados
            if text in ["Select Nomes", "Select Linha", "Selecionar Nível Pé Direito", "Combinar", "Ordenar"]:
                wrapped.hide()
                continue

            # Lógica de Posicionamento na Grid
            if text == "Salvar":
                # Salvar ocupa espaço de 2 botões (coluna 0 e 1 da linha 3)
                grid_cmd.addWidget(wrapped, 3, 0, 1, 2)
            else:
    ...
    def _wrap_help_widget(self, widget, info):
        """Wraps a generic widget with an info button."""
        w = QWidget()
        l = QHBoxLayout(w)
        l.setContentsMargins(0, 0, 0, 0)
        l.setSpacing(4)
    ...
    def init_tab_general(self):
        ...
        self.bg_continuation.addButton(self.rb_cont_next)
        self.bg_continuation.addButton(self.rb_cont_cont)
        ...
        # Grouping for Isolation
        self.bg_side = QButtonGroup(self)
        self.bg_side.addButton(self.rb_side_a)
        self.bg_side.addButton(self.rb_side_b)
        ...
        # Replace chk_prod_fh0 with H1/H2 Buttons
        self.w_fh0 = QWidget(); l_fh0 = QHBoxLayout(self.w_fh0); l_fh0.setContentsMargins(0,0,0,0)
        self.rb_h1_0 = QRadioButton("H1"); self.rb_h2_0 = QRadioButton("H2")
        self.bg_h0 = QButtonGroup(self); self.bg_h0.addButton(self.rb_h1_0); self.bg_h0.addButton(self.rb_h2_0)
        self.rb_h2_0.setChecked(True)
        l_fh0.addWidget(self.rb_h1_0); l_fh0.addWidget(self.rb_h2_0); l_fh0.addStretch()
        ...
        # Wrap widget with label for tooltip
        l_next.addWidget(self._wrap_help_widget(self.w_fh0, "Select H1 (Total) or H2 (Step) height logic."), 1, 1)
        
        # Row 2: H1
        self.chk_prod_h1 = QCheckBox("Lado Esquerdo / Fundo")
        self.w_fh1 = QWidget(); l_fh1 = QHBoxLayout(self.w_fh1); l_fh1.setContentsMargins(0,0,0,0)
        self.rb_h1_1 = QRadioButton("H1"); self.rb_h2_1 = QRadioButton("H2")
        self.bg_h1 = QButtonGroup(self); self.bg_h1.addButton(self.rb_h1_1); self.bg_h1.addButton(self.rb_h2_1)
        self.rb_h2_1.setChecked(True)
        l_fh1.addWidget(self.rb_h1_1); l_fh1.addWidget(self.rb_h2_1); l_fh1.addStretch()

        l_next.addWidget(self._wrap_chk(self.chk_prod_h1, "Captura abertura para o Fundo Esquerdo via seleção de texto no CAD."), 2, 0)
        l_next.addWidget(self._wrap_help_widget(self.w_fh1, "Select H1 (Total) or H2 (Step) height logic."), 2, 1)
        
        # Row 3: H2
        self.chk_prod_h2 = QCheckBox("Lado Direito / Topo")
        self.w_fh2 = QWidget(); l_fh2 = QHBoxLayout(self.w_fh2); l_fh2.setContentsMargins(0,0,0,0)
        self.rb_h1_2 = QRadioButton("H1"); self.rb_h2_2 = QRadioButton("H2")
        self.bg_h2 = QButtonGroup(self); self.bg_h2.addButton(self.rb_h1_2); self.bg_h2.addButton(self.rb_h2_2)
        self.rb_h2_2.setChecked(True)
        l_fh2.addWidget(self.rb_h1_2); l_fh2.addWidget(self.rb_h2_2); l_fh2.addStretch()

        l_next.addWidget(self._wrap_chk(self.chk_prod_h2, "Captura abertura para o Topo Direito via seleção de texto no CAD."), 3, 0)
        l_next.addWidget(self._wrap_help_widget(self.w_fh2, "Select H1 (Total) or H2 (Step) height logic."), 3, 1)
        
        # Row 4: H3
        self.chk_prod_h3 = QCheckBox("Lado Direito / Fundo")
        self.w_fh3 = QWidget(); l_fh3 = QHBoxLayout(self.w_fh3); l_fh3.setContentsMargins(0,0,0,0)
        self.rb_h1_3 = QRadioButton("H1"); self.rb_h2_3 = QRadioButton("H2")
        self.bg_h3 = QButtonGroup(self); self.bg_h3.addButton(self.rb_h1_3); self.bg_h3.addButton(self.rb_h2_3)
        self.rb_h2_3.setChecked(True)
        l_fh3.addWidget(self.rb_h1_3); l_fh3.addWidget(self.rb_h2_3); l_fh3.addStretch()

        l_next.addWidget(self._wrap_chk(self.chk_prod_h3, "Captura abertura para o Fundo Direito via seleção de texto no CAD."), 4, 0)
        l_next.addWidget(self._wrap_help_widget(self.w_fh3, "Select H1 (Total) or H2 (Step) height logic."), 4, 1)
    ...
    def init_tab_panels(self):
        ...
        row2.addWidget(self.u_rb_sarr1); row2.addWidget(self.u_rb_grade1); row2.addWidget(QLabel(" Altura grade 1:")); row2.addWidget(self.u_gh1)
        ...
        row2.addWidget(self.u_rb_sarr2); row2.addWidget(self.u_rb_grade2); row2.addWidget(QLabel(" Altura grade 2:")); row2.addWidget(self.u_gh2)
        row2.addStretch()
        ...
        # Headers
        headers = ["P", "Largura", "Alt 1", "Alt Grade 1", "Tipo 1", "Alt 2", "Alt Grade 2", "Tipo 2", "L.Sup", "L.Inf", "L.Cen"]
        ...
        self.u_h1.textChanged.connect(self.update_model)
        self.u_h1.editingFinished.connect(self._update_heights_smart)
        
        self.u_h2.textChanged.connect(self.update_model)
        self.u_h2.editingFinished.connect(self._update_heights_smart)
        
        # Universal Slabs Logic
        self.u_slab_t.textChanged.connect(lambda t: [r['ls'].setText(t) for r in self.panel_rows])
        # Only propagate text, do not force recalc of heights
    ...
    def init_tab_details(self):
        ...
        # Table Headers
        l_opt.addWidget(QLabel("Item"), 0, 0)
        l_opt.addWidget(QLabel("Nenhum"), 0, 1)
        l_opt.addWidget(QLabel("Extremidade"), 0, 2)
        l_opt.addWidget(QLabel("Pressão"), 0, 3)
        
        # Rows Definition
        # We will use ButtonGroups for exclusivity logic. 
        # IDs: 0=None, 1=Extremidade, 2=Pressão
        
        # 1. H1 Esq (Sarrafo vert. esq)
        l_opt.addWidget(QLabel("Sarrafo H1 Esq"), 1, 0)
        self.bg_h1_l = QButtonGroup(grp_opt)
        self.rb_h1_l_none = QRadioButton(); self.bg_h1_l.addButton(self.rb_h1_l_none, 0)
        self.rb_h1_l_ext = QRadioButton(); self.bg_h1_l.addButton(self.rb_h1_l_ext, 1)
        self.rb_h1_l_press = QRadioButton(); self.bg_h1_l.addButton(self.rb_h1_l_press, 2)
        l_opt.addWidget(self.rb_h1_l_none, 1, 1); l_opt.addWidget(self.rb_h1_l_ext, 1, 2); l_opt.addWidget(self.rb_h1_l_press, 1, 3)
        
        # 2. H1 Dir (Sarrafo vert. dir)
        l_opt.addWidget(QLabel("Sarrafo H1 Dir"), 2, 0)
        self.bg_h1_r = QButtonGroup(grp_opt)
        self.rb_h1_r_none = QRadioButton(); self.bg_h1_r.addButton(self.rb_h1_r_none, 0)
        self.rb_h1_r_ext = QRadioButton(); self.bg_h1_r.addButton(self.rb_h1_r_ext, 1)
        self.rb_h1_r_press = QRadioButton(); self.bg_h1_r.addButton(self.rb_h1_r_press, 2)
        l_opt.addWidget(self.rb_h1_r_none, 2, 1); l_opt.addWidget(self.rb_h1_r_ext, 2, 2); l_opt.addWidget(self.rb_h1_r_press, 2, 3)
        
        # 3. H2 Esq (Sarrafo H2 esq)
        l_opt.addWidget(QLabel("Sarrafo H2 Esq"), 3, 0)
        self.bg_h2_l = QButtonGroup(grp_opt)
        self.rb_h2_l_none = QRadioButton(); self.bg_h2_l.addButton(self.rb_h2_l_none, 0)
        self.rb_h2_l_ext = QRadioButton(); self.bg_h2_l.addButton(self.rb_h2_l_ext, 1)
        self.rb_h2_l_press = QRadioButton(); self.bg_h2_l.addButton(self.rb_h2_l_press, 2)
        l_opt.addWidget(self.rb_h2_l_none, 3, 1); l_opt.addWidget(self.rb_h2_l_ext, 3, 2); l_opt.addWidget(self.rb_h2_l_press, 3, 3)
        
        # 4. H2 Dir (Sarrafo H2 dir)
        l_opt.addWidget(QLabel("Sarrafo H2 Dir"), 4, 0)
        self.bg_h2_r = QButtonGroup(grp_opt)
        self.rb_h2_r_none = QRadioButton(); self.bg_h2_r.addButton(self.rb_h2_r_none, 0)
        self.rb_h2_r_ext = QRadioButton(); self.bg_h2_r.addButton(self.rb_h2_r_ext, 1)
        self.rb_h2_r_press = QRadioButton(); self.bg_h2_r.addButton(self.rb_h2_r_press, 2)
        l_opt.addWidget(self.rb_h2_r_none, 4, 1); l_opt.addWidget(self.rb_h2_r_ext, 4, 2); l_opt.addWidget(self.rb_h2_r_press, 4, 3)
        ...
        # Connect
        for bg in [self.bg_h1_l, self.bg_h1_r, self.bg_h2_l, self.bg_h2_r]: bg.buttonClicked.connect(self.update_model)
        ...
        # 0: Topo/Esq
        self.bg_h0.buttonToggled.connect(lambda b, c: self.hole_rows[0][3].setChecked(self.rb_h1_0.isChecked()))
        self.hole_rows[0][3].toggled.connect(lambda c: self.rb_h1_0.setChecked(c) if c else self.rb_h2_0.setChecked(True))
        
        # 1: Fundo/Esq
        self.bg_h1.buttonToggled.connect(lambda b, c: self.hole_rows[1][3].setChecked(self.rb_h1_1.isChecked()))
        self.hole_rows[1][3].toggled.connect(lambda c: self.rb_h1_1.setChecked(c) if c else self.rb_h2_1.setChecked(True))
    ...
    def load_beam_view(self, key):
        ...
        # Only switch if currently on the disabled Class tab
    ...
    def collect_current_data(self):
        ...
        data = {
            ...
            'sarrafo_left_id': self.bg_h1_l.checkedId(),
            'sarrafo_right_id': self.bg_h1_r.checkedId(),
            'sarrafo_h2_left_id': self.bg_h2_l.checkedId(),
            'sarrafo_h2_right_id': self.bg_h2_r.checkedId(),
            # Legacy/Generator compatibility (True ONLY if Extremidade)
            'sarrafo_left': self.bg_h1_l.checkedId() == 1,
            'sarrafo_right': self.bg_h1_r.checkedId() == 1,
            'sarrafo_h2_esq': self.bg_h2_l.checkedId() == 1,
            'sarrafo_h2_dir': self.bg_h2_r.checkedId() == 1,
            'sarrafo_alt2_esq': self.bg_h2_l.checkedId() == 1,
            'sarrafo_alt2_dir': self.bg_h2_r.checkedId() == 1,
            ...
            'comandos': self.config.get('comandos', {}),
        }
        
        # --- Inject Partner Data for Section View ---
        try:
            if m.combined_faces:
                # Find partner in project_data logic
                # Ideally, look into self.project_data for the ID.
                # However, project_data structure is nested (Obra -> Pav -> vigas).
                # Shortcut if available: vigas dict in current context?
                # Using broad search helper or cache would be better, but let's try searching current pav first.
                
                partner_link = m.combined_faces[0] # Take first partner
                partner_id = partner_link.get('id')
                
                # Search in current pavement vigas
                found_partner = None
                p_vigas = {}
                
                # Tenta pegar vigas do pavimento atual
                if self.current_obra and self.current_pavimento:
                     p_vigas = self.project_data.get(self.current_obra, {}).get(self.current_pavimento, {}).get('vigas', {})
                
                # Se não achar, o código original _auto_combine usa apenas objetos em memória (vigas global no modulo main?)
                # Mas aqui estamos na classe MainApp. A variavel 'vigas' pode estar no escopo global do modulo, mas self.project_data é a fonte oficial.
                
                if partner_id:
                     # 1. Search in current pav
                     for k, v in p_vigas.items():
                         v_obj = v if isinstance(v, VigaState) else self._dict_to_state(v)
                         if v_obj.unique_id == partner_id:
                             found_partner = v_obj
                             break
                     
                     if found_partner:
                         # Convert to dict using same logic as collect_current_data?
                         # Or just passing basic dict
                         p_dict = self._vstate_to_script_dict(found_partner)
                         # Enrichment needed usually done in collect_current_data (like reading UI inputs)
                         # BUT the partner is NOT in the UI inputs right now (unless split view).
                         # We must relay on the SAVED state of the partner.
                         
                         data['partner_data'] = p_dict
                         print(f"Partner {found_partner.name} injected for Section View.")
        except Exception as e:
            print(f"Error injecting partner data: {e}")
        # --------------------------------------------
    ...
    def add_segment_class(self, suggested_name=None):
        ...
        if ok and name.strip():
            ...
            if clean_name not in pav_data['metadata']['segment_classes']:
                pav_data['metadata']['segment_classes'].append(clean_name)
                self.save_session_data()
                # Full refresh to ensure item is in the list
                self.update_classes_combo()
        
        # Robust Selection Strategy
        idx = self.cmb_classes.findText(clean_name)
        if idx == -1:
             # Should not happen if data saved correctly, but try forcing update again
             self.update_classes_combo()
             idx = self.cmb_classes.findText(clean_name)
        ...
        if idx >= 0:
            ...
            self.cmb_classes.setCurrentText(clean_name)
    ...
    def _update_heights_smart(self, *args):
        """
        Updates Panel Heights (H1, H2) based on Universal Inputs + Laje Inferior Logic.
        Rule: 
          if Laje Inferior > 0: H = Geral + 2
          else: H = Geral + 4
        """
        try:
            # 1. Get L.Inf value
            li_text = self.u_slab_b.text().strip().replace(',', '.')
            try:
                li_val = float(li_text)
            except:
                li_val = 0.0

            # 2. Determine Adder
            adder = 2.0 if li_val > 0 else 4.0

            # 3. Update H1 and Grade H1
            h1_text = self.u_h1.text().strip().replace(',', '.')
            if h1_text:
                try:
                    h1_base = float(h1_text)
                    
                    # Panel H1
                    h1_new = h1_base + adder
                    h1_str = f"{h1_new:.2f}".rstrip('0').rstrip('.')
                    for r in self.panel_rows:
                        r['h1'].setText(h1_str)
                        
                    # Grade H1 (Universal + Panels)
                    gh1_new = h1_base + 1.8
                    gh1_str = f"{gh1_new:.2f}".rstrip('0').rstrip('.')
                    self.u_gh1.setText(gh1_str)
                    for r in self.panel_rows:
                        r['g_h1'].setText(gh1_str)
                except:
                    pass 

            # 4. Update H2 and Grade H2
            h2_text = self.u_h2.text().strip().replace(',', '.')
            if h2_text:
                try:
                    h2_base = float(h2_text)
                    
                    # Panel H2
                    h2_new = h2_base # Removido soma de +2 ou +4 conforme solicitado
                    h2_str = f"{h2_new:.2f}".rstrip('0').rstrip('.')
                    for r in self.panel_rows:
                        r['h2'].setText(h2_str)
                        
                    # Grade H2 (Universal + Panels)
                    gh2_new = h2_base - 2.2
                    gh2_str = f"{gh2_new:.2f}".rstrip('0').rstrip('.')
                    self.u_gh2.setText(gh2_str)
                    for r in self.panel_rows:
                        r['g_h2'].setText(gh2_str)
                except:
                    pass
        except Exception as e:
            print(f"Error updating smart heights: {e}")

    def _propagate_li_and_update_heights(self, text):
        # 1. Propagate LI to all rows
        for r in self.panel_rows:
            r['li'].setText(text)
        # 2. Update Heights based on new LI
        self._update_heights_smart()
    ...
    def update_model(self, *args):
        ...
        try:
            ...
            self.model = VigaState(
                ...
                grade_height1_global=self.u_gh1.text(),
                grade_height2_global=self.u_gh2.text(),
                
                sarrafo_left=self.bg_h1_l.checkedId() == 1,
                sarrafo_right=self.bg_h1_r.checkedId() == 1,
                sarrafo_h2_left=self.bg_h2_l.checkedId() == 1,
                sarrafo_h2_right=self.bg_h2_r.checkedId() == 1,
                ...
                prod_force_h1=[self.rb_h1_0.isChecked(), self.rb_h1_1.isChecked(),
                               self.rb_h1_2.isChecked(), self.rb_h1_3.isChecked()],
            ...
            # Auto-refresh Class View if active
            if hasattr(self, 'tabs') and hasattr(self, 'tab_class'):
                if self.tabs.currentWidget() == self.tab_class:
    ...
    def action_next_beam(self):
        """Finaliza a viga atual e prepara a próxima viga solicitando o novo conjunto."""
        # 1. Salva viga atual
        self.update_model()
        self.save_current_fundo()
        
        # 2. Pergunta qual o próximo conjunto/classe
        current_cls = self.cmb_classes.currentText()
        new_cls, ok = QInputDialog.getText(self, "Próxima Viga", 
                                          "Nome do Conjunto/Classe para a próxima viga:", 
                                          text=current_cls)
        
        if not ok:
            return # Cancela se o usuário fechar o input sem confirmar
            
        if new_cls.strip():
            # Tenta adicionar/selecionar a classe informada
            final_cls = self.add_segment_class(suggested_name=new_cls.strip())
            # add_segment_class already handles selection, but we double check
            if final_cls:
                if self.cmb_classes.currentText() != final_cls:
                    self.cmb_classes.setCurrentText(final_cls)
        
        # 3. Prepara nova viga (Incrementa número e limpa campos)
        self.prepare_new()
        
        # FORCE UPDATE MODEL: Ensure the new VigaState captures the class currently selected in the Combo
        self.update_model()
        
        # 4. Inicia captura no AutoCAD
    ...
    def run_sequence_logic(self):
        """Core logic for automated capture sequence in CAD with ESC handling."""
        self.hide()
        self.showMinimized()
        ...
        try:
            ...
            # Step 2: Line (Width)
            self.display_floating("Clique em 2 PONTOS para a LARGURA (ESC cancela)")
            val = self.acad_service.select_line_length()
            if val > 0: 
                self.u_width.setText(f"{val:.2f}")
            else:
                raise CancelledSelectionError("Largura não capturada.")
            
            # Step 3: Levels
            if self.chk_prod_lvl.isChecked():
                 self.display_floating("Selecione NÍVEL LAJE e depois NÍVEL VIGA (ESC cancela)")
                 res = self.acad_service.select_levels()
                 # Nota: select_levels precisará ser atualizado para lançar exception se falhar criticalmente
            ...
            for i, (chk, label) in enumerate(chks):
                if chk.isChecked():
                    self.display_floating(f"Selecione dimensão (ex: 20x50) para: {label} (ESC cancela)")
            ...
            if self.chk_prod_pil_l.isChecked():
                self.display_floating("Selecione PILAR ESQUERDA (2 pontos) (ESC cancela)")
            ...
            if self.chk_prod_pil_r.isChecked():
                self.display_floating("Selecione PILAR DIREITA (2 pontos) (ESC cancela)")
                res = self.acad_service.select_pillar_dims()
                if res.get('dist') is not None: self.pd_d.setText(str(res['dist']))
                if res.get('width') is not None: self.pd_w.setText(str(res['width']))
            
            self.update_model()
            self.statusBar().showMessage("✅ Captura concluída com sucesso!", 5000)
            
        except CancelledSelectionError as e:
            self.statusBar().showMessage(f"⚠️ Seleção Cancelada: {e}", 5000)
            print(f"User Cancelled Sequence: {e}")
        except Exception as e:
            self.statusBar().showMessage(f"❌ Erro na Sequência: {e}", 5000)
            print(f"Sequence Error: {e}")
            
        finally:
            if hasattr(self, '_floating'):
                self._floating.hide()
                try: self._floating.close()
                except: pass
    ...
    def populate_ui_from_state(self, state: VigaState):
        ...
        try:
            ...
            self.u_gh1.setText(self._clean_str(d.get('grade_height1_global', '7.0')))
            self.u_gh2.setText(self._clean_str(d.get('grade_height2_global', '7.0')))
            ...
            if len(prod_force_h1) == 4:
                self.rb_h1_0.setChecked(prod_force_h1[0]); self.rb_h2_0.setChecked(not prod_force_h1[0])
                self.rb_h1_1.setChecked(prod_force_h1[1]); self.rb_h2_1.setChecked(not prod_force_h1[1])
                self.rb_h1_2.setChecked(prod_force_h1[2]); self.rb_h2_2.setChecked(not prod_force_h1[2])
                self.rb_h1_3.setChecked(prod_force_h1[3]); self.rb_h2_3.setChecked(not prod_force_h1[3])
            ...
            # Block signals to prevent overwriting individual panel data
            self.u_h1.blockSignals(True); self.u_h2.blockSignals(True); self.u_width.blockSignals(True)
            ...
            self.u_h1.blockSignals(False); self.u_h2.blockSignals(False); self.u_width.blockSignals(False)
            ...
            # Set Universal Slab Fields based on first panel (to avoid empty/stale comparison)
            if panels:
                p0 = panels[0]
                self.u_slab_t.blockSignals(True); self.u_slab_b.blockSignals(True); self.u_slab_c.blockSignals(True)
                val_t = p0.get('slab_top', '') if isinstance(p0, dict) else p0.slab_top
                val_b = p0.get('slab_bottom', '') if isinstance(p0, dict) else p0.slab_bottom
                val_c = p0.get('slab_center', '') if isinstance(p0, dict) else p0.slab_center
                
                self.u_slab_t.setText(self._clean_str(val_t))
                self.u_slab_b.setText(self._clean_str(val_b))
                self.u_slab_c.setText(self._clean_str(val_c))
                self.u_slab_t.blockSignals(False); self.u_slab_b.blockSignals(False); self.u_slab_c.blockSignals(False)
            ...
            # Sarrafos
            # Sarrafos
            # Helper to set correct button
            def set_bg(bg, new_id, old_val, default=True):
                if new_id is not None:
                     b = bg.button(new_id)
                     if b: b.setChecked(True)
                else:
                    # Legacy fallback
                    val = d.get(old_val, default)
                    # Maps Bool -> 1 (Ext) / 0 (None)
                    bid = 1 if val else 0
                    b = bg.button(bid)
                    if b: b.setChecked(True)

            set_bg(self.bg_h1_l, d.get('sarrafo_left_id'), 'sarrafo_left', True)
            set_bg(self.bg_h1_r, d.get('sarrafo_right_id'), 'sarrafo_right', True)
            set_bg(self.bg_h2_l, d.get('sarrafo_h2_left_id'), 'sarrafo_h2_left', False)
            set_bg(self.bg_h2_r, d.get('sarrafo_h2_right_id'), 'sarrafo_h2_right', False)
    ...
    def _generate_bulk_scripts(self, vigas_dict, folder_name, question_text):
        ...
        # Controle para gerar Visao de Corte Separada
        processed_pairs = set()
        vc_counter = 1
        ...
        for name, vstate_obj in v_items:
            ...
            if vstate.combined_faces:
                 pid = vstate.combined_faces[0].get('id')
                 # Search in the same dictionary 'vigas_dict' first
                 partner_obj = None
                 
                 # Optimization: Look up in local vigas_dict by ID
                 for k2, v2 in vigas_dict.items():
                     v2_obj = v2 if isinstance(v2, VigaState) else self._dict_to_state(v2)
                     if v2_obj.unique_id == pid:
                         partner_obj = v2_obj
                         break
                 
                 if partner_obj:
                     data['partner_data'] = self._vstate_to_script_dict(partner_obj)
                     print(f"Bulk Gen: Injected partner {partner_obj.name} into {vstate.name}")
            # ----------------------------------
            ...
            # --- GERAÇÃO SEPARADA VISÃO DE CORTE ---
            if 'partner_data' in data and data['partner_data']:
                try:
                    p1 = data
                    p2 = data['partner_data']
                    n1 = p1.get('nome', '')
                    n2 = p2.get('nome', '')
                    
                    # Determinar quem é A (esq) e B (dir) para passar na ordem correta
                    dA, dB = None, None
                    l1 = p1.get('lado', '')
                    if l1 == 'A' or '.A' in n1 or 'LADO A' in n1.upper():
                        dA, dB = p1, p2
                    else:
                        dA, dB = p2, p1

                    if dA and dB:
                        # Criar Assinatura Geométrica para evitar duplicados iguais no mesmo conjunto
                        def get_h_val(d, keys):
                            for k in keys:
                                val = d.get(k)
                        ...
                        def get_arr0(d, key):
                            arr = d.get(key, [])
                            return float(arr[0]) if (arr and len(arr) > 0) else 0.0

                        # Assinatura: (Classe, H1_A, H2_A, H1_B, H2_B, LS_A, LI_A, LC_A, LS_B, LI_B, LC_B, Fundo_A, Fundo_B, Tipo_A, Tipo_B)
                        v_class = getattr(vstate, 'segment_class', 'Lista Geral')
                        sig = (
                            v_class,
                            get_h_val(dA, ['paineis_alturas', 'altura_geral']),
                            get_h_val(dA, ['paineis_alturas2', 'altura_2_geral']),
                            get_h_val(dB, ['paineis_alturas', 'altura_geral']),
                            get_h_val(dB, ['paineis_alturas2', 'altura_2_geral']),
                            get_arr0(dA, 'lajes_sup'), get_arr0(dA, 'lajes_inf'), get_arr0(dA, 'lajes_central_alt'),
                            get_arr0(dB, 'lajes_sup'), get_arr0(dB, 'lajes_inf'), get_arr0(dB, 'lajes_central_alt'),
                            float(dA.get('fundo_viga', 0)), float(dB.get('fundo_viga', 0)),
                            str(dA.get('paineis_tipo1', [''])[0]), str(dB.get('paineis_tipo1', [''])[0])
                        )

                        if sig not in processed_pairs:
                            processed_pairs.add(sig)
                            
                            # Gera o arquivo da Visão de Corte
                            vc_path = gerador_inst.gerar_script_visao_corte_arquivo(dA, dB, out_dir, vc_counter)
                            if vc_path:
                                vc_counter += 1
    ...
    def _vstate_to_script_dict(self, m):
        ...
        data = {
            ...
            'sarrafo_h2_esq': m.sarrafo_h2_left,
            'sarrafo_h2_dir': m.sarrafo_h2_right,
            # IDs Injection (Safe access for legacy objects)
            'sarrafo_left_id': getattr(m, 'sarrafo_left_id', 1 if m.sarrafo_left else 0),
            'sarrafo_right_id': getattr(m, 'sarrafo_right_id', 1 if m.sarrafo_right else 0),
            'sarrafo_h2_left_id': getattr(m, 'sarrafo_h2_left_id', 1 if m.sarrafo_h2_left else 0),
            'sarrafo_h2_right_id': getattr(m, 'sarrafo_h2_right_id', 1 if m.sarrafo_h2_right else 0),
            # Legacy/Generator mappings
            'sarrafo_left': getattr(m, 'sarrafo_left_id', 1 if m.sarrafo_left else 0) == 1,
            'sarrafo_right': getattr(m, 'sarrafo_right_id', 1 if m.sarrafo_right else 0) == 1,
            'sarrafo_alt2_esq': getattr(m, 'sarrafo_h2_left_id', 1 if m.sarrafo_h2_left else 0) == 1,
            'sarrafo_alt2_dir': getattr(m, 'sarrafo_h2_right_id', 1 if m.sarrafo_h2_right else 0) == 1,
            'lajes_sup': [p.slab_top for p in m.panels],
            'lajes_inf': [p.slab_bottom for p in m.panels],
            'lajes_central_alt': [p.slab_center for p in m.panels],
    ...
    def _select_names_logic(self):
        ...
        def get_user_input(prompt_text):
            """Solicita via AutoCAD com respiro rápido entre passos."""
            import time
            time.sleep(0.3) # Reduzido de 2.0s para 0.3s
            
            fl = FloatingLabel(prompt_text)
            fl.show_msg(prompt_text)
            QApplication.processEvents()
            
            # Dinamicamente espera o CAD ficar pronto se necessário
            doc = self.acad_service._get_doc()
            if doc: self.acad_service._wait_for_cad_idle(doc, timeout=0.5)
        ...
        val_slab_raw = parse_max_number(txt_slab)
        val_slab_ui = val_slab_raw + 2.0 if val_slab_raw > 0 else 0.0
        
        # Inicialmente preenche L.Sup com valor +2
        self.u_slab_t.setText(f"{val_slab_ui:.2f}")
        ...
        # Use RAW slab value for H1 calculation to preserve total height consistency
        final_h1 = val_beam_total - val_slab_raw - val_top
    ...
    def action_select_pillar(self):
        # Solicitado: Que o botão select pilar ao invez de usar os checkbox pergunte se é pilar esquerda ou direita
        items = ["Pilar Esquerda", "Pilar Direita"]
        side, ok = QInputDialog.getItem(self, "Selecionar Pilar", "Qual pilar deseja capturar?", items, 0, False)
        ...
        try:
            ...
            if "CancelledSelectionError" not in str(type(e)):
    ...
    def action_select_opening(self):
        # Diálogo customizado para selecionar Lado e Tipo (H1/H2)
        dialog = QDialog(self)
        dialog.setWindowTitle("Selecionar Abertura")
        dlg_layout = QVBoxLayout(dialog)
        
        # ComboBox de Lado
        items = ["Topo/Esq", "Fundo/Esq", "Topo/Dir", "Fundo/Dir"]
        cmb_side = QComboBox()
        cmb_side.addItems(items)
        dlg_layout.addWidget(QLabel("Qual abertura preencher?"))
        dlg_layout.addWidget(cmb_side)
        
        # RadioButtons de Altura
        radio_grp = QGroupBox("Tipo de Altura")
        radio_lay = QHBoxLayout(radio_grp)
        rb_h1 = QRadioButton("H1"); rb_h1.setChecked(True)
        rb_h2 = QRadioButton("H2")
        radio_lay.addWidget(rb_h1); radio_lay.addWidget(rb_h2)
        dlg_layout.addWidget(radio_grp)
        ...
        if dialog.exec() == QDialog.Accepted:
            item = cmb_side.currentText()
            idx = items.index(item)
            is_h1 = rb_h1.isChecked()
            
            self.showMinimized()
            try:
                self._select_names_logic_notify("Selecione o Texto da Abertura (ex: 80/60 ou 40x55)")
                ...
                if val1 > 0 or val2 > 0:
                    # Lógica solicitada: 
                    # Distancia = 0 padrao
                    # Profundidade = Maior numero
                    # Largura = Menor numero
                    prof = max(val1, val2)
                    larg = min(val1, val2)
                    dist = 0.0
                    
                    # Preencher campos na aba Detalhes
                    # self.hole_rows[idx] is (dist_widget, p_widget, w_widget, chk_forcar)
                    self.hole_rows[idx][0].setText(f"{dist:.2f}")
                    self.hole_rows[idx][1].setText(f"{prof:.2f}")
                    self.hole_rows[idx][2].setText(f"{larg:.2f}")
                    
                    # Se for H1: marcar checkbox "Forçar Altura H1"
                    # Se for H2: desmarcar
                    self.hole_rows[idx][3].setChecked(is_h1)
                    
                    # Solicitado: Após preencher abertura, marcar opção de continuação
                    self.rb_cont_cont.setChecked(True)
 ...
 # simple merge, not perfect additive but gives idea
    ...
    def reset_fundo(self, new_id=False):
        self._updating = True
        
        # If new_id requested, we start with a fresh state to ensure clear ID
        if new_id and hasattr(self, 'model'):
             self.model.unique_id = str(uuid.uuid4())
             self.model.combined_faces = [] # Clear links on new beam
             # Also clear reused uids in panels
             for p in self.model.panels:
                 p.uid = ""
                 p.reused_from = ""; p.reused_in = ""
                 p.reused_from_h2 = ""; p.reused_in_h2 = ""
                 p.link_saved = False
    ...
    def on_pav_changed(self, text):
        ...
        self.lbl_pav_tag.set_text(text)
        
        # Save previous selection if needed? (Already saved on focus loss/change usually)
        
        # Fix Duplicates on Load
        self.check_and_fix_duplicate_ids()
        
        self.update_vigas_list()
        self.update_classes_combo()
        
        # Load first if any (Optional - maybe better to clear)
        # self.reset_fundo() # Don't reset, just let user pick or stay empty
        
        # Or load the last edited in this pavement?
        # For now, just clear list selection
        self.tree1.clearSelection()
        
    def check_and_fix_duplicate_ids(self):
        """Scans current pavement for beams with same unique_id and fixes them."""
        if not self.current_obra or not self.current_pavimento: return
        
        pav_data = self.project_data.get(self.current_obra, {}).get(self.current_pavimento, {})
        vigas = pav_data.get('vigas', {})
        
        seen_ids = {} # {uid: [list_of_keys]}
        
        # 1. Map IDs
        for k, v in vigas.items():
            raw_uid = v.get('unique_id') if isinstance(v, dict) else v.unique_id
            if not raw_uid: continue
            
            uid = str(raw_uid).strip() # Force string for reliable comparison
            
            if uid not in seen_ids: seen_ids[uid] = []
            seen_ids[uid].append(k)
            
        # 2. Fix Duplicates
        fixed_count = 0
        for uid, keys in seen_ids.items():
            if len(keys) > 1:
                print(f"DEBUG_FIX: Duplicate ID {uid} found in keys: {keys}")
                ...
                for i in range(1, len(keys)):
                    ...
                    if isinstance(v_fix, dict):
                        v_fix['unique_id'] = new_uid
                        v_fix['combined_faces'] = [] # Clear broken links
                    else:
                        v_fix.unique_id = new_uid
                        v_fix.combined_faces = []
                        
                    print(f"DEBUG_FIX: Regenerated ID for '{k_fix}' -> {new_uid}")
                    fixed_count += 1
                    
        if fixed_count > 0:
            QMessageBox.information(self, "Correção Automática", f"Foram detectadas e corrigidas {fixed_count} vigas com IDs duplicados neste pavimento.")
            self.save_session_data()
    ...
    def _dict_to_state(self, vdict):
        ...
        if 'panels' in vdict:
            ...
            for p_data in vdict.get('panels', []):
                ...
                params = {}
                numeric_fields = ['width', 'grade_h1', 'grade_h2', 'slab_top', 'slab_bottom', 'slab_center']
                string_numeric_fields = ['height1', 'height2'] # Can be "0.0" or 0.0
                ...
                for k, v in p_data.items():
                    ...
                    elif k in string_numeric_fields:
                        params[k] = str(v)
    ...
    def on_pav_changed(self, text):
        self.current_pavimento = text
        if hasattr(self, 'lbl_pav_tag'):
            self.lbl_pav_tag.setText(text)
    ...
    def copy_pavimento(self):
        ...
        for vname, vstate in vigas.items():
            ...
            if isinstance(vstate, dict):
                ...
                # FIX: Regenerate ID and Clear Links to prevents collisions with source pavement
                vstate['unique_id'] = str(uuid.uuid4())
                vstate['combined_faces'] = []
                ...
                for p_dict in p_list:
                 ...
                 vstate.floor = new_name # Update floor link
                 # FIX: Regenerate ID and Clear Links
                 vstate.unique_id = str(uuid.uuid4())
                 vstate.combined_faces = []
    ...
    def update_vigas_list(self):
        ...
        for cls_name in sorted_classes:
            ...
            if col == 0: # Coluna Nº
                v_list.sort(key=lambda x: natural_key(getattr(x[1], 'number', '') if not isinstance(x[1], dict) else x[1].get('number', '')), reverse=reverse)
            elif col == 1: # Coluna Nome
                v_list.sort(key=lambda x: natural_key(getattr(x[1], 'name', '') if not isinstance(x[1], dict) else x[1].get('name', '')), reverse=reverse)
            elif col == 2: # Coluna Pav
                v_list.sort(key=lambda x: natural_key(x[2]), reverse=reverse)
            else:
    ...
    def on_viga_header_clicked(self, logical_index):
        """Handler para clique no cabeçalho da lista de vigas."""
        if self.list_sort_column == logical_index:
            # Alterna a ordem se clicar na mesma coluna
            self.list_sort_order = Qt.DescendingOrder if self.list_sort_order == Qt.AscendingOrder else Qt.AscendingOrder
        else:
            # Nova coluna, sempre começa Ascending
            self.list_sort_column = logical_index
            self.list_sort_order = Qt.AscendingOrder
    ...
    def action_uncombine_faces(self):
        ...
        # Ensure integrity BEFORE uncombining
        self.check_and_fix_duplicate_ids()
        ...
        for item in sel:
            ...
            if data and data.get('type') == 'beam':
                ...
                print(f"DEBUG_UNCOMBINE: Unlinking beam '{key}' (ID: {v.unique_id}) from partners: {partner_ids}")
    ...
    def _unlink_partner_globally(self, partner_id, my_id):
        ...
        print(f"DEBUG_UNLINK: Searching for PartnerID={partner_id} to remove MyID={my_id}")
        ...
        for pav_name, pav_data in obra_data.items():
            ...
            for v_name, vstate in vigas.items():
                v_uid = vstate.get('unique_id') if isinstance(vstate, dict) else vstate.unique_id
                
                if str(v_uid).strip() == str(partner_id).strip():
                    print(f"DEBUG_UNLINK: Partner Found! Name={v_name}, Pav={pav_name}")
                    found_any = True
                    
                    if isinstance(vstate, dict):
                         faces = vstate.get('combined_faces', [])
                         original_len = len(faces)
                         vstate['combined_faces'] = [f for f in faces if str(f.get('id')).strip() != str(my_id).strip()]
                         print(f"DEBUG_UNLINK: Removed link. Len {original_len} -> {len(vstate['combined_faces'])}")
                    else:
                         faces = getattr(vstate, 'combined_faces', [])
                         original_len = len(faces)
                         vstate.combined_faces = [f for f in faces if str(f.get('id')).strip() != str(my_id).strip()]
                         print(f"DEBUG_UNLINK: Removed link on Object. Len {original_len} -> {len(vstate.combined_faces)}")
                    # Do not return here - continue searching in case of duplicates!
        
        if not found_any:
            print("DEBUG_UNLINK: Partner NOT FOUND globally.")

    def _get_beam_side(self, beam):
        """Robustly detects Side A or B from name suffix (text after last dot)."""
        name = str(beam.name)
        if '.' in name:
            suffix = name.split('.')[-1].upper()
            # Precisely check if starting with A or B
            if suffix.startswith('A'): return 'A'
            if suffix.startswith('B'): return 'B'
        
        # Fallback to attribute
        side = str(getattr(beam, 'side', 'A')).upper()
        return 'A' if 'A' in side else 'B'

    def action_auto_combine_set(self):
        """Combines beams in the current SELECTION as a single set (ignores specific Number/Class if manually selected)."""
        if not self.current_pavimento or not self.current_obra:
        ...
        if selected_items:
             # User selected specific beams -> Combine THEM regardless of Number/Class
             for item in selected_items:
                 data = item.data(0, Qt.UserRole)
                 if not data or not isinstance(data, dict) or data.get('type') != 'beam':
                     continue
                 ...
                 v_key = data.get('key')
                 vstate = vigas.get(v_key)
                 if vstate:
                     v_obj = vstate if not isinstance(vstate, dict) else self._dict_to_state(vstate)
                     vigas[v_key] = v_obj
                     target_beams.append(v_obj)
                     
             if target_beams:
                 print(f"DEBUG_AUTO_COMBINE_SET: Selected {len(target_beams)} beams for combining.")
                 ops = self._auto_combine_algorithm(target_beams)
                 if ops > 0:
                    QMessageBox.information(self, "Sucesso", f"Combinação automática por SELEÇÃO realizada ({ops} vínculos).")
                    self.save_session_data(); self.update_vigas_list()
                    return
                 else:
                    QMessageBox.information(self, "Info", "Nenhuma combinação possível para os itens selecionados.")
                    return
                
        # Fallback: Current Beam's Class Group
        if self.model:
            current_class = self.model.segment_class
            all_in_class = []
            for k, v in vigas.items():
                obj = v if not isinstance(v, dict) else self._dict_to_state(v)
                if obj.segment_class == current_class:
                    vigas[k] = obj
                    all_in_class.append(obj)
            
            print(f"DEBUG_AUTO_COMBINE_SET: Processing Group '{current_class}' with {len(all_in_class)} beams.")
            ops = self._auto_combine_algorithm(all_in_class)
            if ops > 0:
                QMessageBox.information(self, "Sucesso", f"Combinação automática do GRUPO '{current_class}' realizada ({ops} vínculos).")
                self.save_session_data(); self.update_vigas_list()
            else:
                QMessageBox.information(self, "Info", "Nenhuma combinação nova possível para o grupo atual.")
        else:
            QMessageBox.warning(self, "Aviso", "Selecione vigas na lista ou abra uma viga para usar esta função.")

    def action_auto_combine_pav(self):
        """Combines ALL sets in the current pavement automatically (Grouped strictly by Class)."""
        if not self.current_pavimento or not self.current_obra:
             return
        ...
        # Group by CLASS ONLY
        sets_map = {} # (class) -> [beams]
        
        for k, v in vigas.items():
            obj = v if not isinstance(v, dict) else self._dict_to_state(v)
            vigas[k] = obj
            cls = obj.segment_class
            if cls not in sets_map: sets_map[cls] = []
            sets_map[cls].append(obj)
            
        count_operations = 0
        for cls, subset in sets_map.items():
            print(f"DEBUG_AUTO_COMBINE_PAV: Processing Class '{cls}' with {len(subset)} beams.")
            ops = self._auto_combine_algorithm(subset)
            count_operations += ops
            
        if count_operations > 0:
            QMessageBox.information(self, "Sucesso", f"Combinação automática do pavimento realizada ({count_operations} vínculos).")
            self.save_session_data()
            self.update_vigas_list()
        else:
             QMessageBox.information(self, "Info", "Nenhuma combinação necessária no pavimento (Verifique se há lados A e B em cada classe).")

    def _auto_combine_algorithm(self, beam_list):
        """Implements A-Asc (Suffix) vs B-Desc (Suffix) pairing logic with clean state."""
        # 1. Clear ALL existing combination links for these beams first to maintain integrity
        for b in beam_list:
            # We must be careful to not only clear them but also unlink them from whoever they were linked to
            # But for "Automatic Combine", the user usually wants a full reset of the set's links.
            # Unlinking globally is slow, so we at least clear the local list.
            b.combined_faces = []

        side_a = [b for b in beam_list if self._get_beam_side(b) == 'A']
        side_b = [b for b in beam_list if self._get_beam_side(b) == 'B']
        
        if not side_a or not side_b:
            print(f"DEBUG_AUTO_COMBINE: Incomplete sides. A={len(side_a)}, B={len(side_b)} in list of {len(beam_list)} beams.")
            return 0
            
        def get_suffix(beam):
             if '.' in beam.name: return beam.name.split('.')[-1]
             return beam.name

        # Sorting
        side_a.sort(key=get_suffix) # Aa, Ab, Ac...
        side_b.sort(key=get_suffix, reverse=True) # Bc, Bb, Ba...
        
        print(f"DEBUG_AUTO_COMBINE: Sides Balanced. A:{[b.name for b in side_a]} | B:{[b.name for b in side_b]}")
        ...
        for i in range(max_len):
            # One-to-one pairing with reuse of the last element if sizes differ
            idx_a = i if i < len_a else len_a - 1
            idx_b = i if i < len_b else len_b - 1
            
            beam_a = side_a[idx_a]
            beam_b = side_b[idx_b]
            
            # Prevent double linking the same pair if we are reusing last element
            already_pair = False
            for l in beam_a.combined_faces:
                if str(l.get('id')) == str(beam_b.unique_id):
                    already_pair = True
                    break
            
            if not already_pair:
                beam_a.combined_faces.append({'name': beam_b.name, 'id': beam_b.unique_id})
                beam_b.combined_faces.append({'name': beam_a.name, 'id': beam_a.unique_id})
                ops += 1
                print(f"DEBUG_AUTO_COMBINE: Linked {beam_a.name} <-> {beam_b.name}")
