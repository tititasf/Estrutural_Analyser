...
# Referência dinâmica: X da parede esquerda do PAI.A (posição final real no script)
_posicao_parede_esquerda_pai_a = None

def definir_posicao_parede_esquerda_pai_a(posicao):
    """Define a posição X final da parede esquerda do PAI.A para uso como base das grades especiais (Grade E)."""
    global _posicao_parede_esquerda_pai_a
    _posicao_parede_esquerda_pai_a = posicao

def obter_posicao_parede_esquerda_pai_a():
    """Obtém a posição X final da parede esquerda do PAI.A previamente calculada."""
    global _posicao_parede_esquerda_pai_a
    return _posicao_parede_esquerda_pai_a
    ...
    try:
        ...
        if (gerador._globais_pilar_especial.get('tipo_pilar') == 'L' and
            ...
            if (pilar_num == 2 and componente == 'pai_a' and tipo_alteracao == 'tamanho'):
                # Ajuste baseado na largura do pilar 1
                ajuste_largura = (largura_pilar_1 - 20.0) * 1  # Direto: se larg_1=30, ajuste=+10
                valor_ajustado = valor_original + ajuste_largura
                ...
                # NOVO: Ajuste adicional baseado no comprimento do pilar especial 1 vs 100cm fixo
                comprimento_pilar_1 = float(gerador._globais_pilar_especial.get('comp_1', 0) or 0)
                diferenca_100 = comprimento_pilar_1 - 100.0
                
                if abs(diferenca_100) > 1e-6:  # Se diferente de 100cm
                    # PAI.A: diminui do tamanho da extremidade ESQUERDA do retângulo
                    ajuste_comp_1 = -diferenca_100
                    valor_ajustado = valor_ajustado + ajuste_comp_1  # Aplicar sobre o valor já ajustado pela largura
                    
                    print(f"[AJUSTE-COMP-1] PAI.A Pilar 2: comp_1={comprimento_pilar_1}cm, diferença={diferenca_100:+.1f}cm")
                    print(f"[AJUSTE-COMP-1] Ajuste extremidade esquerda: {ajuste_comp_1:+.1f}cm")
                
                # NOVO: Ajuste adicional baseado no comprimento do pilar especial 2 vs 100cm fixo
                comprimento_pilar_2 = float(gerador._globais_pilar_especial.get('comp_2', 0) or 0)
                diferenca_100_2 = comprimento_pilar_2 - 100.0
                
                if abs(diferenca_100_2) > 1e-6:  # Se diferente de 100cm
                    # PAI.A: cresce do tamanho baseado na diferença do comprimento 2
                    ajuste_comp_2 = diferenca_100_2
                    valor_ajustado = valor_ajustado + ajuste_comp_2  # Aplicar sobre o valor já ajustado
                    
                    print(f"[AJUSTE-COMP-2] PAI.A Pilar 2: comp_2={comprimento_pilar_2}cm, diferença={diferenca_100_2:+.1f}cm")
                    print(f"[AJUSTE-COMP-2] Ajuste crescimento: {ajuste_comp_2:+.1f}cm")
                    print(f"[AJUSTE-COMP-2] Tamanho final: {valor_original} -> {valor_ajustado}")
                
                if robo_logger:
                    robo_logger.info(f"🔧 AJUSTE LARGURA - PAI.A Pilar 2: larg_1={largura_pilar_1}cm, ajuste={ajuste_largura:+.1f}cm")
                    robo_logger.info(f"   Tamanho: {valor_original} -> {valor_ajustado}")
                    if abs(diferenca_100) > 1e-6:
                        robo_logger.info(f"🔧 AJUSTE COMP-1 - PAI.A Pilar 2: comp_1={comprimento_pilar_1}cm, diferença={diferenca_100:+.1f}cm")
                        robo_logger.info(f"   Ajuste extremidade esquerda: {ajuste_comp_1:+.1f}cm")
                    if abs(diferenca_100_2) > 1e-6:
                        robo_logger.info(f"🔧 AJUSTE COMP-2 - PAI.A Pilar 2: comp_2={comprimento_pilar_2}cm, diferença={diferenca_100_2:+.1f}cm")
                        robo_logger.info(f"   Ajuste crescimento: {ajuste_comp_2:+.1f}cm")
            ...
            # 6. PAI.B PILAR 2: ajuste de tamanho baseado nos comprimentos e, se especial, na largura 2
            if (pilar_num == 2 and componente == 'pai_b' and tipo_alteracao == 'tamanho'):
                # Apenas ajuste baseado no comprimento do pilar especial 1 vs 100cm fixo
                comprimento_pilar_1 = float(gerador._globais_pilar_especial.get('comp_1', 0) or 0)
                diferenca_100 = comprimento_pilar_1 - 100.0
                
                if abs(diferenca_100) > 1e-6:  # Se diferente de 100cm
                    # PAI.B: diminui do tamanho da extremidade ESQUERDA do retângulo
                    # Para afetar a extremidade esquerda, aplicamos o ajuste diretamente no valor original
                    ajuste_comp_1 = -diferenca_100
                    valor_ajustado = valor_original + ajuste_comp_1  # Aplicar diretamente no valor original
                    
                    print(f"[AJUSTE-COMP-1] PAI.B Pilar 2: comp_1={comprimento_pilar_1}cm, diferença={diferenca_100:+.1f}cm")
                    print(f"[AJUSTE-COMP-1] Ajuste extremidade esquerda: {ajuste_comp_1:+.1f}cm")
                else:
                ...
                if abs(diferenca_100_2) > 1e-6:  # Se diferente de 100cm
                    # PAI.B: cresce do tamanho baseado na diferença do comprimento 2
                    ajuste_comp_2 = diferenca_100_2
                    valor_ajustado = valor_ajustado + ajuste_comp_2  # Aplicar sobre o valor já ajustado
                    
                    print(f"[AJUSTE-COMP-2] PAI.B Pilar 2: comp_2={comprimento_pilar_2}cm, diferença={diferenca_100_2:+.1f}cm")
                    print(f"[AJUSTE-COMP-2] Ajuste crescimento: {ajuste_comp_2:+.1f}cm")
                    print(f"[AJUSTE-COMP-2] Tamanho final: {valor_original} -> {valor_ajustado}")

                # NOVO: Ajuste adicional SOMENTE PARA PILAR 2 ESPECIAL baseado na largura 2 (20cm como base)
                if (
                    gerador._globais_pilar_especial.get('tipo_pilar') == 'L' and
                    getattr(gerador, 'pilar_especial_ativo_var', None) and
                    gerador.pilar_especial_ativo_var.get()
                ):
                    largura_pilar_2 = float(gerador._globais_pilar_especial.get('larg_2', 20) or 20)
                    diferenca_larg2 = largura_pilar_2 - 20.0
                    if abs(diferenca_larg2) > 1e-6:
                        valor_ajustado = valor_ajustado + diferenca_larg2
                        print(f"[AJUSTE-LARG-2] PAI.B Pilar 2: larg_2={largura_pilar_2}cm, diferença={diferenca_larg2:+.1f}cm")
                        print(f"[AJUSTE-LARG-2] Tamanho (±dif larg_2): {valor_ajustado - diferenca_larg2} -> {valor_ajustado}")
                
                if robo_logger:
                    if abs(diferenca_100) > 1e-6:
                        robo_logger.info(f"🔧 AJUSTE COMP-1 - PAI.B Pilar 2: comp_1={comprimento_pilar_1}cm, diferença={diferenca_100:+.1f}cm")
                        robo_logger.info(f"   Ajuste extremidade esquerda: {ajuste_comp_1:+.1f}cm")
                    else:
                        robo_logger.info(f"🔧 AJUSTE COMP-1 - PAI.B Pilar 2: Sem ajuste (comp_1={comprimento_pilar_1}cm = 100cm)")
                    if abs(diferenca_100_2) > 1e-6:
                        robo_logger.info(f"🔧 AJUSTE COMP-2 - PAI.B Pilar 2: comp_2={comprimento_pilar_2}cm, diferença={diferenca_100_2:+.1f}cm")
                        robo_logger.info(f"   Ajuste crescimento: {ajuste_comp_2:+.1f}cm")
                    if (
                        gerador._globais_pilar_especial.get('tipo_pilar') == 'L' and
                        getattr(gerador, 'pilar_especial_ativo_var', None) and
                        ...
                        if abs(diferenca_larg2) > 1e-6:
                            robo_logger.info(f"🔧 AJUSTE LARG-2 - PAI.B Pilar 2: larg_2={largura_pilar_2}cm, diferença={diferenca_larg2:+.1f}cm")
                            robo_logger.info(f"   Tamanho (±dif larg_2): {valor_ajustado - diferenca_larg2} -> {valor_ajustado}")
                
                return valor_ajustado
            
            # 7. PAI.B PILAR 2: ajuste de posição baseado na largura do pilar 2 + comprimento do pilar 1
            if (pilar_num == 2 and componente == 'pai_b' and tipo_alteracao == 'posicao'):
                # Ajuste baseado na largura 2 do pilar 2
                ajuste_largura_2 = (largura_pilar_2 - 20.0) * -1  # Inverso: se larg_2=30, ajuste=-10
                valor_ajustado = valor_original + ajuste_largura_2
                
                print(f"[AJUSTE-LARGURA] PAI.B Pilar 2 POSIÇÃO (LARGURA 2):")
                print(f"   Largura 2: {largura_pilar_2}cm, ajuste: {ajuste_largura_2:+.1f}cm")
                print(f"   Posição: {valor_original} -> {valor_ajustado}")
                
                # NOVO: Ajuste adicional de posição baseado no comprimento do pilar especial 1 vs 100cm fixo
                comprimento_pilar_1 = float(gerador._globais_pilar_especial.get('comp_1', 0) or 0)
                diferenca_100 = comprimento_pilar_1 - 100.0
                
                if abs(diferenca_100) > 1e-6:  # Se diferente de 100cm
                    # PAI.B: move para a direita (posição) 1x a diferença para compensar
                    ajuste_posicao_comp_1 = diferenca_100 * 1.0  # 1x a diferença
                    valor_ajustado = valor_ajustado + ajuste_posicao_comp_1  # Aplicar sobre o valor já ajustado pela largura
                    
                    print(f"[AJUSTE-COMP-1] PAI.B Pilar 2 POSIÇÃO: comp_1={comprimento_pilar_1}cm, diferença={diferenca_100:+.1f}cm")
                    print(f"[AJUSTE-COMP-1] Ajuste posição (1x diferença): {ajuste_posicao_comp_1:+.1f}cm")
                
                # NOVO: Ajuste adicional de posição baseado no comprimento do pilar especial 2 vs 100cm fixo
                comprimento_pilar_2 = float(gerador._globais_pilar_especial.get('comp_2', 0) or 0)
                diferenca_100_2 = comprimento_pilar_2 - 100.0
                
                if abs(diferenca_100_2) > 1e-6:  # Se diferente de 100cm
                    # PAI.B: move para a esquerda (posição) 1x a diferença do comprimento 2
                    ajuste_posicao_comp_2 = -diferenca_100_2  # Negativo para mover para a esquerda
                    valor_ajustado = valor_ajustado + ajuste_posicao_comp_2  # Aplicar sobre o valor já ajustado
                    
                    print(f"[AJUSTE-COMP-2] PAI.B Pilar 2 POSIÇÃO: comp_2={comprimento_pilar_2}cm, diferença={diferenca_100_2:+.1f}cm")
                    print(f"[AJUSTE-COMP-2] Ajuste posição esquerda (1x diferença): {ajuste_posicao_comp_2:+.1f}cm")
                    print(f"[AJUSTE-COMP-2] Posição final: {valor_original} -> {valor_ajustado}")
                
                if robo_logger:
                    robo_logger.info(f"🔧 AJUSTE POSIÇÃO - PAI.B Pilar 2: Apenas comprimentos dos pilares especiais")
                    robo_logger.info(f"   Posição base: {valor_original}")
                    if abs(diferenca_100) > 1e-6:
                        robo_logger.info(f"🔧 AJUSTE COMP-1 - PAI.B Pilar 2 POSIÇÃO: comp_1={comprimento_pilar_1}cm, diferença={diferenca_100:+.1f}cm")
                        robo_logger.info(f"   Ajuste posição (1x diferença): {ajuste_posicao_comp_1:+.1f}cm")
                    if abs(diferenca_100_2) > 1e-6:
                        robo_logger.info(f"🔧 AJUSTE COMP-2 - PAI.B Pilar 2 POSIÇÃO: comp_2={comprimento_pilar_2}cm, diferença={diferenca_100_2:+.1f}cm")
                        robo_logger.info(f"   Ajuste posição esquerda (1x diferença): {ajuste_posicao_comp_2:+.1f}cm")
...
class GeradorPilar:
    ...
    def criar_janela_configuracoes(self):
        ...
        self.config_window.geometry("600x660")  # Altura aumentada em 50cm (500px)
    ...
    def _obter_parafusos_especiais_para_uso(self, letra):
        ...
        try:
            ...
            if hasattr(self, '_dados_parafusos_especiais') and self._dados_parafusos_especiais:
                print(f"[DEBUG-TRANSFERENCIA] Estrutura dos dados recebidos: {type(self._dados_parafusos_especiais)}")
                print(f"[DEBUG-TRANSFERENCIA] Chaves disponíveis: {list(self._dados_parafusos_especiais.keys()) if isinstance(self._dados_parafusos_especiais, dict) else 'N/A'}")
                
                # Verificar se os dados estão em formato de lista (parafusos_a/parafusos_e)
                chave_lista = f'parafusos_{letra}'
                if chave_lista in self._dados_parafusos_especiais:
                    print(f"[DEBUG-TRANSFERENCIA] Encontrada chave {chave_lista} nos dados")
                    # Formato: {'parafusos_a': [lista], 'parafusos_e': [lista]}
                    lista_parafusos = self._dados_parafusos_especiais[chave_lista]
                    print(f"[DEBUG-TRANSFERENCIA] Lista de parafusos {letra.upper()} encontrada: {lista_parafusos} (tipo: {type(lista_parafusos)})")
                    if isinstance(lista_parafusos, list):
                        parafusos = [float(v) for v in lista_parafusos if v and float(v) > 0]
                        print(f"[DEBUG-TRANSFERENCIA] Parafusos {letra.upper()} processados: {parafusos}")
                        if parafusos:
                            print(f"[DEBUG] Parafusos especiais {letra.upper()} obtidos da lista: {parafusos}")
                ...
                # Formato antigo: {'par_a': {'par_a_1': valor, ...}, 'par_e': {'par_e_1': valor, ...}}
    ...
    def gerar_script(self):
        ...
        try:
            ...
            # Variável para determinar se devemos aplicar ajustes de compatibilidade (2.2cm)
            # Aplicar quando comprimento > 222 OU quando pilar especial estiver ativo
            # Esta variável é usada em vários lugares: METAL.A, METAL.B, Grade A, Grade B, cotas, etc.
            # pilar_especial_ativo já foi verificado anteriormente (linha 1513)
            aplicar_ajustes_compatibilidade = comprimento_pilar_global > 222 or pilar_especial_ativo
...
"""
            ...
            print(f"[DEBUG-PAI-A] Iniciando desenho de PAI.A")
            posicao_x_pai_a = aplicar_alteracao_pilar_especial(self.x_inicial-11, 'posicao', 'pai_a', 1, self)
            print(f"[DEBUG-PAI-A] posicao_x_pai_a inicial: {posicao_x_pai_a}")
            ...
            print(f"[DEBUG-PAI-A] Verificando pilar especial: {self._globais_pilar_especial is not None}")
            if self._globais_pilar_especial and self._globais_pilar_especial.get('tipo_pilar') == 'L':
                eh_pilar2 = bool(getattr(self, 'pilar_especial_ativo_var', None) and self.pilar_especial_ativo_var.get())
                print(f"[DEBUG-PAI-A] Tipo L detectado, eh_pilar2: {eh_pilar2}")
                try:
                    if eh_pilar2:
...
"""
            ...
            # Verificar se checkbox pilar especial está ativo
            checkbox_pilar_especial_ativo = bool(
                getattr(self, 'pilar_especial_ativo_var', None) 
                and self.pilar_especial_ativo_var.get()
            )
            
            if self._globais_pilar_especial and self._globais_pilar_especial.get('tipo_pilar') == 'L':
                if eh_p2:
                    # Pilar 2 (Script 2 especial): manter início e encurtar direita (-24)
                    posicao_x_pai_a = current_start_a
                    
                    # Ajuste baseado em largura_2 para Script 2 especial
                    largura_2_especial = float(self._globais_pilar_especial.get('larg_2', 20) or 20)
                    ajuste_largura_pai_a = largura_2_especial - 20.0
                    
                    # Somar o ajuste no tamanho total (aplicar ANTES de calcular end_x)
                    comprimento_pai_a += ajuste_largura_pai_a
                    
                    # Ajustar posição inicial conforme o sinal do ajuste
                    if ajuste_largura_pai_a > 0:
                        # Resultado positivo: mover posição inicial para a esquerda
                        posicao_x_pai_a -= ajuste_largura_pai_a
                        if robo_logger:
                            robo_logger.info(f"PAI.A Script 2 - Ajuste positivo ({ajuste_largura_pai_a:+.2f}cm): movendo posição inicial para esquerda, tamanho total aumentado")
                    elif ajuste_largura_pai_a < 0:
                        # Resultado negativo: mover posição inicial para a direita
                        posicao_x_pai_a -= ajuste_largura_pai_a  # Subtrair negativo = somar (mover direita)
                        if robo_logger:
                            robo_logger.info(f"PAI.A Script 2 - Ajuste negativo ({ajuste_largura_pai_a:+.2f}cm): movendo posição inicial para direita, tamanho total ajustado")
                    
                    # Calcular end_x_pai_a com o novo comprimento e ajuste de -24
                    end_x_pai_a = posicao_x_pai_a + comprimento_pai_a - 24.0
                else:
                    # Pilar 1 (Script 1 especial): manter fim atual
                    end_x_pai_a = current_end_a
                    posicao_x_pai_a = end_x_pai_a - comprimento_pai_a
                    
                    # Ajustes adicionais do retângulo PAI.A para Script 1 conforme modo
                    # Aplicar ajustes diretamente no comprimento antes de recalcular posições
                    if tipo_linha == "mline":
                        # Modo Ini: aumentar mais 2.2cm do tamanho total
                        comprimento_pai_a += 2.2
                    else:
                        # Modo Nova: diminuir 0.2cm do tamanho total
                        comprimento_pai_a -= 0.2
                    
                    # Recalcular posições com o novo comprimento, mantendo a extremidade direita fixa
                    end_x_pai_a = current_end_a
                    posicao_x_pai_a = end_x_pai_a - comprimento_pai_a
                    
                    # Ajustes finos de posição inicial após recalcular tamanho
                    if tipo_linha == "mline":
                        # Modo Ini: mover posição inicial 2.2cm para a direita
                        posicao_x_pai_a += 2.2
                        # Ajustar end_x_pai_a para manter o tamanho correto
                        end_x_pai_a = posicao_x_pai_a + comprimento_pai_a
                    else:
                        # Modo Nova: mover posição inicial 0.2cm para a esquerda
                        posicao_x_pai_a -= 0.2
                        # Ajustar end_x_pai_a para manter o tamanho correto
                        end_x_pai_a = posicao_x_pai_a + comprimento_pai_a
            elif checkbox_pilar_especial_ativo and not eh_p2:
                # Aplicar ajustes quando checkbox pilar especial está ativo e é Script 1 (não pilar 2)
                end_x_pai_a = current_end_a
                posicao_x_pai_a = end_x_pai_a - comprimento_pai_a
                
                # Ajustes adicionais do retângulo PAI.A conforme modo
                # Aplicar ajustes diretamente no comprimento antes de recalcular posições
                if tipo_linha == "mline":
                    # Modo Ini: aumentar mais 2.2cm do tamanho total
                    comprimento_pai_a += 2.2
            ...
            # Registrar referência dinâmica: usar a posição FINAL REAL que será escrita no script
            try:
                pilar_atual_id = getattr(self, 'pilar_atual', 'desconhecido')
                print(f"[REFERENCIA-DINAMICA-PRE] pilar_atual={pilar_atual_id}, eh_p2={eh_p2}, posicao_x_pai_a={posicao_x_pai_a}")
                definir_posicao_parede_esquerda_pai_a(posicao_x_pai_a)
                if robo_logger:
                    robo_logger.info(f"[REFERENCIA-DINAMICA] Parede esquerda PAI.A FINAL REAL registrada: {posicao_x_pai_a}")
                print(f"[REFERENCIA-DINAMICA] Parede esquerda PAI.A definida: {posicao_x_pai_a} (pilar_atual={pilar_atual_id})")
            except Exception as e:
                print(f"[ERRO] Falha ao definir posição PAI.A: {e}")
                pass
...
"""
            ...
            elif aplicar_ajustes_compatibilidade:
                posicao_texto_cota = self.y_inicial - 80  # 80cm abaixo quando aplicar_ajustes_compatibilidade
            else:
...
"""
            ...
            # Desenhar SAR244+.1 e SAR244+.2 se comprimento_pilar > 222 OU se pilar especial estiver ativo
            if aplicar_ajustes_compatibilidade:
                # Identificar se é pilar especial tipo L e qual script (1 ou 2)
                eh_pilar_especial_l = bool(
                    self._globais_pilar_especial
                    and self._globais_pilar_especial.get('tipo_pilar') == 'L'
                )
                eh_script_2_sar = bool(
                    eh_pilar_especial_l
                    and getattr(self, 'pilar_especial_ativo_var', None)
                    and self.pilar_especial_ativo_var.get()
                )
                eh_script_1_sar = bool(eh_pilar_especial_l and not eh_script_2_sar)
                
                # SAR244+.1 (acima do PAI.B)
                # Calcular posições base
                sar244_1_x_inicial = self.x_inicial - 11
                sar244_1_x_final = self.x_inicial + comprimento_pilar_global + 11
                
                # Ajustes específicos para SAR244+.1 em pilares especiais
                if eh_script_1_sar:
                    # Script 1 especial: diminuir tamanho 13cm + largura_2 da parte direita, manter posição inicial normal
                    # Obter largura_2 da aba pilares especiais
                    largura_2_especial = 0.0
                    if self._globais_pilar_especial:
                        largura_2_especial = float(self._globais_pilar_especial.get('larg_2', 0) or 0)
                    sar244_1_x_final -= (13.0 + largura_2_especial)
                elif eh_script_2_sar:
                    # Script 2 especial: mover ponto inicial 11.2cm para a direita, diminuir tamanho total 24.0cm (35.2 - 11.2) da extremidade direita
                    # Isso resulta em um aumento líquido de 11.2cm no tamanho total
                    sar244_1_x_inicial += 11.2
                    sar244_1_x_final -= 24.0  # 35.2 - 11.2 = 24.0 (aumenta tamanho total em 11.2cm)
                
                script += f""";
_ZOOM
C {sar244_1_x_inicial},{self.y_inicial+largura_pilar_global+4} 1
;
"""
                if tipo_linha == "mline":
                    script += f"""mline
...
S
2.2
{sar244_1_x_inicial},{self.y_inicial+largura_pilar_global+4.2}
{sar244_1_x_final},{self.y_inicial+largura_pilar_global+4.2}

;
"""
                else:
                    script += f"""_PLINE
{sar244_1_x_inicial},{self.y_inicial+largura_pilar_global+2}
{sar244_1_x_inicial},{self.y_inicial+largura_pilar_global+4.2}
{sar244_1_x_final},{self.y_inicial+largura_pilar_global+4.2}
{sar244_1_x_final},{self.y_inicial+largura_pilar_global+2}
C
;
"""
                # Calcular centro do SAR244+.1 para zoom e hatch
                sar244_1_x_centro = (sar244_1_x_inicial + sar244_1_x_final) / 2
                script += f""";
_ZOOM
C {sar244_1_x_centro},{self.y_inicial+largura_pilar_global+5.1} 1
;
"""
                if tipo_linha != "mline":
                    script += f"""HP2
{sar244_1_x_centro},{self.y_inicial+largura_pilar_global+3.1}
;
"""
                script += f"""_LAYER
S {self.config_manager.get_config("layers", "sarrafos")}
...
"""
                ...
                # SAR244+.2 (abaixo do PAI.A)
                # Calcular posições base
                sar244_2_x_inicial = self.x_inicial - 11
                sar244_2_x_final = self.x_inicial + comprimento_pilar_global + 11
                
                # Ajustes específicos para SAR244+.2 em pilares especiais
                if eh_script_1_sar:
                    # Script 1 especial: aumentar o tamanho em 0.2cm
                    sar244_2_x_final += 0.2
                elif eh_script_2_sar:
                    # Script 2 especial: mover ponto inicial 13cm para a esquerda, diminuir tamanho total em 11cm + 13cm (total 24cm) da parte direita
                    sar244_2_x_inicial -= 13.0
                    sar244_2_x_final -= 24.0  # 11 + 13 = 24cm
                
                script += f""";
_ZOOM
C {sar244_2_x_inicial},{self.y_inicial-6.2} 1
;
"""
                if tipo_linha == "mline":
                    script += f"""mline
...
S
2.2
{sar244_2_x_inicial},{self.y_inicial-2}
{sar244_2_x_final},{self.y_inicial-2}

;
"""
                else:
                    script += f"""_PLINE
{sar244_2_x_inicial},{self.y_inicial-4.2}
{sar244_2_x_inicial},{self.y_inicial-2}
{sar244_2_x_final},{self.y_inicial-2}
{sar244_2_x_final},{self.y_inicial-4.2}
C
;
"""
                # Calcular centro do SAR244+.2 para zoom e hatch
                sar244_2_x_centro = (sar244_2_x_inicial + sar244_2_x_final) / 2
                script += f""";
_ZOOM
C {sar244_2_x_centro},{self.y_inicial-5.1} 1
;
"""
                if tipo_linha != "mline":
                    script += f"""HP2
{sar244_2_x_centro},{self.y_inicial-3.1}
;
"""
                script += f"""_LAYER
S {self.config_manager.get_config("layers", "sarrafos")}
...
"""
            ...
            # CORREÇÃO: Para pilares especiais (tipo L), não desenhar cota comum do PAI.D (tanto pilar 1 quanto pilar 2)
            if not (
                self._globais_pilar_especial
                and self._globais_pilar_especial.get('tipo_pilar') == 'L'
            ):
                # Apenas para pilares comuns: usar coordenadas normais e subir o texto em 5cm
                posicao_x_pai_c = self.x_inicial
                end_x_pai_d = self.x_inicial + comprimento_pilar_global
                pos_y_texto_cota = self.y_inicial + 4  # Subir 5cm da posição atual (5 + 5 = 10)
                
                script += f"""_DIMLINEAR
{posicao_x_pai_c},{self.y_inicial}
{end_x_pai_d},{self.y_inicial}
{((posicao_x_pai_c + end_x_pai_d) / 2) + 15},{pos_y_texto_cota}
;
;
;
"""
                ...
                script += f"""_DIMLINEAR
{end_x_pai_d},{self.y_inicial}
{end_x_pai_d},{self.y_inicial + largura_pilar_global}
{end_x_pai_d + 30},{self.y_inicial + largura_pilar_global - 10}
;
;
;
"""
...
"""
            ...
            # Adicionar cotas especiais para pilares especiais
            if (
                self._globais_pilar_especial
                and self._globais_pilar_especial.get('tipo_pilar') == 'L'
            ):
                # Definir posição Y do texto da cota (comum para ambos os pilares)
                pos_y_texto_cota_especial = self.y_inicial + 5
                
                # Cota especial lateral PAI.C para pilar 1 (texto 30cm à esquerda)
                if not self.pilar_especial_ativo_var.get():  # Pilar 1
                    posicao_x_pai_c_pilar1 = self.x_inicial
                    texto_x_pai_c = posicao_x_pai_c_pilar1 - 30  # 30cm à esquerda do PAI.C
                    
                    script += f"""_DIMLINEAR
{posicao_x_pai_c_pilar1},{self.y_inicial}
{posicao_x_pai_c_pilar1},{self.y_inicial + largura_pilar_global}
{texto_x_pai_c},{pos_y_texto_cota_especial}
;
"""

                # Cota especial lateral PAI.D para pilar 2 (texto 30cm à direita)
                if self.pilar_especial_ativo_var.get():  # Pilar 2
                    posicao_x_pai_d_pilar2 = end_x_pai_b - 11  # Posição do PAI.D do pilar 2
                    texto_x_pai_d = posicao_x_pai_d_pilar2 + 30  # 30cm à direita do PAI.D
                    
                    script += f"""_DIMLINEAR
{posicao_x_pai_d_pilar2},{self.y_inicial}
{posicao_x_pai_d_pilar2},{self.y_inicial + largura_pilar_global}
{texto_x_pai_d},{pos_y_texto_cota_especial}
;
"""
...
"""
            ...
            # Desenhar METAL.A
            # Aplicar ajuste de 2.2cm quando aplicar_ajustes_compatibilidade (comprimento > 222 OU pilar especial ativo)
            if aplicar_ajustes_compatibilidade:
            ...
            # Ajustes adicionais para METAL.A quando pilar especial está ativo
            checkbox_pilar_especial_ativo_metal = bool(
                getattr(self, 'pilar_especial_ativo_var', None) 
                and self.pilar_especial_ativo_var.get()
            )
            if checkbox_pilar_especial_ativo_metal:
                # Verificar se é Script 2 (pilar 2)
                eh_p2_metal_a = bool(
                    self._globais_pilar_especial
                    and self._globais_pilar_especial.get('tipo_pilar') == 'L'
                    and checkbox_pilar_especial_ativo_metal
                )
                if eh_p2_metal_a:
                    # Script 2 especial: aumentar 2.2cm da extremidade esquerda
                    posicao_x_a -= 2.2  # Move para a esquerda (aumenta a extremidade esquerda)
                    comprimento_metal_a += 2.2  # Aumenta o comprimento para compensar
                    
                    # Ajuste fino para modo NOVA: mover posição inicial 2.4cm para a direita
                    if tipo_linha != "mline":  # Modo NOVA
                        posicao_x_a += 2.4  # Move para a direita
...
"""
            ...
            if self._globais_pilar_especial and self._globais_pilar_especial.get('tipo_pilar') == 'L':
                ...
                if not self.pilar_especial_ativo_var.get():
                    # PARTE 1 (Pilar 1) - Script 1 especial tipo L
                    ...
                    # Ajuste para Script 1 especial: diminuir 2.2cm do tamanho total (modo INI e NOVA)
                    comprimento_metal_b -= 2.2
                else:
                    # PARTE 2 (Pilar 2)
            ...
            # Ajustes adicionais para METAL.B quando pilar especial está ativo
            checkbox_pilar_especial_ativo_metal = bool(
                getattr(self, 'pilar_especial_ativo_var', None) 
                and self.pilar_especial_ativo_var.get()
            )
            
            # Verificar se é Script 1 especial tipo L (checkbox não ativo para Script 2)
            eh_script_1_tipo_l = bool(
                self._globais_pilar_especial
                and self._globais_pilar_especial.get('tipo_pilar') == 'L'
                and getattr(self, 'pilar_especial_ativo_var', None)
                and not self.pilar_especial_ativo_var.get()
            )
            
            if checkbox_pilar_especial_ativo_metal:
                # Verificar se é Script 2 (pilar 2) ou Script 1 (não pilar 2)
                eh_p2_metal_b = bool(
                    self._globais_pilar_especial
                    and self._globais_pilar_especial.get('tipo_pilar') == 'L'
                    and checkbox_pilar_especial_ativo_metal
                ...
                if eh_p2_metal_b:
                    # Script 2 especial: diminuir 2.2cm da extremidade esquerda
                    posicao_x_b += 2.2  # Move para a direita (diminui a extremidade esquerda)
                    comprimento_metal_b -= 2.2  # Diminui o comprimento para compensar
                    
                    # Ajuste fino para modo NOVA: mover posição 2.4cm para a esquerda e aumentar tamanho 4.8cm
                    if tipo_linha != "mline":  # Modo NOVA
                        posicao_x_b -= 2.4  # Move para a esquerda
                        comprimento_metal_b += 4.8  # Aumenta o tamanho na extremidade direita
                else:
                    # Script 1 especial (checkbox ativo mas não é Script 2): diminuir 2.2cm do tamanho total
                    # IMPORTANTE: Este ajuste mantém a posição inicial e diminui apenas o tamanho total (modo INI e NOVA)
                    comprimento_metal_b = comprimento_metal_b - 2.2  # Diminui o comprimento total em 2.2cm
...
"""
            ...
            if not (self._globais_pilar_especial and self._globais_pilar_especial.get('tipo_pilar') == 'L'):
                ...
                if aplicar_ajustes_compatibilidade:
                    posicao_y_cota_total += 40  # Subir 40cm quando aplicar_ajustes_compatibilidade
...
"""
            ...
            # Desenhar GRA.A
            # Aplicar ajuste de 2.2cm quando aplicar_ajustes_compatibilidade (comprimento > 222 OU pilar especial ativo)
            posicao_y_a = -(7 if not aplicar_ajustes_compatibilidade else 9.2) - 4.4 # Soma 4.8 para subir a grade
            posicao_x = 0 - 11 # Base
...
"""
                    ...
                    posicoes_blocos = self.obter_posicoes_blocos_por_detalhes(i, is_grade_b=False, base_x=posicao_x)
                    try:
                        first_block = posicoes_blocos[0] if posicoes_blocos else None
                        delta_sync = (first_block - posicao_x) if first_block is not None else None
                        print(f"[SYNC] GRA.A Grade {i+1}: rect_x={posicao_x}, first_block_x={first_block}, delta={delta_sync}")
                    except Exception:
                        pass
                    
                    # Calcular offset para TODAS as grades do Lado A do Pilar 2 (Grade E) - ANTES do loop de blocks
                    offset_grade_e = 0
                    if eh_modo_l and eh_pilar2:  # TODAS as grades do Pilar 2 (lado A)
                        posicao_parede_pai_a = obter_posicao_parede_esquerda_pai_a()
                        print(f"[GRADE-E-OFFSET-CALC] Grade {i+1}: eh_modo_l={eh_modo_l}, eh_pilar2={eh_pilar2}, posicao_parede_pai_a={posicao_parede_pai_a}")
                        if posicao_parede_pai_a is not None and posicoes_blocos:
                            # Para Grade 1: calcular offset relativo da parede esquerda do PAI.A + 2cm
                            # Para outras grades: usar o mesmo offset para manter alinhamento
                            if i == 0:
                                base_original = self.x_inicial + posicoes_blocos[0]
                                nova_base = posicao_parede_pai_a + 0
                                offset_grade_e = nova_base - base_original
                                print(f"[GRADE-E-OFFSET-CALC] Grade {i+1} (BASE): base_original={base_original}, nova_base={nova_base}, offset={offset_grade_e:+.1f}")
                            else:
                                # Grades 2, 3, etc: aplicar o mesmo offset de +2cm
                                offset_grade_e = 0.001
                                print(f"[GRADE-E-OFFSET-CALC] Grade {i+1} (OFFSET FIXO): offset={offset_grade_e:+.1f}cm")
                            if robo_logger:
                                robo_logger.info(f"[GRADE-E-OFFSET] Grade {i+1}: offset_grade_e={offset_grade_e:+.1f} será aplicado a TODOS os blocks")
...
"""
                        ...
                        # Aplicar offset da Grade E se foi calculado (todos os blocks da Grade E)
                        if offset_grade_e != 0:
                            posicao_final_block += offset_grade_e
                            print(f"[GRADE-E-BLOCK] Grade {i+1}, Block {j+1}: pos_original={self.x_inicial + posicao_ajustada}, offset={offset_grade_e:+.1f}, pos_final={posicao_final_block}")
                            if robo_logger:
                                robo_logger.info(f"[GRADE-E-BLOCK] Grade {i+1}, Block {j+1}: offset_grade_e={offset_grade_e:+.1f}, posicao_final={posicao_final_block}")
                        elif eh_modo_l and eh_pilar2 and i != 0:
                            # Outras grades do Pilar 2 (não Grade E): aplicar ajuste fixo -13cm + largura
                            posicao_block_ajustada = posicao_final_block - 13.0
                            largura_pilar_1 = float(self._globais_pilar_especial.get('larg_1', 20) or 20)
                            ajuste_largura = (largura_pilar_1 - 20.0) * -1
                            posicao_final_block = posicao_block_ajustada + ajuste_largura
                            if robo_logger:
                                robo_logger.info(f"Grade A Block - Pilar 2 (Grade {i+1}): ajuste fixo -13cm + largura {largura_pilar_1}cm (ajuste {ajuste_largura:+.1f}cm)")
                                robo_logger.info(f"Grade A Block - Pilar 2 (Grade {i+1}): {posicao_block_ajustada} -> {posicao_final_block}")
...
"""
                        ...
                        # Usar o mesmo offset calculado para Grade E
                        if offset_grade_e != 0:
                            ajuste_cota_grade_a = offset_grade_e
                            print(f"[GRADE-E-COTA] Grade {i+1}: usando offset_grade_e={offset_grade_e:+.1f} para TODAS as cotas")
                            if robo_logger:
                                robo_logger.info(f"[GRADE-E-COTA] Grade {i+1}: offset_grade_e={offset_grade_e:+.1f}")
                        elif eh_modo_l and eh_pilar2 and i != 0:
                            # Outras grades do Pilar 2 (não Grade E): aplicar ajuste fixo -13cm + largura
                            ajuste_base = -13.0
                            largura_pilar_1 = float(self._globais_pilar_especial.get('larg_1', 20) or 20)
                            ...
                            if robo_logger:
                                robo_logger.info(f"Grade A Cota - Pilar 2 (Grade {i+1}): ajuste fixo -13cm + largura {largura_pilar_1}cm (ajuste {ajuste_largura:+.1f}cm)")
                                robo_logger.info(f"Grade A Cota - Pilar 2 (Grade {i+1}): ajuste total {ajuste_cota_grade_a:+.1f}cm")
...
"""
            ...
            # Aplicar ajuste de 2.2cm quando aplicar_ajustes_compatibilidade (comprimento > 222 OU pilar especial ativo)
            posicao_y_b = largura_pilar_global + (5 if not aplicar_ajustes_compatibilidade else 7.2) + 4  + 2.4# Soma 4.8 para subir a grade
            posicao_x = 0 - 11 # Base
            ...
            if eh_modo_l and eh_pilar2:
                ...
                # Ajustes finos de posição inicial para Script 2 especial (após o ajuste de 25.4cm)
                if tipo_linha == "mline":
                    # Modo INI: mover 2.2cm para a direita
                    posicao_x += 2.2
                    if robo_logger:
                        robo_logger.info(f"Grade B - Script 2 Especial Modo INI: movendo mais 2.2cm para direita: {posicao_x - 2.2} -> {posicao_x}")
                else:
                    # Modo NOVA: mover 0.2cm para a esquerda
                    posicao_x -= 0.2
                    if robo_logger:
                        robo_logger.info(f"Grade B - Script 2 Especial Modo NOVA: movendo 0.2cm para esquerda: {posicao_x + 0.2} -> {posicao_x}")
...
"""
                    ...
                    # Calcular índice relativo dentro das grades ativas (mesma lógica do lado A)
                    # Isso garante que a primeira grade ativa tenha índice 0, segunda tenha índice 1, etc.
                    grades_ativas_anteriores = sum(1 for idx in range(i) if comprimentos_b[idx] > 0)
                    indice_relativo_grade_b = grades_ativas_anteriores
                    
                    # SAR.GRA.B baseados nos detalhes dos campos (incluindo pontas)
                    # Passar a mesma base X usada para o retângulo da grade B, para sincronizar o início
                    posicoes_blocos = self.obter_posicoes_blocos_por_detalhes(i, is_grade_b=True, base_x=posicao_x)
                    try:
                        first_block = posicoes_blocos[0] if posicoes_blocos else None
                        delta_sync = (first_block - posicao_x) if first_block is not None else None
                        print(f"[SYNC] GRA.B Grade {i+1}: rect_x={posicao_x}, first_block_x={first_block}, delta={delta_sync}")
                    except Exception:
                        pass
                    
                    # Calcular offset para Grade F (Pilar 2, modo INI) - mover 2.4cm para a esquerda
                    offset_grade_f = 0
                    print(f"[DEBUG-GRADE-F] Grade B {i+1}: eh_modo_l={eh_modo_l}, eh_pilar2={eh_pilar2}, tipo_linha={tipo_linha}")
                    if eh_modo_l and eh_pilar2 and tipo_linha == "mline":  # Modo INI, SOMENTE Pilar 2
                        offset_grade_f = -2.4
                        print(f"[GRADE-F-OFFSET] Grade {i+1}: modo INI detectado, aplicando offset={offset_grade_f:+.1f}cm (esquerda)")
                        if robo_logger:
                            robo_logger.info(f"[GRADE-F-OFFSET] Grade {i+1}: modo INI, offset={offset_grade_f:+.1f}cm será aplicado a TODOS os blocks")
                    else:
                        print(f"[DEBUG-GRADE-F] Grade B {i+1}: NÃO aplicando offset (condição não atendida)")
                    
                    # Inserir todos os blocks baseado nos detalhes
                    # CORREÇÃO: Usar índice relativo dentro das grades ativas (mesma lógica do lado A)
                    # A ordem dos blocks não deve ser invertida - deve refletir a mesma lógica dos blocks que as grades do lado A usam
                    for j, pos_x in enumerate(posicoes_blocos, 0):
                        # Definir tipo de block baseado na posição - MESMA LÓGICA DO LADO A
                        # Usar indice_relativo_grade_b em vez de i para garantir consistência
                        if j == 0:
                            # Primeiro block (ponta esquerda)
                            if grades_ativas == 1 or indice_relativo_grade_b == 0:
                                block_type = "sar_gra_b1"  # B1 para primeira grade ou grade única
                            else:
                                block_type = "sar_gra_b3"  # B3 para outras grades
                        elif j == len(posicoes_blocos) - 1:
                            # Último block (ponta direita)
                            if grades_ativas == 1 or indice_relativo_grade_b == grades_ativas - 1:
...
"""
                        ...
                        posicao_ajustada = pos_x
                        # Usar indice_relativo_grade_b em vez de i para garantir consistência
                        if j == len(posicoes_blocos) - 1 and grades_ativas > 1 and indice_relativo_grade_b < grades_ativas - 1 and block_type == "sar_gra_b3":
                            # Block da ponta direita substituído por central em grades que não são a última - mover 3.5cm para esquerda
                            posicao_ajustada = pos_x - 3.5
                            ...
                            print(f"[DEBUG-AJUSTE] Grade B - Sem ajuste - j={j}, grades_ativas={grades_ativas}, indice_relativo={indice_relativo_grade_b}, block_type={block_type}")
                        
                        # Aplicar ajuste de posição para pilares especiais nos blocks dos detalhes da Grade B
                        # Sincronizar sem adicionar deslocamento extra (25.4 já foi aplicado ao retângulo)
                        posicao_final_block = self.x_inicial + posicao_ajustada
                        
                        # Aplicar offset da Grade F se foi calculado (modo INI)
                        if offset_grade_f != 0:
                            posicao_final_block += offset_grade_f
                            print(f"[GRADE-F-BLOCK] Grade {i+1}, Block {j+1}: pos_original={self.x_inicial + posicao_ajustada}, offset={offset_grade_f:+.1f}, pos_final={posicao_final_block}")
                            if robo_logger:
                                robo_logger.info(f"[GRADE-F-BLOCK] Grade {i+1}, Block {j+1}: offset_grade_f={offset_grade_f:+.1f}, posicao_final={posicao_final_block}")
...
"""
                    ...
                    # Remover ajuste duplicado nas cotas (já sincronizado pela base X)
                    ajuste_cota_grade_b = 0
                    
                    # Aplicar offset da Grade F (modo INI) às cotas
                    if offset_grade_f != 0:
                        ajuste_cota_grade_b = offset_grade_f
                        print(f"[GRADE-F-COTA] Grade {i+1}: usando offset_grade_f={offset_grade_f:+.1f} para TODAS as cotas")
                        if robo_logger:
                            robo_logger.info(f"[GRADE-F-COTA] Grade {i+1}: offset_grade_f={offset_grade_f:+.1f}")
            ...
            if self._globais_pilar_especial and self._globais_pilar_especial.get('tipo_pilar') == 'L':
                try:
                    ...
                    if parafusos_especiais:
                        ...
                        # CORREÇÃO DEFINITIVA: Para pilar 2 especial, desenhar 1 parafuso a menos
                        if pilar_atual == 2:
                            quantidade_parafusos = max(1, len(distancias) - 1)
                            print(f"[CORREÇÃO-DEFINITIVA] Pilar 2 especial: {len(distancias)} distâncias -> {quantidade_parafusos} parafusos (reduzido em 1)")
                            ...
                            if debug_parafusos:
                                debug_parafusos.parafuso_correcao(
                                    ...
                                    "Pilar 2 especial - reduzido em 1"
                                )
                            ...
                            # NOVO: Regra para Pilar 1 especial - se (comp_1 - larg_2) < 40, forçar apenas 1 parafuso
                            try:
                                comp_1_qtd = float(self._globais_pilar_especial.get('comp_1', 0) or 0)
                                larg_2_qtd = float(self._globais_pilar_especial.get('larg_2', 0) or 0)
                                diferenca_pilar_1 = comp_1_qtd - larg_2_qtd
                                
                                if diferenca_pilar_1 < 40.0:
                                    quantidade_parafusos = 1
                                    print(f"[PILAR-1-REGRA] Forçando 1 parafuso: comp_1 - larg_2 = {diferenca_pilar_1:.1f} < 40")
                                    if debug_parafusos:
                                        debug_parafusos.info(f"PILAR 1 REGRA: comp1={comp_1_qtd}, larg2={larg_2_qtd}, expr={diferenca_pilar_1:.1f} < 40")
                                else:
                                    quantidade_parafusos = len(distancias)
                                    print(f"[PILAR-1-NORMAL] Usando quantidade normal: comp_1 - larg_2 = {diferenca_pilar_1:.1f} >= 40")
                            except Exception:
                                quantidade_parafusos = len(distancias)
                                print(f"[PILAR-1-ERRO] Erro ao calcular regra, usando quantidade normal: {quantidade_parafusos}")
                        else:
                            quantidade_parafusos = len(distancias)
...
"""
            ...
            if (
                ...
                posicao_x_parafuso = self.x_inicial + 49.5
                # NOVO: Se (comp_2 - larg_1) < 80, mover a posição inicial +20cm para a direita
                try:
                    comp_2_regra = float(self._globais_pilar_especial.get('comp_2', 0) or 0)
                    larg_1_regra = float(self._globais_pilar_especial.get('larg_1', 0) or 0)
                    if (comp_2_regra - larg_1_regra) < 65.0:
                        posicao_x_parafuso += 30.0
                        print(f"[PARAFUSO-REGRA] Pos. inicial deslocada +20cm (comp_2 - larg_1 < 80)")
                        if robo_logger:
                            robo_logger.info("🔧 PARAFUSO-REGRA: posição inicial +20cm (comp_2 - larg_1 < 80)")
                except Exception:
                    pass
                # NOVO: Regra especial quando (comp_2 - larg_1) < 80: mover posição inicial + larg_1
                try:
                    comp_2_regra = float(self._globais_pilar_especial.get('comp_2', 0) or 0)
                    larg_1_regra = float(self._globais_pilar_especial.get('larg_1', 0) or 0)
                    diferenca_regra = comp_2_regra - larg_1_regra
                    if diferenca_regra < 65.0:
                        posicao_x_parafuso = posicao_x_parafuso + larg_1_regra
                        print(f"[PARAFUSO-REGRA] Pos. inicial + larg_1: +{larg_1_regra}cm (comp_2 - larg_1 = {diferenca_regra} < 80)")
                except Exception:
                    pass
            ...
            # Lista para armazenar posições reais dos parafusos (para cotas do pilar 2)
            posicoes_parafusos_reais = []
            ...
            if pilar_atual == 2:
                ...
                # Para cotas do Pilar 2, manter regra de 2 parafusos (não reduzir)
                quantidade_parafusos_cotas_pilar2 = max(2, quantidade_parafusos - 2)
                ...
                print(f"[COTAS-PILAR2] Parafusos para cotas: {quantidade_parafusos} -> {quantidade_parafusos_cotas_pilar2} (reduzido em 2 para cotas)")
                # NOVO: Se (comp_2 - larg_1) < 80, forçar apenas 1 parafuso simples
                try:
                    comp_2_regra = float(self._globais_pilar_especial.get('comp_2', 0) or 0)
                    larg_1_regra = float(self._globais_pilar_especial.get('larg_1', 0) or 0)
                    diferenca_regra = comp_2_regra - larg_1_regra
                    if diferenca_regra < 65.0:
                        quantidade_parafusos_corrigida = 1
                        print(f"[PARAFUSO-REGRA] Forçando 1 parafuso (comp_2 - larg_1 = {diferenca_regra} < 80)")
                except Exception:
                    pass
            elif pilar_atual == 1 and is_pilar_especial:
                # Para pilar 1 especial, desenhar 1 parafuso a menos (penúltimo vira último)
                quantidade_parafusos_corrigida = max(2, quantidade_parafusos - 1)
                print(f"[CORREÇÃO-DIRETA] Pilar 1 especial: {quantidade_parafusos} -> {quantidade_parafusos_corrigida} parafusos (reduzido em 1, penúltimo vira último)")
                
                # NOVO: Regra adicional para Pilar 1 especial - se (comp_1 - larg_2) < 40, forçar apenas 1 parafuso
                try:
                    comp_1_regra = float(self._globais_pilar_especial.get('comp_1', 0) or 0)
                    larg_2_regra = float(self._globais_pilar_especial.get('larg_2', 0) or 0)
                    diferenca_pilar_1_regra = comp_1_regra - larg_2_regra
                    
                    if diferenca_pilar_1_regra < 40.0:
                        quantidade_parafusos_corrigida = 1
                        print(f"[PILAR-1-REGRA-CORRIGIDA] Forçando 1 parafuso: comp_1 - larg_2 = {diferenca_pilar_1_regra:.1f} < 40")
                        if debug_parafusos:
                            debug_parafusos.info(f"PILAR 1 REGRA CORRIGIDA: comp1={comp_1_regra}, larg2={larg_2_regra}, expr={diferenca_pilar_1_regra:.1f} < 40")
                except Exception:
                    print(f"[PILAR-1-ERRO-CORRIGIDA] Erro ao calcular regra, mantendo quantidade: {quantidade_parafusos_corrigida}")
            else:
...
"""
                ...
                # CORREÇÃO: Para pilares especiais (pilar 2), calcular posição usando distâncias acumuladas
                if (self._globais_pilar_especial and self._globais_pilar_especial.get('tipo_pilar') == 'L' and 
                    hasattr(self, '_dados_parafusos_especiais') and self._dados_parafusos_especiais and
                    getattr(self, 'pilar_especial_ativo_var', None) and self.pilar_especial_ativo_var.get()):
                    # Para pilares especiais (pilar 2), usar posição inicial + distâncias acumuladas
                    posicao_x_atual = posicao_x_parafuso
                    for j in range(i):
                        if j < len(distancias):
                            posicao_x_atual += distancias[j]
                    # NOVO: Se (comp_2 - larg_1) < 80, mover o primeiro parafuso para a esquerda pela diferença para 80
                    try:
                        comp_2_regra = float(self._globais_pilar_especial.get('comp_2', 0) or 0)
                        larg_1_regra = float(self._globais_pilar_especial.get('larg_1', 0) or 0)
                        diferenca_regra = comp_2_regra
                        if diferenca_regra < 85.0:
                            desloc_esq = 81.0 - diferenca_regra
                            posicao_x_atual = posicao_x_atual - desloc_esq
                            print(f"[PARAFUSO-REGRA] Parafuso {i+1} deslocado {-desloc_esq}cm (para a esquerda)")
                    except Exception:
                        pass
                    
                    # NOVO: Deslocamento extra para Pilar 2 especial quando comp_2 < 85
                    try:
                        comp_2_extra = float(self._globais_pilar_especial.get('comp_2', 0) or 0)
                        larg_1_regra = float(self._globais_pilar_especial.get('larg_1', 0) or 0)
                        if comp_2_extra < 85.0:
                            desloc_extra = 10 + larg_1_regra # 10cm para a esquerda
                            posicao_x_atual = posicao_x_atual - desloc_extra
                            print(f"[PARAFUSO-EXTRA] Pilar 2 especial: comp_2={comp_2_extra} < 85, deslocamento extra {-desloc_extra}cm")
                            if robo_logger:
                                robo_logger.info(f"🔧 PARAFUSO-EXTRA: Pilar 2 especial - comp_2={comp_2_extra} < 85, deslocamento extra {-desloc_extra}cm")
                    except Exception:
                        pass
                else:
                    # Para pilares comuns (pilar 1), usar lógica original
                    posicao_x_atual = posicao_x_parafuso
                    if largura_especial and (i == 0 or i == quantidade_parafusos - 1):
                ...
                # Armazenar posição real do parafuso para uso nas cotas (sempre para pilar 2)
                if pilar_atual == 2:
                    posicoes_parafusos_reais.append(posicao_x_atual)
                    print(f"[POSICAO-ARMAZENADA] Pilar 2 - Parafuso {i+1}: posição X = {posicao_x_atual:.2f}")
...
"""
                ...
                if not usar_mline:
                    ...
                    # CORREÇÃO: Para pilar 1 especial, o último parafuso (que era penúltimo) deve ter detalhes como central
                    # CORREÇÃO: Para pilar 2 especial, o primeiro parafuso deve ter detalhes como central
                    is_pilar_1_especial = (pilar_atual == 1 and is_pilar_especial)
                    is_pilar_2_especial = (pilar_atual == 2 and is_pilar_especial)
                    is_ultimo_parafuso = (i == quantidade_parafusos_corrigida - 1)
                    is_primeiro_parafuso = (i == 0)
                    # NOVO: Forçar simples (sem detalhes centrais) quando (comp_2 - larg_1) < 80 para o primeiro parafuso do pilar 2 especial
                    forcar_simples_primeiro = False
                    try:
                        comp_2_regra = float(self._globais_pilar_especial.get('comp_2', 0) or 0)
                        larg_1_regra = float(self._globais_pilar_especial.get('larg_1', 0) or 0)
                        diferenca_regra = comp_2_regra - larg_1_regra
                        forcar_simples_primeiro = (is_pilar_2_especial and is_primeiro_parafuso and diferenca_regra < 65.0)
                    except Exception:
                        pass
                    
                    if forcar_simples_primeiro or (is_primeiro_parafuso and not is_pilar_2_especial) or (is_ultimo_parafuso and not is_pilar_1_especial):
                        # Parafusos das extremidades: NÃO inserir blocos PAR_CIMA e PAR_BAIXO
                        # Exceção: pilar 1 especial - último parafuso tem detalhes como central
                        # Exceção: pilar 2 especial - primeiro parafuso tem detalhes como central
                        pass
                    else:
...
"""
                ...
                # Aplicar ajuste de 2.2cm nos blocks de parafusos quando aplicar_ajustes_compatibilidade
                # (comprimento > 222 OU pilar especial ativo) para manter compatibilidade
                if aplicar_ajustes_compatibilidade:
                    y_superior_block += 2.2  # Block do topo sobe 2.2cm
                    y_inferior_block -= 2.2  # Block do fundo desce 2.2cm
...
"""
                ...
                if i < quantidade_parafusos_corrigida - 1 and not is_pilar_2:
                    ...
                    # Ajustar pontos conforme solicitado: primeiro parafuso 1cm para direita
                    ...
                    # Ajuste para última cota: aplicar -1cm no ponto direito
                    if i == quantidade_parafusos_corrigida - 2:  # Penúltima iteração = última cota
                        if not is_pilar_especial:  # Apenas para pilares comuns
                            ponto_direito -= 1.0  # Move 1cm para esquerda
...
"""
            ...
            if is_pilar_2:
                print(f"[COTAS-PILAR2] Verificando geração de cotas: {len(posicoes_parafusos_reais)} parafusos armazenados")
                if len(posicoes_parafusos_reais) > 1:
                    # Calcular y_inferior para as cotas (mesmo cálculo usado nos parafusos)
                    if comprimento_pilar_global < 223:
                        altura_parafuso = largura_pilar_global + 36.8
                    else:
                        altura_parafuso = largura_pilar_global + 41.2
                    ...
                    print(f"[COTAS-PILAR2] Gerando cotas para {len(posicoes_parafusos_reais)} parafusos")
                    print(f"[COTAS-PILAR2] Posições: {posicoes_parafusos_reais}")
                    
                    # Usar as posições reais dos parafusos que foram desenhados
...
"""
                ...
                # Desenhar cotas entre parafusos usando as posições reais
                for i in range(len(posicoes_parafusos_reais) - 1):
                    # Ponto esquerdo da cota (posição real do parafuso atual)
                    ponto_esquerdo = posicoes_parafusos_reais[i]
                    
                    # Ponto direito da cota (posição real do próximo parafuso)
                    ponto_direito = posicoes_parafusos_reais[i + 1]
                    
                    # AJUSTE: Para a última cota (último parafuso válido), ajustar 1.5cm para a esquerda
                    if i == len(posicoes_parafusos_reais) - 2:  # Última cota (penúltimo para último)
                        ponto_direito = ponto_direito - 1.5  # Ajuste de 1.5cm para a esquerda
                        print(f"[COTA-PILAR2] Última cota ajustada: ponto direito movido 1.5cm para esquerda ({posicoes_parafusos_reais[i + 1]:.2f} -> {ponto_direito:.2f})")
                    ...
                    if distancia_cota > 0.1:
                        ...
                        # Adicionar cota usando as posições reais dos parafusos
                        script += f"""_DIMLINEAR
{ponto_esquerdo},{y_inferior}
{ponto_direito},{y_inferior}
{(ponto_esquerdo + ponto_direito) / 2},{posicao_texto_cota}
;
"""
                        print(f"[COTA-PILAR2] Cota {i+1}: {ponto_esquerdo:.2f} -> {ponto_direito:.2f} = {distancia_cota:.2f}cm")
                
                # Adicionar cota da ponta esquerda do PAI.A até o primeiro parafuso (apenas para pilar 2 especial)
                if len(posicoes_parafusos_reais) > 0:
                    # Usar a posição real do primeiro parafuso
                    posicao_primeiro_parafuso = posicoes_parafusos_reais[0]
                    
                    # Usar a posição final calculada do PAI.A (posicao_x_pai_a) que inclui os ajustes
                    esquina_esquerda_pai_a = posicao_x_pai_a
                    
                    # Verificar se a distância é maior que zero para evitar cotas de valor 0
                    distancia_extra_esquerda = abs(posicao_primeiro_parafuso - esquina_esquerda_pai_a)
                    if distancia_extra_esquerda > 0.1:  # Só desenhar cota se a distância for maior que 0.1cm
                        # Posição do texto da cota: mesma altura das cotas dos parafusos existentes (30cm)
                        posicao_texto_cota_esquerda = y_inferior - 30
                        
                        script += f"""_DIMLINEAR
{esquina_esquerda_pai_a},{y_inferior}
{posicao_primeiro_parafuso},{y_inferior}
{(esquina_esquerda_pai_a + posicao_primeiro_parafuso) / 2},{posicao_texto_cota_esquerda}
;
"""
                        print(f"[COTA-PILAR2] Cota extra esquerda: {esquina_esquerda_pai_a:.2f} -> {posicao_primeiro_parafuso:.2f} = {distancia_extra_esquerda:.2f}cm")
...
;
"""
            # Verificar modo (NOVA/INI) - DEFINIR ANTES DE QUALQUER USO
            ...
            if self.config_manager.get_config("drawing_options", "incluir_texto_nome") == "sim":
                ...
                if desenhar_pai_c and desenhar_pai_d:
                    ...
                    script += f"""_MTEXT
{self.x_inicial+30},{(self.y_inicial+largura_pilar_global/2)+100}
J MC
0
{nome_pilar}
({comprimento_texto}X{largura_texto})
...
"""
                    ...
                    if modo_nova:
                        ...
                        # CORREÇÃO: Mostrar apenas o nome do pilar, sem dimensões
                        script += f"""_MTEXT
...
"""
            ...
            if self.config_manager.get_config("drawing_options", "textos_abcd") == "blocos":
                # Usar blocos TA, TB, TC, TD com regras para pilares especiais
                if desenhar_pai_c and desenhar_pai_d:
                    # PILAR NORMAL: blocos padrão
                    script += f"""-INSERT
...
"""
                    ...
                    # PILAR 1 ESPECIAL: TD não desenha, TC movido
                    script += f"""-INSERT
{self.config_manager.get_config("blocks", "ta")}
{self.x_inicial+comprimento_pilar_global/2+10},{self.y_inicial+5}
1
...
-INSERT
{self.config_manager.get_config("blocks", "tc")}
{self.x_inicial+5+2},{self.y_inicial+largura_pilar_global/2-2+3}
1
0
;
"""
                elif not desenhar_pai_c:
                    # PILAR 2 ESPECIAL: TC não desenha, TD movido
                    # Modo INI: substituir TA por TE e TB por TF
                    modo_ini = not modo_nova  # Modo INI é quando tipo_linha != "pline"
                    
                    if modo_ini:
                        # Modo INI: usar TE e TF
                        block_ta = self.config_manager.get_config("blocks", "te")
                        block_tb = self.config_manager.get_config("blocks", "tf")
                        print(f"[SCRIPT2-INI] Modo INI detectado: substituindo TA por TE ({block_ta}) e TB por TF ({block_tb})")
                    else:
                        # Modo NOVA: usar TA e TB normalmente
                        block_ta = self.config_manager.get_config("blocks", "ta")
                        block_tb = self.config_manager.get_config("blocks", "tb")
                    
                    script += f"""-INSERT
{block_ta}
{self.x_inicial+comprimento_pilar_global/2+10-30},{self.y_inicial+5}
1
0
;
-INSERT
{block_tb}
{self.x_inicial+comprimento_pilar_global/2-10-20},{self.y_inicial+largura_pilar_global-5}
1
0
;
-INSERT
{self.config_manager.get_config("blocks", "td")}
{self.x_inicial+comprimento_pilar_global-6-25},{self.y_inicial+largura_pilar_global/2-7+3}
1
0
;
"""
...
"""
            ...
            # Adicionar parafusos horizontais apenas se largura >= 40cm E NÃO for pilar especial
            # Para pilares especiais, não adicionar parafusos horizontais independente da largura
            pilar_especial_ativo = self._verificar_pilar_especial_ativo()
            
            # Log da decisão sobre parafusos horizontais
            if pilar_especial_ativo:
                self.log_mensagem(f"PILAR ESPECIAL DETECTADO - Parafusos horizontais DESABILITADOS para largura {largura_pilar_global}cm", "info")
            elif largura_pilar_global >= 40:
                self.log_mensagem(f"PILAR COMUM - Adicionando parafusos horizontais para largura {largura_pilar_global}cm", "info")
            else:
                self.log_mensagem(f"PILAR COMUM - Largura {largura_pilar_global}cm < 40cm, parafusos horizontais não aplicados", "info")
            
            if largura_pilar_global >= 40 and not pilar_especial_ativo:
...
"""
                ...
                # Aplicar ajuste de 2.2cm quando aplicar_ajustes_compatibilidade para manter compatibilidade
                if aplicar_ajustes_compatibilidade:
                    posicao_y_horizontal_superior += 2.2  # Parafuso horizontal superior sobe 2.2cm
...
"""
                ...
                # Aplicar ajuste de 2.2cm quando aplicar_ajustes_compatibilidade para manter compatibilidade
                if aplicar_ajustes_compatibilidade:
                    posicao_y_horizontal_inferior -= 2.2  # Parafuso horizontal inferior desce 2.2cm
...
"""
                    ...
                    script += f"""_SCALE
{ponto1_x - 300},{ponto1_y + 200}
{ponto2_x + 200},{ponto2_y - 200}
...
"""
    ...
    def atualizar_preview(self):
        try:
            ...
            # Considerar apenas valores positivos (evita desenhar quando zero/não definido)
            distancias_validas = [d for d in distancias if d > 0]

            # Desenhar os parafusos (exceto o segundo parafuso se comprimento < 30)
            # CORREÇÃO: As distâncias são as posições absolutas dos parafusos em relação à origem
            for i, distancia_atual in enumerate(distancias_validas):
                # Ajustar posição do parafuso para condições especiais (largura >= 40cm)
                # CORREÇÃO: Para pilares especiais (pilar 2), calcular posição usando distâncias acumuladas
                if (self._globais_pilar_especial and self._globais_pilar_especial.get('tipo_pilar') == 'L' and 
                    hasattr(self, '_dados_parafusos_especiais') and self._dados_parafusos_especiais and
                    ...
                    posicao_x_atual = posicao_x_parafuso
                    for j in range(i):
                        if j < len(distancias_validas):
                            posicao_x_atual += distancias_validas[j]
                else:
                    # Para pilares comuns (pilar 1), usar posição absoluta em relação à origem
                    posicao_x_atual = posicao_x_parafuso + distancia_atual
                    if largura_especial and (i == 0 or i == len(distancias_validas) - 1):
                        if i == 0:  # Primeiro parafuso - mover para dentro (2.5cm da parede)
                            posicao_x_atual = self.x_inicial - 14.5  # 2.5cm para fora da parede esquerda
                        else:  # Último parafuso - mover para dentro (2.5cm da parede)
                            posicao_x_atual = self.x_inicial + comprimento_pilar + 14.5  # 2.5cm para fora da parede direita
                ...
                # Para posições absolutas não acumulamos a origem; mantemos -12 para todos
    ...
    def obter_posicoes_blocos_por_detalhes(self, index_grade, is_grade_b=False, base_x=None):
        """
        ...
        if eh_pilar_especial:
            ...
            if letra_grade:
                ...
                # Usar a mesma lógica das grades especiais: acessar detalhes das globais ou dos campos da interface
                # Primeiro, tentar obter das globais do pilar especial
                globais = obter_globais_pilar_especial()
                if globais and 'detalhes_grades_especiais' in globais:
                    print(f"[DEBUG] Usando detalhes das globais do pilar especial")
                    detalhes_especiais = globais['detalhes_grades_especiais']
                    
                    # Mapear letra_grade para grade_num (1, 2, 3)
                    # Para pilares especiais, usamos sempre grade_num = 1 para simplificar
                    # mas podemos mapear corretamente se necessário
                    for i in range(1, 6):  # 5 detalhes
                        campo_nome = f"detalhe_{letra_grade}_1_{i}"
                        valor = detalhes_especiais.get(campo_nome, '0')
                        print(f"[DEBUG] Detalhes globais {campo_nome}: '{valor}'")
                        detalhes.append(valor)
                        if valor and str(valor) != '0':
                            campos_encontrados += 1
                else:
                    # Fallback: usar atributos do gerador se existirem
                    if is_grade_b:
                ...
                posicoes = []
                # Usar a mesma base X que o retângulo utilizou, se fornecida
                posicao_x = base_x if base_x is not None else self.calcular_posicao_inicial_grade(index_grade)
