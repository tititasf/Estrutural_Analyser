...
pattern = re.compile(r'    def _on_save_sa_clicked\(self\):.*?QTimer\.singleShot\(2000, lambda: self\._btn_save_sa\.setText\("💾 Salvar Alterações"\)\)', re.DOTALL)
...
if pattern.search(content):
    content = pattern.sub(def_new, content)
    open(fpath, 'w', encoding='utf-8').write(content)
    print('Sucesso injetando no comparison engine!')
else:
    print('Metodo _on_save_sa_clicked nao encontrado via Regex!')
